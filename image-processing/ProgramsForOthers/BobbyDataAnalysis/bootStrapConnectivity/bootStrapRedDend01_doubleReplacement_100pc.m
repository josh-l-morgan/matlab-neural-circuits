...
histX = [0:1:10];
maxAx = max(preIn);
realCountAx = hist(preIn(postIn == 1),1:1:maxAx);
realHistAx = hist(realCountAx,histX);
bar(histX,realHistAx); pause(.01)

histAxSyn = hist(preIn,1:1:maxAx);
redFraction = sum(postIn ==1)/length(postIn);
predictRed = histAxSyn * redFraction;

%% Find deviance
realDifs = realCountAx-predictRed;
realDeviance = (1/redFraction).^((realDifs.^2)./histAxSyn);
realMeanDeviance = mean(realDeviance);

hist(realDeviance,0:1:100)

cat(1,histAxSyn,realCountAx,realDeviance)'
pause
...
%     newDend = post(newPos);
    %%  count real mults
    CountAx = hist(newAx(newDend == 1),1:1:maxAx);
    
    
    difs = CountAx-predictRed;
    Deviance(i,:) = (1/redFraction).^((difs.^2)./histAxSyn);

    
    %CountAll = hist(newAx,1:1:maxAx);
    %HistAx(i,:) = hist(CountAx,histX);
...
%% Scrutinize Deviance
capDeviance = Deviance;
capDeviance(Deviance>10) = 10;
meanDevHist = hist(mean(capDeviance,1),[0:1:20]);
bar(meanDevHist)

meanTestDeviance = mean(capDeviance,2);
realCapDeviance = realDeviance;
realCapDeviance(realCapDeviance>10) = 10;
Pdev = sum(meanTestDeviance>=mean(realCapDeviance))/reps
...
% modelOnes = HistAx(:,2);
% modelOnes = sort(modelOnes,'ascend');
% Pones = sum(modelOnes>=realHistAx(2))/reps;
