function[cP] = anaBulbs(Iraw,showResult);
if ~exist('showResult')
    showResult = 1;
end
voxelSize = [1 1 1];
...
scatter3(cents(:,1),cents(:,2),cents(:,3),'.','r')
%% Connect Watershed
...
for i = 2: numWI
    ...
    nodes = cat(1,cents(i,:),nodes); %start with a node list that is the center followed by all the joints
    ...
    elseif size(nodes,1) == 2  % 1 joint (end point)
        ...
        if 1
            ...
            nPair = [ones(length(jIDs)-1,1) [2:length(jIDs)]']; %simple center to joint edges created
            
        else %old way broken due to mystery 'paired'
            
            nPair = zeros(size(nodes,1),2);
            for n = 1:size(nodes,1)  %find first pairings (nearest neighbors)
                dists = myDist(nodes,nodes(n,:));
                nPair(n,:) = sort([n find(dists == min(dists(dists>0)),1)]);
            end
...
%         pos(1,:) = n(pr(p,1),:);
%         pos(2,:) = n(pr(p,2),:);
%         plot(pos(:,2),pos(:,1),'linewidth',2,'color','r');
%     end
%     hold off
%         pause
% 
    ...
    if 1 %show 3d
        ...
        for i = 1:length(skel)
            ...
            for p = 1:size(pr,1)
                pos(1,:) = n(pr(p,1),:);
                pos(2,:) = n(pr(p,2),:);
                plot3(pos(:,2),pos(:,1),pos(:,3),'linewidth',5,'color','r');
            end
        end
