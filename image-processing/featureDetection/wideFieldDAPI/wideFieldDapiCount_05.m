...
https://docs.google.com/spreadsheets/d/1IsauOeMB0Aq4frOAf1WXlt1Z_wawKTtmSshAEbutdog/edit#gid=0

%}


%SPN = uigetfolder('H:\Victoria\AMIRA\Tiff\')
autoThreshold = 0; %run auto threshold 1 or manual 0
...
for ix = 9;%1:length(uId)
    ...
    if hasImage(1) & hasImage(4) & sum(hasImage(2:3))>0 % then process
        ...
        if isempty(runSubset) %use subset of planes or not
            useZ = 1:zs;
        else
            useZ = runSubset;
        end
        ...
        if 1%ifShow
            ...
            Isum = zeros(size(Si,1),size(Si,2),3,'double');
            for i = 1:us
        ...
        if autoThreshold
            ...
            needCount = 3;
            startCount = 0;
            wasLow = areaCount(valPeak);
            bestDif = 0;
            for p = valPeak:length(maxValCount)-1

                remainingMax = max(maxValCount(p:end));
                currentDif = remainingMax-maxValCount(p);
            ...
            if (sum(maxVals>threshIntensity)/length(maxVals))<.2
                threshIntensity = 2;
            end
            ...
            areaPeak = find(areaRange==300);

            %gm = fitgmdist([areaRange' areaCount'],2)


            countUp = 0;
            needCount = 3;
            startCount = 0;
            wasLow = areaCount(areaPeak);
            bestDif = 0;
            for p = areaPeak:-1:2

                remainingMax = max(areaCount(1:p));
                currentDif = remainingMax-areaCount(p);
                if currentDif >bestDif
                    bestDif = currentDif;
                    startCount = p;
                end
    ...
    dat(ix).hasImage = hasImage;
    dat(ix).YXZ = cat(2,passCent(:,[2 1]),passPropZ);
    dat(ix).areaFCI = [sum(areaF) sum(areaC) sum(areaI)];
    dat(ix).countFCI = [sum(isFull) sum(isContra) sum(isIpsi)];
    dat(ix).areaF=sum(areaF)
    dat(ix).areaC=sum(areaC)
    dat(ix).areaI=sum(areaI)
    dat(ix).countF=sum(isFull)
    dat(ix).countC=sum(isContra)
    dat(ix).countI=sum(isIpsi)
    
    dat(ix).p.maxIntensity = maxVals(pass);
    dat(ix).p.area = areas(pass);
    dat(ix).p.isFull = find(isFull);
    dat(ix).p.isContra = find(isContra);
    dat(ix).p.isIpsi = find(isIpsi);
    dat(ix).p.unfilteredCentroids = centroids;

    dat(ix).z.areaF = areaF;
    dat(ix).z.areaC = areaC;
    dat(ix).z.areaI = areaI;
    dat(ix).z.dapiFullZ = dapiFullZ;
    dat(ix).z.dapiContraZ = dapiContraZ;
    dat(ix).z.dapiIpsiZ = dapiIpsiZ;
    
    dat(ix).Isum = Isum;


    save([SPN 'dat2.mat'],'dat')
