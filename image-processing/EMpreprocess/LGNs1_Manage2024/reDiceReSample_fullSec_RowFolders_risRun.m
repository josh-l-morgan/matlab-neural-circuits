...
SPN = '/storage1/fs1/jlmorgan/Active/morganLab/DATA/LGN/LGNs1/2023transfer/mip0_batch2/'
TPN = '/storage1/fs1/jlmorgan/Active/morganLab/DATA/LGN/LGNs1/2023transfer/LGNs1_vsvi_risRun_matlab/';
...
for s =1:length(secDir)
    ...
    if exist(oldDir,'dir')
        ...
        for i = 1:length(Ir)
            ...
            for y = 1:fac
                currentRow = (rs(i))*fac + y;
                rowFolder = sprintf('%d/', currentRow) ;
                if ~exist([newDir rowFolder],'dir'),mkdir([newDir rowFolder]);end
                for x = 1:fac
                    yStart = (y-1)* newRes + 1;
                    xStart = (x-1) * newRes + 1;
                    ...
                    if (yStart<ys) & ( xStart<xs)
                        ...
                        newFileNames{cnt} = sprintf('%s%d.png',rowFolder,ncs(cnt));
        ...
        for m = 1:length(mipDir)
            ...
            clear newFileNames
            for i = 1:length(uNewInd)
                rowDir = sprintf('%s%d',newDir,uNewRow(i));
                if ~exist(rowDir,'dir'),mkdir(rowDir);end
                newFileNames{i} = sprintf('%d/%d.png',uNewRow(i),uNewCol(i));
            end
