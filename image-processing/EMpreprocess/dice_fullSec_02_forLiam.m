...
mipDir = {'6'};
invert = 0;
writeEmpty = 0; %write tiles that sum to 0

%18858       18251


dSPN = dir(SPN); %dSPN = dir([SPN '*.png']);

iNams = {dSPN.name};

for i = 3:5 %length(iNams)
    disp(sprintf('running section %d of %d',i,length(iNams)))
    nam = iNams{i};
    I = imread([SPN nam]);
    z = num2str(i);
    if ~exist([TPN z],'dir'),mkdir([TPN z]);end

    for m = 1:length(mipDir)

        if ~exist([TPN z '\' mipDir{m} ],'dir'),mkdir([TPN z '\' mipDir{m}]);end
        mV = str2num(mipDir{m});
        if mV
            Id = imresize(I,1/(2^mV),'bilinear');
        else
            Id = I;
        end
        ...
        for r = 1:length(ys)
            for c = 1:length(xs)
                ...
                yStop = min(ys(r)+tileRes-1,size(Id,1));
                xStop = min(xs(c)+tileRes-1,size(Id,2));

                tile = Id(ys(r):yStop,xs(c):xStop);
                if (size(tile,1)<tileRes) | (size(tile,2) <tileRes)
                    tile(tileRes,tileRes) = 0;
                end

                tileSum = sum(tile(:));

                if tileSum | writeEmpty 
                    if invert
                        tile = 256-tile;
                    end
                    ...
                    tileName = sprintf('%d_%d.tif',r,c);


                    for p = 1:10
                        pass = 1;
                        try
                            imwrite(uint8(tile),[TPN z '\' mipDir{m} '\' tileName],'Compression','lzw')
                        catch err
                            pass = 0;
                            pause(3)
                        end
