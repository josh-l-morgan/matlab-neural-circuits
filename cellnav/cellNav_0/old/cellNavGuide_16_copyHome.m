function varargout = cellNavGuide_16(varargin)
% CELLNAVGUIDE_16 MATLAB code for cellNavGuide_16.fig
%      CELLNAVGUIDE_16, by itself, creates a new CELLNAVGUIDE_16 or raises the existing
%      singleton*.
%
%      H = CELLNAVGUIDE_16 returns the handle to a new CELLNAVGUIDE_16 or the handle to
%      the existing singleton*.
%
%      CELLNAVGUIDE_16('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CELLNAVGUIDE_16.M with the given input arguments.
%
%      CELLNAVGUIDE_16('Property','Value',...) creates a new CELLNAVGUIDE_16 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cellNavGuide_16_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cellNavGuide_16_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cellNavGuide_16

% Last Modified by GUIDE v2.5 09-Jun-2020 17:57:58
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @cellNavGuide_16_OpeningFcn, ...
    'gui_OutputFcn',  @cellNavGuide_16_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before cellNavGuide_16 is made visible.
function cellNavGuide_16_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cellNavGuide_16 (see VARARGIN)


% Choose default command line output for cellNavGuide_16
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes cellNavGuide_16 wait for user response (see UIRESUME)
...
d = dir(glob.fvDir)
if ~exist([glob.fvDir 'tis.mat'],'file')
    
    if exist('.\lastFvDir.mat')
        load(['.\lastFvDir.mat']);
        if exist(lastFvDir,'dir')
...
function varargout = cellNavGuide_16_OutputFcn(hObject, eventdata, handles)
...
plot(handles.axesDepth,surface,depth)
set(handles.axesDepth,'XLim',[0 1],'YLim',[-.5 1.5])
