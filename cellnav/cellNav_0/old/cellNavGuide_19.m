function varargout = cellNavGuide_19(varargin)
% CELLNAVGUIDE_19 MATLAB code for cellNavGuide_19.fig
%      CELLNAVGUIDE_19, by itself, creates a new CELLNAVGUIDE_19 or raises the existing
%      singleton*.
%
%      H = CELLNAVGUIDE_19 returns the handle to a new CELLNAVGUIDE_19 or the handle to
%      the existing singleton*.
%
%      CELLNAVGUIDE_19('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CELLNAVGUIDE_19.M with the given input arguments.
%
%      CELLNAVGUIDE_19('Property','Value',...) creates a new CELLNAVGUIDE_19 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cellNavGuide_19_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cellNavGuide_19_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cellNavGuide_19

% Last Modified by GUIDE v2.5 12-Jun-2020 13:55:49
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @cellNavGuide_19_OpeningFcn, ...
    'gui_OutputFcn',  @cellNavGuide_19_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before cellNavGuide_19 is made visible.
function cellNavGuide_19_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cellNavGuide_19 (see VARARGIN)

% Choose default command line output for cellNavGuide_19
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes cellNavGuide_19 wait for user response (see UIRESUME)
...
function varargout = cellNavGuide_19_OutputFcn(hObject, eventdata, handles)
...
if val
    ...
    for i = 1:length(showCells)
        ...
        if exist(fileName,'file')
            load(fileName);
            glob.p.preCells(i) = renderFVnav(fv,[.2 .4 1],.6)
...
if val
    ...
    for i = 1:length(showCells)
        ...
        if exist(fileName,'file')
            load(fileName);
             glob.p.postCells(i) = renderFVnav(fv,[1 .4 .2],.6)
