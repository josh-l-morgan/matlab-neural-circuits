function varargout = cellNavGuide_29(varargin)
% CELLNAVGUIDE_29 MATLAB code for cellNavGuide_29.fig
%      CELLNAVGUIDE_29, by itself, creates a new CELLNAVGUIDE_29 or raises the existing
%      singleton*.
%
%      H = CELLNAVGUIDE_29 returns the handle to a new CELLNAVGUIDE_29 or the handle to
%      the existing singleton*.
%
%      CELLNAVGUIDE_29('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CELLNAVGUIDE_29.M with the given input arguments.
%
%      CELLNAVGUIDE_29('Property','Value',...) creates a new CELLNAVGUIDE_29 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cellNavGuide_29_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cellNavGuide_29_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cellNavGuide_29

% Last Modified by GUIDE v2.5 15-Jun-2020 17:08:20
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @cellNavGuide_29_OpeningFcn, ...
    'gui_OutputFcn',  @cellNavGuide_29_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before cellNavGuide_29 is made visible.
function cellNavGuide_29_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cellNavGuide_29 (see VARARGIN)

% Choose default command line output for cellNavGuide_29
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes cellNavGuide_29 wait for user response (see UIRESUME)
...
function varargout = cellNavGuide_29_OutputFcn(hObject, eventdata, handles)
...
I = getframe(handles.mainAx);
I = I.cdata;
imwrite(I,[glob.save.dir glob.save.fileName '.png']);
...
save([glob.save.dir glob.save.fileName '.mat'],'glob')
...
if isfield(glob.syn,'g')
    L = length(glob.syn.g)+1;
else
    L = 1;
end

glob.syn.g(L).idx = glob.syn.listSynIdx;

glob.syn.g(L).col = repmat(glob.syn.col,[length(glob.syn.listSynIdx) 1]);
glob.g(L).alph = glob.syn.alph;

if glob.syn.preTypeID>0
    typeName1 = tis.cells.type.typeNames{glob.syn.preTypeID};
    if glob.syn.preSubTypeID>0
        subTypeName1 = tis.cells.type.subTypeNames{glob.syn.preTypeID}{glob.syn.preSubTypeID};
    else
        subTypeName1 = '-';
    end
else
...
if glob.syn.postTypeID>0
    typeName2 = tis.cells.type.typeNames{glob.syn.postTypeID};
    if glob.syn.postSubTypeID>0
        subTypeName2 = tis.cells.type.subTypeNames{glob.syn.postTypeID}{glob.syn.postSubTypeID};
    else
        subTypeName1 = '-';
    end
else
...
glob.syn.g(L).name = sprintf('%d %s %s to %s %s',L,typeName1,subTypeName1,typeName2,subTypeName2);
showSynGroup(L)

groupNames ={glob.syn.g.name};
...
% handles    structure with handles and user data (see GUIDATA)
global glob
glob.syn.markerSize = glob.syn.markerSize ^ .9;
glob.syn.markerSize = max(1,glob.syn.markerSize);
...
glob.syn.markerSize = glob.syn.markerSize ^ 1.1;
glob.syn.markerSize = max(1,glob.syn.markerSize);
...
val = get(glob.handles.togglebutton_synDisplay,'Value');

if val

try glob.syn.p.delete
end

    
    %preType = get(glob.handles.
    
    uPreType = glob.syn.preTypeID;
    uPostType = glob.syn.postTypeID;
    uPreSubType = glob.syn.preSubTypeID;
    uPostSubType = glob.syn.postSubTypeID;
    uPrePickIdx = glob.syn.prePickIdx;
    uPostPickIdx = glob.syn.postPickIdx;
