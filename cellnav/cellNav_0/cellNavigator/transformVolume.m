...
% Last Modified by GUIDE v2.5 24-Jun-2020 20:52:00
...
tform = globTranslate.tform;
volID = get(handles.popupmenu_chooseVolume,'Value');
glob.vol.tforms{volID} = tform;

save([glob.dir.Volumes glob.vol.names{volID} '\tform.mat'],'tform');
...
for i = 1:size(pts1str)
    ...
    X = str2num(line(nums(1):nums(new(1))));
    Y = str2num(line(nums(new(1)+1):nums(new(2))));
    Z = str2num(line(nums(new(2)+1):nums(end)));
    pts1(i,:) = [Y X Z];
end
...
scatter3(handles.axes1,pts1(:,1),pts1(:,2),pts1(:,3),'o','k','filled')
hold on
scatter3(handles.axes1,pts2(:,1),pts2(:,2),pts2(:,3),'o','b','filled')
scatter3(handles.axes1,pts3(:,1),pts3(:,2),pts3(:,3),'o','r','filled')
hold off
rotate3d on
% 
% 
