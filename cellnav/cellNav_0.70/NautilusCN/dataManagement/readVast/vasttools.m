...
% Version 1.2.1.29x by Daniel Berger, August 2014 - June 04 2020

function [] = vasttools()
  global vdata;
  if isfield(vdata,'state')
    waitfor(warndlg('VastTools is already running. Close other instance. If necessary use "clear -global vdata" to fix.','Error starting VastTools'));
    return;
  end;
  ...
  if (vdata.vast.thisversionnr~=5)
    waitfor(warndlg(sprintf('WARNING: This version of VastTools expects API version 5, but VastControlClass.m reports version %d. This may lead to problems.',vdata.vast.thisversionnr),'VastTools - API version mismatch'));
  end;
  ...
  vdata.fh = figure('units','pixels',...
    ...
    'menubar','none',...
    'numbertitle','off',...
    'name','VastTools Version 1.2.1.29x (c) June 04 2020 by Daniel Berger');
  ...
  vdata.ui.menu.exportobj = uimenu(vdata.ui.menu.exportmenu,'Label','Export 3D Objects as Mesh Files ...','Callback',{@callback_exportobj},'Enable','off');
  ...
  vdata.ui.menu.exportskeletonmodels = uimenu(vdata.ui.menu.exportmenu,'Label','Export Skeleton 3D Models ...','Callback',{@callback_exportskeleton3dmodels},'Enable','off');
  ...
  vdata.ui.menu.measurebrightness = uimenu(vdata.ui.menu.measuremenu,'Label','Measure Masked Image Brightness ...','Callback',{@callback_measurebrightness},'Enable','off');
...
function [] = updategui()
  ...
  if ((vdata.state.guiblocked)||(vdata.state.isconnected==0))
    ...
    set(vdata.ui.menu.exportskeletonmodels,'Enable','off');
    set(vdata.ui.menu.measurevolumes,'Enable','off');
    set(vdata.ui.menu.measurebrightness,'Enable','off');
    ...
    set(vdata.ui.menu.exportskeletonmodels,'Enable','on');
    set(vdata.ui.menu.measurevolumes,'Enable','on');
    set(vdata.ui.menu.measurebrightness,'Enable','on');
...
function [] = callback_connect(varargin)
  ...
  if (vdata.state.isconnected==0)
    ...
    if (res==0)
      waitfor(warndlg(['ERROR: Connecting to VAST at ' vdata.state.connect.ip ' port ' sprintf('%d',vdata.state.connect.port) ' failed. Please enable the Remote Control Server in VAST (in the main menu under "Window", "Remote Control API Server"; click "Enable") and make sure the IP and Port settings are correct!'],'Error connecting to VAST'));
      return;
    end;
...
%     set(vdata.ui.menu.euclidiantool,'Enable','on');
    ...
    if (res==0)
      waitfor(warndlg('ERROR: Disconnecting from VAST failed.','Error disconnecting from VAST'));
      return;
    end
    vdata.state.isconnected=0;
...
function [] = callback_exportobj(varargin)
  ...
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No volume open in VAST.','VastTools OBJ exporting'));
    return;
  end;
...
%   end;
  ...
  if (~isfield(vdata.data,'exportobj'))
    ...
    vdata.data.exportobj.fileformat=1;
...
%   vz=vinfo.voxelsizez;
  ...
  str{1}=sprintf('Mip %d - (%.2f nm, %.2f nm, %.2f nm) voxels',0,vinfo.voxelsizex,vinfo.voxelsizey,vinfo.voxelsizez);
  for i=1:1:vinfo.nrofmiplevels-1
    vx=vinfo.voxelsizex*mipscalematrix(i,1);
    vy=vinfo.voxelsizey*mipscalematrix(i,2);
    vz=vinfo.voxelsizez*mipscalematrix(i,3);
    str{i+1}=sprintf('Mip %d - (%.2f nm, %.2f nm, %.2f nm) voxels',i,vx,vy,vz);
    %vx=vx*2; vy=vy*2;
  end;
  ...
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','File Format:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  str=cell(2,1);
  str{1}='.OBJ / .MTL (Wavefront OBJ)';
  str{2}='.PLY (ASCII; Polygon, Stanford Triangle)';
  pmh4 = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.exportobj.fileformat,'Position',[120 vpos 290 20]);
  vpos=vpos-30;
  ...
  if (vdata.state.lastcancel==0)
    ...
    vdata.data.exportobj.fileformat=get(pmh4,'value');
  ...
  if (vdata.state.lastcancel==0)
    ...
    if (vdata.data.exportobj.objectcolors==2)
      if (~isfield(vdata.data,'measurevol'))
        waitfor(warndlg('ERROR: To use segment volume colors, please compute volumes first ("Measure / Measure Segment Volumes" in the main menu)!','Export 3D Surfaces as OBJ Files'));
        releasegui();
        return;
      end;
...
function [] = callback_region_settobbox(varargin)
  ...
  if (min(size(bbx))==0)
    waitfor(warndlg('ERROR - Cannot set area, bounding box undefined.','VastTools - Bounding Box'));
    return;
  end;
  ...
  set(vdata.temp.e_zmax,'String',sprintf('%d', vdata.data.region.zmax));
  if (varargin{3}==1) callback_update_targetimagesize(); end;
  waitfor(warndlg('Area set to combined bounding box of selected segment and its children. - CAUTION: Bounding boxes may be not always correct. Please make sure you cover the intended region.','Using Bounding Boxes in VastTools'));
          ...
          if (min(size(values))>0)
            ...
            while ((segnr<=size(values,1))&&(vdata.state.lastcancel==0))
              ...
              if (size(v,1)>0)
                ...
                v(:,1)=v(:,1)*param.yscale*param.yunit*mipfactx;
                v(:,2)=v(:,2)*param.xscale*param.xunit*mipfacty;
...
%     end;
  ...
  f2d=z(f2);
  %if (size(f2d,1)>size(f2d,2))
  if (size(f2d,2)==1) 
    %Workaround for special case because Matlab changes single rows to single columns for no good reason.
    f2d=f2d';
  end;
...
function vertlines2obj_mtllink(v,ln,filename,objectname,mtlfilename,materialname,appendflag,isfirst,vidofs)
  % Save a set of vertex coordinates and lines as a Wavefront/Alias Obj file
  % VERTFACE2OBJ(v,f,fname)
  %     v is a Nx3 matrix of vertex coordinates.
  %     ln is a Mx2 matrix of vertex indices.
  %     fname is the filename to save the obj file.
  ...
  for i=1:1000:size(ln,1)
    str=[];
    for j=i:min([i+999 size(ln,1)])
      str=[str sprintf('l %d %d\n',ln(j,1)+vidofs,ln(j,2)+vidofs)];
    end;
    if (min(size(str))>0)
      fprintf(fid,str);
    end;
...
function savematerialfile(filename,materialname,color,spec,appendflag)
  ...
  fprintf(fid,'# MTL writer by Daniel Berger, Apr 2020\n');
...
function savemeshtoply(v,f,filename,r,g,b)
  fid = fopen(filename, 'wt');
  fprintf(fid,'ply\n');
  fprintf(fid,'format ascii 1.0\n');
  fprintf(fid,'comment File written by VastTools.m, by Daniel Berger, April 2020\n');
...
function savelinestoply(v,ln,filename,r,g,b)
  fid = fopen(filename, 'wt');
  fprintf(fid,'ply\n');
  fprintf(fid,'format ascii 1.0\n');
  fprintf(fid,'comment File written by VastTools.m, by Daniel Berger, April 2020\n');
...
function savelinestoswc(vmu,ln,diamu,filename)
  fid = fopen(filename, 'wt');
  fprintf(fid,'# SWC file written by VAST/VastTools.m\n');
  fprintf(fid,'# id type x(mu) y(mu) z(mu) radius(mu) parentid\n');
  ln=int32(ln);
  ln(ln(:,1)==0,1)=-1;
  for i=1:size(ln,1);
    fprintf(fid,'%d %d %.4f %.4f %.4f %.4f %d\n',ln(i,2),0,vmu(i,1),vmu(i,2),vmu(i,3),diamu(i),ln(i,1));
  end
  fprintf(fid,'\n');
  fclose(fid);
...
function [] = callback_exportparticles(varargin)
  ...
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No volume open in VAST.','VastTools Particle Cloud exporting'));
    return;
  end;
  ...
  nrofsegments=vdata.vast.getnumberofsegments();
  if (nrofsegments==0)
    waitfor(warndlg('ERROR: No segmentation available in VAST.','VastTools Particle Cloud exporting'));
    return;
  end;
...
function [] = exportparticles()
  ...
  if (param.skipmodelgeneration==0)
    if (min(size(param.sourcemodelobj))==0)
      waitfor(warndlg('ERROR: No 3D model file selected.','Loading source model failed!'));
    ...
    if ((size(v,2)==0)||(size(f,2)==0))
      waitfor(warndlg([sprintf('ERROR: %d vertices and %d triangles found in "',size(v,2),size(f,2)) vdata.data.exportcloud.sourcemodelobj '". Aborting.'],'Loading source model failed!'));
      set(vdata.ui.message,'String','Aborted.');
      set(vdata.ui.cancelbutton,'Enable','off');
      vdata.state.lastcancel=0;
      return;
    ...
    for s=1:length(seg)
      if (vdata.state.lastcancel==0)
        ...
        if (min(size(vcoordsmu))>0)
          ...
          set(vdata.ui.message,'String',{'Exporting Particle Clouds ...', ['Saving ' filename ' as Wavefront OBJ.....']});
          pause(0.01);
          ...
          savematerialfile(mtlfilenamewithpath,materialname,seg{s}.colors/255,1.0,0);
  ...
  if ((vdata.data.exportcloud.savecoords==1)&&(vdata.state.lastcancel==0))
    ...
    if (fid>0)
      ...
      waitfor(warndlg(['WARNING: Could not open "' vdata.data.exportcloud.coordsfilename '" for writing.'],'Saving coordinates file failed!'));
    end;
...
function [] = callback_exportcloud_sourcemodelbrowse(varargin)
  ...
  if (filename~=0)
    ...
    if ((size(v,2)==0)||(size(f,2)==0))
      waitfor(warndlg([sprintf('ERROR: %d vertices and %d triangles found in "',size(v,2),size(f,2)) vdata.data.exportcloud.sourcemodelobj '".'],'Loading source model failed!'));
...
function [] = callback_exportbox(varargin)
  ...
  if (vdata.state.lastcancel==0)
    ...
    if (sum(planedir)>1)
      waitfor(warndlg('ERROR: Zero volume box and not a plane. Aborting.','Export 3D Box as OBJ File'));
      releasegui();
      return; 
    end;
    ...
    if (sum(planedir)==1)
      if (vdata.data.exportbox.style==1)
        waitfor(warndlg('ERROR: Wireframe planes are currently not supported. To export a wireframe model please define a nonplanar box. Aborting.','Export 3D Box as OBJ File'));
        releasegui();
        return;
      end;
      ...
      case 3 %Screenshots texture
        ...
        [selectedlayernr, selectedemlayernr, selectedsegmentlayernr, res] = vdata.vast.getselectedlayernr();
        [mipscalematrix, res] = vdata.vast.getmipmapscalefactors(selectedemlayernr);
        mfact=mipscalematrix(vdata.data.exportbox.miplevel,:);
        ...
        xmin=floor(vdata.data.region.xmin/mfact(1));
        xmax=floor(vdata.data.region.xmax/mfact(1))-1;
        ymin=floor(vdata.data.region.ymin/mfact(2));
        ymax=floor(vdata.data.region.ymax/mfact(2))-1;
        
        %zsections=vdata.data.region.zmin:vdata.data.exportbox.slicestep:vdata.data.region.zmax;
        zsections=floor(vdata.data.region.zmin/mfact(3)):max([1 floor(vdata.data.exportbox.slicestep/mfact(3))]):floor(vdata.data.region.zmax/mfact(3));
        tsizex=(xmax-xmin+1);
        tsizey=(ymax-ymin+1);
...
%           boxtex(tsizez+tsizey+1,tsizez+tsizex+1:end,:)=boxtex(tsizez+tsizey,tsizez+tsizex+1:end,:);
      ...
      if (vdata.state.lastcancel==0)
        if (usetexture==0)
          ...
          savematerialfile(materialfilenamewithpath, materialname, objectcolor, 0.0,0);
        else
...
function [] = callback_box_getvoxelsize(varargin)
  ...
  if (vdata.state.isconnected)
    ...
    waitfor(warndlg('ERROR: Not connected to VAST. please connect before using this function.','Not connected to VAST'));
  end;
...
function [] = callback_exportscalebar(varargin)
  ...
  vinfo=vdata.vast.getinfo();
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No volume open in VAST.','VastTools Export 3D Scale Bar as OBJ File'));
    return;
  end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Skeleton Exporting Functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  
function [] = callback_exportskeleton3dmodels(varargin)
  global vdata;
  
  if (~checkconnection()) return; end;
  vinfo=vdata.vast.getinfo();
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No volume open in VAST.','VastTools - Export Skeletons as 3D Models'));
    return;
  end;
  
  [slaynr, semlaynr, salaynr, sslaynr, stlaynr, res] = vdata.vast.getselectedlayernr2();
  if (salaynr<0)
    waitfor(warndlg('ERROR: No annotation layer open in VAST.','VastTools - Export Skeletons as 3D Models'));
    return;
  end;
  
  blockgui();
  
  if (~isfield(vdata.data,'exportsk'))
    vdata.data.exportsk.xscale=0.001;  %Use these to scale the exported models
    vdata.data.exportsk.yscale=0.001;
    vdata.data.exportsk.zscale=0.001;
    vdata.data.exportsk.xunit=vinfo.voxelsizex;%6*4;  %in nm
    vdata.data.exportsk.yunit=vinfo.voxelsizey; %6*4;  %in nm
    vdata.data.exportsk.zunit=vinfo.voxelsizez; %30; %in nm
    vdata.data.exportsk.offsetx=0; %to translate the exported models in space
    vdata.data.exportsk.offsety=0;
    vdata.data.exportsk.offsetz=0;
    vdata.data.exportsk.invertnormals=0;
    vdata.data.exportsk.swapxy=1;
    vdata.data.exportsk.invertz=1;
    
    vdata.data.exportsk.extractwhich=2;
    vdata.data.exportsk.targetfileprefix='Skeleton_';
    vdata.data.exportsk.fileformat=1;
    vdata.data.exportsk.objectcolors=1;
    vdata.data.exportsk.targetfolder=pwd;
    vdata.data.exportsk.includefoldernames=1;
  end;
  
  
  scrsz = get(0,'ScreenSize');
  figheight=340;
  f = figure('units','pixels','position',[50 scrsz(4)-100-figheight 490 figheight],'menubar','none','numbertitle','off','name','VastTools - Export Skeletons as 3D Models','resize','off');
  pos = get(f,'position');
  ax = axes('units','pix','outerposition',[0 0 pos([3 4])],'position',[0 0 pos([3 4])],'parent',f,'visible','off','xlim',[0 pos(3)],'ylim',[0 pos(4)]);
  vpos=figheight-40;
  
  uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 150 15],'String','Scale models by    X:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e11 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[170 vpos 50 20],'String',sprintf('%f',vdata.data.exportsk.xscale),'horizontalalignment','left');
  uicontrol('Style','text', 'Units','Pixels', 'Position',[240 vpos 150 15],'String','Y:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e12 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[260 vpos 50 20],'String',sprintf('%f',vdata.data.exportsk.yscale),'horizontalalignment','left');
  uicontrol('Style','text', 'Units','Pixels', 'Position',[330 vpos 150 15],'String','Z:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e13 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[350 vpos 50 20],'String',sprintf('%f',vdata.data.exportsk.zscale),'horizontalalignment','left');
  vpos=vpos-30;
  
  uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 150 15],'String','Model output offset   X:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e14 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[170 vpos 50 20],'String',sprintf('%f',vdata.data.exportsk.offsetx),'horizontalalignment','left');
  uicontrol('Style','text', 'Units','Pixels', 'Position',[240 vpos 150 15],'String','Y:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e15 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[260 vpos 50 20],'String',sprintf('%f',vdata.data.exportsk.offsety),'horizontalalignment','left');
  uicontrol('Style','text', 'Units','Pixels', 'Position',[330 vpos 150 15],'String','Z:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e16 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[350 vpos 50 20],'String',sprintf('%f',vdata.data.exportsk.offsetz),'horizontalalignment','left');
  vpos=vpos-40;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15], 'Tag','t1','String','Export what:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  str=cell(4,1);
  str{1}='All skeletons individually, uncollapsed';
  str{2}='All skeletons, collapsed as in VAST';
  str{3}='Selected object and children, uncollapsed';
  str{4}='Selected object and children, collapsed as in VAST';

  pmh2 = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.exportsk.extractwhich,'Position',[120 vpos 290 20]);
  vpos=vpos-30;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','File name prefix:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  e20 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[120 vpos 290 20],'String',vdata.data.exportsk.targetfileprefix,'horizontalalignment','left');
  vpos=vpos-30;
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','Object colors:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  str=cell(2,1);
  str{1}='Object colors from VAST';
  str{2}='Edge weights as JET colormap';
  pmh3 = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.exportsk.objectcolors,'Position',[120 vpos 290 20]);
  vpos=vpos-30;
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','Target folder:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  vdata.temp.e21 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[120 vpos 290 20],'String',vdata.data.exportsk.targetfolder,'horizontalalignment','left');
  p = uicontrol('Style','PushButton', 'Units','Pixels', 'Position',[420 vpos 60 20], 'String','Browse...', 'CallBack',{@callback_exportsk_browse});
  vpos=vpos-30;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','File Format:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  str=cell(4,1);
  str{1}='.OBJ / .MTL (Wavefront OBJ) single files';
  str{2}='.OBJ / .MTL (Wavefront OBJ) concatenated in one file';
  str{3}='.PLY (ASCII; Polygon, Stanford Triangle)';
  str{4}='.SWC (ASCII; Standardized neuron morphology format)';
  pmh4 = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.exportsk.fileformat,'Position',[120 vpos 290 20]);
  vpos=vpos-30;
  
  c1 = uicontrol('Style','checkbox', 'Units','Pixels', 'Position',[30 vpos 250 15],'Value',vdata.data.exportsk.includefoldernames,'string','Include Vast folder names in file names','backgroundcolor',get(f,'color')); 
  c2 = uicontrol('Style','checkbox', 'Units','Pixels', 'Position',[300 vpos 200 15],'Value',vdata.data.exportsk.invertz,'string','Invert Z axis','backgroundcolor',get(f,'color')); 
  vpos=vpos-25;

  %c3 = uicontrol('Style','checkbox', 'Units','Pixels', 'Position',[300 vpos 130 15],'Value',vdata.data.exportsk.invertnormals,'string','Invert Normals','backgroundcolor',get(f,'color'));
  %c4 = uicontrol('Style','checkbox', 'Units','Pixels', 'Position',[30 vpos 200 15],'Value',vdata.data.exportsk.onefile,'string','Concatenate all in one file','backgroundcolor',get(f,'color'));
  
  c3 = uicontrol('Style','checkbox', 'Units','Pixels', 'Position',[30 vpos 250 15],'Value',vdata.data.exportsk.invertnormals,'string','Invert Normals','backgroundcolor',get(f,'color'));
  c4 = uicontrol('Style','checkbox', 'Units','Pixels', 'Position',[300 vpos 200 15],'Value',vdata.data.exportsk.swapxy,'string','Swap X and Y axes','backgroundcolor',get(f,'color'));
  ...
  if (vdata.state.lastcancel==0)
    %vdata.data.exportsk.xunit = str2num(get(e8,'String'));
    %vdata.data.exportsk.yunit = str2num(get(e9,'String'));
    %vdata.data.exportsk.zunit = str2num(get(e10,'String'));
    vdata.data.exportsk.xscale = str2num(get(e11,'String'));
    vdata.data.exportsk.yscale = str2num(get(e12,'String'));
    vdata.data.exportsk.zscale = str2num(get(e13,'String'));
    vdata.data.exportsk.offsetx = str2num(get(e14,'String'));
    vdata.data.exportsk.offsety = str2num(get(e15,'String'));
    vdata.data.exportsk.offsetz = str2num(get(e16,'String'));
    
    vdata.data.exportsk.extractwhich=get(pmh2,'value');
    vdata.data.exportsk.exportmodestring=get(pmh2,'string');
    vdata.data.exportsk.exportmodestring=vdata.data.exportsk.exportmodestring{vdata.data.exportsk.extractwhich};
    vdata.data.exportsk.objectcolors=get(pmh3,'value');
    vdata.data.exportsk.targetfileprefix=get(e20,'String');
    vdata.data.exportsk.targetfolder=get(vdata.temp.e21,'String');
    vdata.data.exportsk.fileformat=get(pmh4,'value');
    vdata.data.exportsk.includefoldernames = get(c1,'value');
    vdata.data.exportsk.invertz = get(c2,'value');
    vdata.data.exportsk.invertnormals = get(c3,'value');
    vdata.data.exportsk.swapxy = get(c4,'value');
    %vdata.data.exportsk.write3dsmaxloader = get(c4,'value');
  ...
  if (vdata.state.lastcancel==0)
    if ((vdata.data.exportsk.targetfolder(end)~='/')&&(vdata.data.exportsk.targetfolder(end)~='\'))
      vdata.data.exportsk.targetfolder=[vdata.data.exportsk.targetfolder '/'];
    end;
    
    if ((vdata.data.exportsk.xunit==0)||(vdata.data.exportsk.yunit==0)||(vdata.data.exportsk.zunit==0))
      res = questdlg(sprintf('Warning: The voxel size is set to (%f,%f,%f) which will result in a zero-sized model. Are you sure you want to continue?',vdata.data.exportbar.xunit,vdata.data.exportbar.yunit,vdata.data.exportbar.zunit),'Export 3D Scale Bar as OBJ File','Yes','No','Yes');
      if strcmp(res,'No')
        releasegui();
        return; 
  ...
  %%% CONTINUE PROGRAMMING HERE
    exportskeleton3dmodels();
  end;
  
  releasegui();
  
function [] = callback_exportsk_browse(varargin)
  global vdata;
  foldername = uigetdir(vdata.data.exportsk.targetfolder,'VastTools - Select target folder for OBJ files:');
  if (foldername~=0)
    set(vdata.temp.e21,'String',foldername);
    vdata.data.exportsk.targetfolder=foldername;
  end;
  
function [] = exportskeleton3dmodels()
  global vdata;
  
  if (~checkconnection()) return; end;
  set(vdata.ui.cancelbutton,'Enable','on');
  
  param=vdata.data.exportsk;
  %rparam=vdata.data.region;
  
  set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...','Loading Metadata ...'});
  pause(0.1);

  [data,res] = vdata.vast.getannolayerobjectdata();
  [name,res] = vdata.vast.getannolayerobjectnames();
%   seglayername=getselectedseglayername();
  maxobjectnumber=max(data(:,1));
%   
%   xmin=bitshift(rparam.xmin,-param.miplevel);
...
%   
  ...
  if (param.includefoldernames==1)
    ...
    for i=1:1:size(data,1)
      j=i;
      while data(j,15)~=0 %Check if parent is not 0
        j=data(j,15);
        fullname{i}=[name{j} '.' fullname{i}];
      end;
    end;
  ...
  % Compute list of objects to export
  switch param.extractwhich
    case {1, 2}  %All segments individually, uncollapsed
    %case 2 %moved for now because collapsing doesn't work atm
      objects=uint32([data(:,1) data(:,2)]);
      %vdata.vast.setsegtranslation([],[]);
      
%     case 2  %All segments, collapsed as in Vast
%       %4: Collapse segments as in the view during segment text file exporting
%       objects=unique(data(:,19));
%       objects=uint32([objects data(objects,3)]);
%       %vdata.vast.setsegtranslation(data(:,1),data(:,18));
      
    case {3, 4}  %Selected segment and children, uncollapsed
    %case 4
      selected=find(bitand(data(:,3),65536)>0);
      if (min(size(selected))==0)
        objects=uint32([data(:,1) data(:,3)]);
      else
        selected=[selected getchildtreeids_anno(data,selected)];
        objects=uint32([selected' data(selected,3)]);
      end;
      %vdata.vast.setsegtranslation(data(selected,1),data(selected,1));
...
%       end;
  ...
  sk=cell(size(objects,1),1);
  for s=1:length(sk)
    sk{s}.id=objects(s,1);
    sk{s}.name=name{objects(s,1)};
    sk{s}.colors=data(objects(s,1),4:6);
    %sk{s}.coords=[];
    %sk{s}.volumes=[];
  end;
  
  %Compute list of coordinates
  set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...',sprintf('Loading skeleton data 1/%d...',length(sk))});
  pause(0.1);
  
  s=1;
  while ((s<=length(sk))&&(vdata.state.lastcancel==0))
    [sk{s}.aodata, res] = vdata.vast.getannoobject(objects(s,1));
    
    if (mod(s,100)==0)
      set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...',sprintf('Loading skeleton data %d/%d...',s,length(sk))});
      pause(0.1);
    end;
...
%   
  if (vdata.state.lastcancel==0)
%     %Build particle cloud models and save
    set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...','Saving skeleton models ...'});
    pause(0.1);

    if (param.fileformat==2)
      combinedfilename=[param.targetfolder param.targetfileprefix sprintf('concatenated.obj')];
      combinedmtlfilename=[param.targetfileprefix sprintf('concatenated.mtl')];
      combinedmtlfilenamewithpath=[combinedfilename(1:end-3) 'mtl'];
      
      %Delete if exists
      if (exist(combinedfilename, 'file') == 2)
        delete(combinedfilename);
      end;
      if (exist(combinedmtlfilenamewithpath, 'file') == 2)
        delete(combinedmtlfilenamewithpath);
      end;
    end;
    ...
    for s=1:length(sk)
      if (vdata.state.lastcancel==0)
        if (sk{s}.aodata.type==1) %skeletons only
          sid=sk{s}.id;
          vcoordsmu=double(sk{s}.aodata.nodematrix(:,[2 3 4]));
          if (min(size(vcoordsmu))>0)
            vcoordsmu(:,1)=vcoordsmu(:,1)*param.xscale*param.xunit;
            vcoordsmu(:,2)=vcoordsmu(:,2)*param.yscale*param.yunit;
            vcoordsmu(:,3)=vcoordsmu(:,3)*param.zscale*param.zunit;
...
%           fp=fp';
            ...
            %add offset if requested
            if (vdata.data.exportsk.offsetx~=0)
              vcoordsmu(:,1)=vcoordsmu(:,1)+vdata.data.exportsk.offsetx;
            end;
            if (vdata.data.exportsk.offsety~=0)
              vcoordsmu(:,2)=vcoordsmu(:,2)+vdata.data.exportsk.offsety;
            end;
            if (vdata.data.exportsk.offsetz~=0)
              vcoordsmu(:,3)=vcoordsmu(:,3)+vdata.data.exportsk.offsetz;
            end;
            
            %Swap X and Y to make consistent with other model exports
            if (vdata.data.exportsk.swapxy==1)
              vcoordsmu=[vcoordsmu(:,2) vcoordsmu(:,1) vcoordsmu(:,3)];
            end;
            
            ln=[sk{s}.aodata.nodematrix(:,5) [1:size(sk{s}.aodata.nodematrix,1)]'];
            ln2=ln;
            ln((ln(:,1)==0),:)=[]; %remove nonexistent edge
            diamu=sk{s}.aodata.nodediameters/1000;
            ...
            switch param.fileformat
              case 1 %OBJ single files
                filename=[param.targetfolder param.targetfileprefix sprintf('_%04d_%s.obj',sid,on)];
                objectname=[param.targetfileprefix sprintf('_%04d_%s',sid,sk{s}.name)];
                mtlfilename=[param.targetfileprefix sprintf('_%04d_%s.mtl',sid,on)];
                mtlfilenamewithpath=[filename(1:end-3) 'mtl'];
                materialname=[param.targetfileprefix sprintf('_%04d_material',sid)];
                set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...', ['Saving ' filename ' as Wavefront OBJ.....']});
                pause(0.01);
                vertlines2obj_mtllink(vcoordsmu,ln,filename,objectname,mtlfilename,materialname,0,1,0);
                savematerialfile(mtlfilenamewithpath,materialname,double(sk{s}.colors)/255.0,1.0,0);
              case 2 %OBJ concatenated single file
                %filename=[param.targetfolder param.targetfileprefix sprintf('_%04d_%s.obj',sid,on)];
                objectname=[param.targetfileprefix sprintf('_%04d_%s',sid,sk{s}.name)];
                %mtlfilename=[param.targetfileprefix sprintf('_%04d_%s.mtl',sid,on)];
                %mtlfilenamewithpath=[filename(1:end-3) 'mtl'];
                materialname=[param.targetfileprefix sprintf('_%04d_material',sid)];
                set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...', ['Saving to ' combinedfilename ' as Wavefront OBJ.....']});
                pause(0.01);
                vertlines2obj_mtllink(vcoordsmu,ln,combinedfilename,objectname,combinedmtlfilename,materialname,1,isfirst,vidofs);
                savematerialfile(combinedmtlfilenamewithpath,materialname,double(sk{s}.colors)/255.0,1.0,1);
                isfirst=0;
                vidofs=vidofs+size(vcoordsmu,1);
              case 3 %PLY single files
                filename=[param.targetfolder param.targetfileprefix sprintf('_%04d_%s.ply',sid,on)];
                objectname=[param.targetfileprefix sprintf('_%04d_%s',sid,sk{s}.name)];
                set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...', ['Saving ' filename ' as PLY file.....']});
                pause(0.01);
                savelinestoply(vcoordsmu,ln,filename,sk{s}.colors(1),sk{s}.colors(2),sk{s}.colors(3));
              case 4 %.SWC (ASCII; Standardized neuron morphology format)
                filename=[param.targetfolder param.targetfileprefix sprintf('_%04d_%s.swc',sid,on)];
                set(vdata.ui.message,'String',{'Exporting Skeleton 3D Models ...', ['Saving ' filename ' as SWC file.....']});
                pause(0.01);
                savelinestoswc(vcoordsmu,ln2,diamu,filename);
            end;
...
function [] = callback_exportprojection(varargin)
  ...
  vinfo=vdata.vast.getinfo();
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No volume open in VAST.','VastTools projection image exporting'));
    return;
  end;
...
%  blockgui();
  ...
  if (vdata.state.lastcancel==0)
    ...
    if ((nrofsegments==0) && ((vdata.data.exportproj.imagesource==1)||(vdata.data.exportproj.imagesource==2)||(vdata.data.exportproj.opacitysource<5)))
      waitfor(warndlg('ERROR: No segmentation available in VAST. Cannot use segmentation during exporting!','VastTools projection image exporting'));
      releasegui();
      setcanceledmsg();
      return;
    end;
...
function [] = renderprojection()
    ...
    for slnn=1:length(seglayernrs);
      ...
      switch segpreprocess
        ...
        case 3  %Selected segment and children, uncollapsed
          ...
          if (min(size(selected))~=0)
            selected=[selected getchildtreeids_seg(ldata,selected)];
          end;
  ...
  switch vdata.data.exportproj.projaxis
    case 1 %'+X (lowest X in front)'
      ...
      projectback=[[0 0 mipfact xmin*mipfact]; [mipfact 0 0 ymin*mipfact]; [0 slicestep*zscale 0 zmin]];
    ...
    case 2 %'-X (highest X in front)'
      ...
      projectback=[[0 0 -mipfact xmax*mipfact]; [-mipfact 0 0 ymax*mipfact]; [0 slicestep*zscale 0 zmin]];
      if (slicestep==1) %load in slabs
        nrofslabs=ceil(tdepth/slabthickness);
        sslabbox=zeros(nrofslabs,6);
        slabstart=tdmin:slabthickness:tdmin+tdepth-1;
    ...
    case 3 %'+Y (lowest Y in front)'
      ...
      tdmin=ymin;
      tdir=1;
      projectback=[[-mipfact 0 0 xmax*mipfact]; [0 0 mipfact ymin*mipfact]; [0 slicestep*zscale 0 zmin]];
      if (slicestep==1) %load in slabs
        nrofslabs=ceil(tdepth/slabthickness);
        sslabbox=zeros(nrofslabs,6);
        slabstart=tdmin:slabthickness:tdmin+tdepth-1;
    ...
    case 4 %'-Y (highest Y in front)'
      ...
      tdmin=ymin;
      tdir=-1;
      projectback=[[mipfact 0 0 xmin*mipfact]; [0 0 -mipfact ymax*mipfact]; [0 slicestep*zscale 0 zmin]];
      if (slicestep==1) %load in slabs
        nrofslabs=ceil(tdepth/slabthickness);
        sslabbox=zeros(nrofslabs,6);
        slabstart=tdmin:slabthickness:tdmin+tdepth-1;
    ...
    case 5 %'+Z (lowest Z in front)'
      ...
      tdmin=zmin;
      tdir=1;
      projectback=[[mipfact 0 0 xmin*mipfact]; [0 mipfact 0 ymin*mipfact]; [0 0 slicestep*zscale zmin]];
      if (slicestep==1) %load in slabs
        nrofslabs=ceil(tdepth/slabthickness);
        sslabbox=zeros(nrofslabs,6);
        slabstart=tdmin:slabthickness:tdmin+tdepth-1;
    ...
    case 6 %'-Z (highest Z in front)'
      ...
      tdmin=zmin;
      tdir=-1;
      projectback=[[-mipfact 0 0 xmax*mipfact]; [0 mipfact 0 ymin*mipfact]; [0 0 -slicestep*zscale zmax]];
      if (slicestep==1) %load in slabs
        nrofslabs=ceil(tdepth/slabthickness);
        sslabbox=zeros(nrofslabs,6);
        slabstart=tdmin:slabthickness:tdmin+tdepth-1;
  ...
  while ((d<=nrofslabs)&&(vdata.state.lastcancel==0))
    if ((imagesource==3)||(opacitysource==5)||(opacitysource==6))
      ...
      v=sslabbbox(d,:);
      slabsize=(v(2)-v(1)+1)*(v(4)-v(3)+1)*(v(6)-v(5)+1)*3;
      maxslabsize=4096*4096*16;
      if (slabsize<4096*4096*16)
        ...
        if (sslabbbox(d,5)==sslabbbox(d,6))
          %in this case a 3d array will be returned, but a 4d array with singular dimension 3 is expected below.
          scsimage=reshape(scsimage,size(scsimage,1),size(scsimage,2),1,size(scsimage,3));
        end;
      else
        scsimage=zeros(v(4)+1,v(2)+1,v(6)+1,3,'uint8'); %initialize as YXZC
        patchstartx=v(1):4096:v(2)-1;
        patchstarty=v(3):4096:v(4)-1;
        patchstartz=v(5):4096:v(6)-1;
        for z=1:length(patchstartz)
          pz1=patchstartz(z);
          pz2=min([pz1+4095, v(6)]);
          for y=1:length(patchstarty)
            py1=patchstarty(y);
            py2=min([py1+4095, v(4)]);
            for x=1:length(patchstartx)
              px1=patchstartx(x);
              px2=min([px1+4095, v(2)]);
              [scsimagepatch,res] = vdata.vast.getscreenshotimage(param.miplevel,px1,px2,py1,py2,pz1,pz2,collapsesegments);
              if (sslabbbox(d,5)==sslabbbox(d,6))
                scsimagepatch=reshape(scsimage,size(scsimagepatch,1),size(scsimagepatch,2),1,size(scsimagepatch,3));
              end;
              %scsimagepatch comes in as YXZC
              scsimage(py1+1:py2+1,px1+1:px2+1,pz1+1:pz2+1,:)=scsimagepatch;
            end;
          end;
        end;
      end;
    ...
    if ((imagesource==1)||(opacitysource==1)||(opacitysource==3))
      if ((sslabbbox(d,2)-sslabbbox(d,1)<4096)&&(sslabbbox(d,4)-sslabbbox(d,3)<4096))
        for slnn=1:length(seglayernrs);
          sln=seglayernrs(slnn);
          %set segmentation layer
          res=vdata.vast.setselectedlayernr(sln);
        ...
        %Get data in tiles to reduce memory footprint
        sir=zeros(sslabbbox(d,2)-sslabbbox(d,1)+1,sslabbbox(d,4)-sslabbbox(d,3)+1,sslabbbox(d,6)-sslabbbox(d,5)+1,'uint8');
        sig=zeros(sslabbbox(d,2)-sslabbbox(d,1)+1,sslabbbox(d,4)-sslabbbox(d,3)+1,sslabbbox(d,6)-sslabbbox(d,5)+1,'uint8');
        sib=zeros(sslabbbox(d,2)-sslabbbox(d,1)+1,sslabbbox(d,4)-sslabbbox(d,3)+1,sslabbbox(d,6)-sslabbbox(d,5)+1,'uint8');
        opimage=zeros(sslabbbox(d,2)-sslabbbox(d,1)+1,sslabbbox(d,4)-sslabbbox(d,3)+1,sslabbbox(d,6)-sslabbbox(d,5)+1,'uint8');
        
        for tilex=sslabbbox(d,1):4096:sslabbbox(d,2)
          tx1=tilex; tx2=min([tilex+4095 sslabbbox(d,2)]);
          for tiley=sslabbbox(d,3):4096:sslabbbox(d,4)
            ty1=tiley; ty2=min([tiley+4095 sslabbbox(d,4)]);
            ...
            for slnn=1:length(seglayernrs)
              ...
              res=vdata.vast.setsegtranslation(sltranspre{sln},sltranspost{sln});
              %get data
              [si,res] = vdata.vast.getsegimageRLEdecoded(param.miplevel,tx1,tx2,ty1,ty2,sslabbbox(d,5),sslabbbox(d,6),0);
              %combine data
              %         if (min(size(opimage))==0)
              %           opimage=si;
              %           %translate opimage to rgb
              ...
              ti=opimage(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:);
              ti(si>0)=si(si>0); %opimage(si>0)=si(si>0); %overwrite voxels in earlier layers
              opimage(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:)=ti;
              
              ti=sir(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:);
              ti(si>0)=slcols{sln}(si(si>0),1); %sir(si>0)=slcols{sln}(si(si>0),1);
              sir(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:)=ti;
              
              ti=sig(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:);
              ti(si>0)=slcols{sln}(si(si>0),2); %sig(si>0)=slcols{sln}(si(si>0),2);
              sig(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:)=ti;
              
              ti=sib(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:);
              ti(si>0)=slcols{sln}(si(si>0),3); %sib(si>0)=slcols{sln}(si(si>0),3);
              sib(tx1-sslabbbox(d,1)+1:tx2-sslabbbox(d,1)+1,ty1-sslabbbox(d,3)+1:ty2-sslabbbox(d,3)+1,:)=ti;
            end;
          end;
        end;
      end;
    ...
    for i=zlist
      ...
      if (depthattenuate~=1)
        rsim=uint8(double(rsim)*depthattenuate);
        gsim=uint8(double(gsim)*depthattenuate);
        bsim=uint8(double(bsim)*depthattenuate);
      end;
      
      %APPLY SHADOW MAP
      if (vdata.data.exportproj.useshadows)
        rsim=uint8(double(rsim).*shadow);
        gsim=uint8(double(gsim).*shadow);
        bsim=uint8(double(bsim).*shadow);
      end;
...
%           ttranspmap=ttranspmap.*stranspmap;
          rtim=rtim + (double(rsim).*(1-stranspmap)).*(ttranspmap);  %transpmap is 1 for transparent and 0 for opaque
          gtim=gtim + (double(gsim).*(1-stranspmap)).*(ttranspmap);
          btim=btim + (double(bsim).*(1-stranspmap)).*(ttranspmap);
          ttranspmap=ttranspmap.*stranspmap;
        case 2 %2: Additive
          rtim=rtim + double(rsim).*(1-stranspmap);
          gtim=gtim + double(gsim).*(1-stranspmap);
          btim=btim + double(bsim).*(1-stranspmap);
          ttranspmap=ttranspmap.*stranspmap;
        case 3 %3: Maximum projection
          rim=double(rsim).*(1-stranspmap); rtim(rim>rtim)=rim(rim>rtim); clear rim;
          gim=double(gsim).*(1-stranspmap); gtim(gim>gtim)=gim(gim>gtim); clear gim;
          bim=double(bsim).*(1-stranspmap); btim(bim>btim)=bim(bim>btim); clear bim;
          ttranspmap=ttranspmap.*stranspmap;
    ...
    if (vdata.data.exportproj.showinwindow==1)
      targetimage=zeros(theight,twidth,3,'uint8');
      targetimage(:,:,1)=rtim;
      targetimage(:,:,2)=gtim;
      targetimage(:,:,3)=btim;
      imshow(targetimage,'parent',lax);
...
function [] = callback_measurevol(varargin)
  ...
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No image volume open in VAST.','VastTools volume measurement'));
    return;
  end;
  
  nrofsegments=vdata.vast.getnumberofsegments();
  if (nrofsegments==0)
    waitfor(warndlg('ERROR: No segmentation available in VAST.','VastTools volume measurement'));
    return;
  end;
  ...
  str=cell(vinfo.nrofmiplevels,1);
  [selectedlayernr,selectedemlayernr,selectedsegmentlayernr,res]=vdata.vast.getselectedlayernr();
  mipfactmatrix=vdata.vast.getmipmapscalefactors(selectedsegmentlayernr);
  vx=vinfo.voxelsizex;
  vy=vinfo.voxelsizey;
  vz=vinfo.voxelsizez;
  for i=1:1:vinfo.nrofmiplevels
    if ((i-1)>0)
     vx=vinfo.voxelsizex*mipfactmatrix(i-1,1);
     vy=vinfo.voxelsizey*mipfactmatrix(i-1,2);
     vz=vinfo.voxelsizez*mipfactmatrix(i-01,3);
    end;
    str{i}=sprintf('Mip %d - (%.2f nm, %.2f nm, %.2f nm) voxels',i-1,vx,vy,vz);
    %vx=vx*2; vy=vy*2;
  end;
...
%   areazmax=vdata.data.region.zmax;
  ...
  if (miplevel==0)
    areaxmin=vdata.data.region.xmin;
    areaxmax=vdata.data.region.xmax;
    areaymin=vdata.data.region.ymin;
    areaymax=vdata.data.region.ymax;
    areazmin=vdata.data.region.zmin;
    areazmax=vdata.data.region.zmax;
  else
    areaxmin=floor(vdata.data.region.xmin/mipfactmatrix(miplevel,1));
    areaxmax=floor(vdata.data.region.xmax/mipfactmatrix(miplevel,1));
    areaymin=floor(vdata.data.region.ymin/mipfactmatrix(miplevel,2));
    areaymax=floor(vdata.data.region.ymax/mipfactmatrix(miplevel,2));
    areazmin=floor(vdata.data.region.zmin/mipfactmatrix(miplevel,3));
    areazmax=floor(vdata.data.region.zmax/mipfactmatrix(miplevel,3));
  end;
  ...
  while ((z<=size(tilestartz,2))&&(vdata.state.lastcancel==0))
    ...
    while ((y<=size(tilestarty,2))&&(vdata.state.lastcancel==0))
      ...
      while ((x<=size(tilestartx,2))&&(vdata.state.lastcancel==0))
        ...
        %Volumes
        %[segimage, res]=vdata.vast.getsegimageRLEdecoded(miplevel,minx,maxx,miny,maxy,minz,maxz,0);
        %imwrite(segimage,[vdata.data.measurevol.targetfile '_image.png']);
  ...
  if (vdata.state.lastcancel==0)
    ...
    voxsizex=vdata.data.measurevol.xunit;
    voxsizey=vdata.data.measurevol.yunit;
    voxsizez=vdata.data.measurevol.zunit;
    if (vdata.data.measurevol.miplevel>0)
      voxsizex=voxsizex*mipfactmatrix(vdata.data.measurevol.miplevel,1);
      voxsizey=voxsizey*mipfactmatrix(vdata.data.measurevol.miplevel,2);
      voxsizez=voxsizez*mipfactmatrix(vdata.data.measurevol.miplevel,3);
    end;
    voxelvol=voxsizex*voxsizey*voxsizez;
    ...
    if (fid>0)
      ...
      fprintf(fid,'%% Area: full-res: (%d-%d, %d-%d, %d-%d); at mip %d: (%d-%d, %d-%d, %d-%d)\n',vdata.data.region.xmin,vdata.data.region.xmax,vdata.data.region.ymin,vdata.data.region.ymax,vdata.data.region.zmin,vdata.data.region.zmax,vdata.data.measurevol.miplevel,areaxmin,areaxmax,areaymin,areaymax,areazmin,areazmax);
      fprintf(fid,'%% Computed at voxel size: (%f,%f,%f)\n',voxsizex,voxsizey,voxsizez);
      ...
      waitfor(warndlg(['WARNING: Could not open "' vdata.data.measurevol.targetfile '" for writing.'],'Saving volumes failed!'));
    end;
  end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Image Brightness Measurement Functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  

function [] = callback_measurebrightness(varargin)
  global vdata;
  ...
  if (min([vinfo.datasizex vinfo.datasizey vinfo.datasizez])==0)
    waitfor(warndlg('ERROR: No image volume open in VAST.','VastTools masked image brightness measurement'));
    return;
  end;
  
  nrofsegments=vdata.vast.getnumberofsegments();
  if (nrofsegments==0)
    waitfor(warndlg('ERROR: No segmentation available in VAST.','VastTools masked image brightness measurement'));
    return;
  end;
  ...
  if (~isfield(vdata.data,'measurebrightness'))
    vdata.data.measurebrightness.miplevel=0;
    %vdata.data.measurebrightness.xunit=vinfo.voxelsizex;
    %vdata.data.measurebrightness.yunit=vinfo.voxelsizey;
    %vdata.data.measurebrightness.zunit=vinfo.voxelsizez;
    vdata.data.measurebrightness.maskby=1;
    vdata.data.measurebrightness.measurewhat=1;
    vdata.data.measurebrightness.targetfoldername=[pwd '\'];
    vdata.data.measurebrightness.targetfilename='maskedbrightnessstats.txt';
  end;
  
  [selectedlayernr, selectedemlayernr, selectedsegmentlayernr, res] = vdata.vast.getselectedlayernr();
  [linfo_em, res] = vdata.vast.getlayerinfo(selectedemlayernr);
  [linfo_seg, res] = vdata.vast.getlayerinfo(selectedsegmentlayernr);
  
  vdata.data.measurebrightness.emlayernr=selectedemlayernr;
  vdata.data.measurebrightness.emlayerbpp=linfo_em.bytesperpixel;
  vdata.data.measurebrightness.seglayernr=selectedsegmentlayernr;
  
  if ((linfo_em.bytesperpixel~=1)&&(linfo_em.bytesperpixel~=3))
    waitfor(warndlg('ERROR: Measuring brightness is currently only supported for 8-bit and 24-bit image layers.','VastTools masked image brightness measurement'));
    releasegui();
    return;
  end;
  ...
  scrsz = get(0,'ScreenSize');
  figheight=380;
  f = figure('units','pixels','position',[50 scrsz(4)-100-figheight 500 figheight],'menubar','none','numbertitle','off','name','VastTools - Measure Masked Image Brightness','resize','off');

  vpos=figheight-30;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 400 15], 'String',['Source Image Layer:  "' linfo_em.name '"'],'backgroundcolor',get(f,'color'),'horizontalalignment','left');
  vpos=vpos-20;
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 400 15], 'String',['Masking Layer:  "' linfo_seg.name '"'],'backgroundcolor',get(f,'color'),'horizontalalignment','left');
  vpos=vpos-30;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 120 15], 'String','Analyze at resolution:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  str=cell(vinfo.nrofmiplevels,1);
  vx=vinfo.voxelsizex;
  vy=vinfo.voxelsizey;
  vz=vinfo.voxelsizez;
  ...
  pmh = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.measurebrightness.miplevel+1,'Position',[170 vpos 310 20]);
  vpos=vpos-50;
...
%   vpos=vpos-30;
  ...
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','Mask by:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  str=cell(4,1);
  str{1}='All segments individually, uncollapsed';
  str{2}='All segments, collapsed as in VAST';
  str{3}='Selected segment and children, uncollapsed';
  str{4}='Selected segment and children, collapsed as in VAST';
  pmh2 = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.measurebrightness.maskby,'Position',[120 vpos 360 20]);
  vpos=vpos-30;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 100 15],'String','Measure what:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  if (linfo_em.bytesperpixel==1)
    str=cell(2,1);
    str{1}='Mean pixel brightness values';
    str{2}='Pixel brightness histograms';
  else
    str=cell(4,1);
    str{1}='Mean pixel brightness values (R, G, B source channels)';
    str{2}='Pixel brightness histograms (Red source channel)';
    str{3}='Pixel brightness histograms (Green source channel)';
    str{4}='Pixel brightness histograms (Blue source channel)';
  end;
  pmh3 = uicontrol('Style','popupmenu','String',str,'Value',vdata.data.measurebrightness.measurewhat,'Position',[120 vpos 360 20]);
  vpos=vpos-30;
  
  t = uicontrol('Style','text', 'Units','Pixels', 'Position',[30 vpos 280 15],'String','Target text file for masked image brightness results:','backgroundcolor',get(f,'color'),'horizontalalignment','left');
  vpos=vpos-25;
  vdata.temp.e10 = uicontrol('Style','Edit', 'Units','Pixels', 'Position',[30 vpos 380 20],'String',[vdata.data.measurebrightness.targetfoldername vdata.data.measurebrightness.targetfilename],'horizontalalignment','left');
  p1 = uicontrol('Style','PushButton', 'Units','Pixels', 'Position',[420 vpos 60 20], 'String','Browse ...','CallBack',{@callback_measurebrightness_browse});
  vpos=vpos-30;
  ...
  if (vdata.state.lastcancel==0)
    vdata.data.measurebrightness.miplevel=get(pmh,'value')-1;
    vdata.data.region.xmin = str2num(get(vdata.temp.e_xmin,'String'));
    vdata.data.region.xmax = str2num(get(vdata.temp.e_xmax,'String'));
    vdata.data.region.ymin = str2num(get(vdata.temp.e_ymin,'String'));
    vdata.data.region.ymax = str2num(get(vdata.temp.e_ymax,'String'));
    ...
    %vdata.data.measurebrightness.xunit = str2num(get(vdata.temp.e7,'String'));
    %vdata.data.measurebrightness.yunit = str2num(get(vdata.temp.e8,'String'));
    %vdata.data.measurebrightness.zunit = str2num(get(vdata.temp.e9,'String'));
    vdata.data.measurebrightness.maskby = get(pmh2,'value');
    vdata.data.measurebrightness.measurewhat = get(pmh3,'value');
    vdata.data.measurebrightness.exportmodestring = get(pmh2,'string');
    vdata.data.measurebrightness.exportmodestring = vdata.data.measurebrightness.exportmodestring{vdata.data.measurebrightness.maskby};
    vdata.data.measurebrightness.targetfile = get(vdata.temp.e10,'String');
    [vdata.data.measurebrightness.targetfoldername,vdata.data.measurebrightness.targetfilename]=splitfilename(vdata.data.measurebrightness.targetfile);
  end;
  ...
  if (vdata.state.lastcancel==0)
    measuremaskedbrightness();
  end;
  releasegui();
  
  
function [] = callback_measurebrightness_browse(varargin)
  global vdata;
  defaultname=[vdata.data.measurebrightness.targetfoldername vdata.data.measurebrightness.targetfilename];
  [targetfilename,targetfoldername,filterindex] = uiputfile({'*.txt','Text Files (*.txt)'; '*.*', 'All Files (*.*)'},'Select Target Text File',defaultname);
  if ((~isequal(targetfilename,0)) && (~isequal(targetfoldername,0)))
    set(vdata.temp.e10,'String',[targetfoldername targetfilename]);
    vdata.data.measurebrightness.targetfilename=targetfilename;
    vdata.data.measurebrightness.targetfoldername=targetfoldername;
  end;
  
  
function res=measuremaskedbrightness()
  global vdata;
  ...
  miplevel=vdata.data.measurebrightness.miplevel;
  areaxmin=bitshift(vdata.data.region.xmin,-miplevel);
  areaxmax=bitshift(vdata.data.region.xmax+1,-miplevel)-1;
  areaymin=bitshift(vdata.data.region.ymin,-miplevel);
  areaymax=bitshift(vdata.data.region.ymax+1,-miplevel)-1;
  ...
  analyzewhich=vdata.data.measurebrightness.maskby;
  
  [apiversion, res] = vdata.vast.getapiversion();
  if (apiversion>=4)
    res = vdata.vast.setapilayersenabled(1);
    res = vdata.vast.setselectedapilayernr(vdata.data.measurebrightness.emlayernr);
    res = vdata.vast.setselectedapilayernr(vdata.data.measurebrightness.seglayernr);
  end;
  ...
  measurewhat=vdata.data.measurebrightness.measurewhat;
  bpp=vdata.data.measurebrightness.emlayerbpp;
  if (bpp==1)
    if (measurewhat==1)
      nrvox=zeros(max(objects(:,1)),1);
      voxdata=zeros(max(objects(:,1)),1);
    else
      nrvox=zeros(max(objects(:,1)),1);
      voxdata=zeros(max(objects(:,1)),256);
    end;
  end;
  if (bpp==3)
    if (measurewhat==1) 
      %'Mean pixel brightness values (R, G, B source channels)';
      nrvox=zeros(max(objects(:,1)),1);
      voxdata=zeros(max(objects(:,1)),3);
    else
      %'Pixel brightness histograms (Red source channel)'
      %'Pixel brightness histograms (Green source channel)'
      %'Pixel brightness histograms (Blue source channel)'
      nrvox=zeros(max(objects(:,1)),1);
      voxdata=zeros(max(objects(:,1)),256);
    end;
  end;
  ...
  while ((z<=size(tilestartz,2))&&(vdata.state.lastcancel==0))
    ...
    while ((y<=size(tilestarty,2))&&(vdata.state.lastcancel==0))
      ...
      while ((x<=size(tilestartx,2))&&(vdata.state.lastcancel==0))
        ...
        message={'Measuring Masked Pixel Brightness ...',sprintf('Loading Segmentation Cube (%d,%d,%d) of (%d,%d,%d)...',x,y,z,nrxtiles,nrytiles,nrztiles)};
        set(vdata.ui.message,'String',message);
        pause(0.01);
        
        %Load segmentation and image layer data
        %[v,n,res] = vdata.vast.getRLEcountunique(miplevel,minx,maxx,miny,maxy,minz,maxz,0);
        [segimage,v,n,res] = vdata.vast.getsegimageRLEdecodedcountunique(miplevel,minx,maxx,miny,maxy,minz,maxz,0,1);
        if (res==1)
          n(v==0)=[];
          v(v==0)=[];
          if ((min(size(v))>0)&&(vdata.state.lastcancel==0))
            message={'Measuring Masked Pixel Brightness ...',sprintf('Loading Image Cube (%d,%d,%d) of (%d,%d,%d)...',x,y,z,nrxtiles,nrytiles,nrztiles)};
            set(vdata.ui.message,'String',message);
            pause(0.001);
            [emimage,res]= vdata.vast.getemimage(vdata.data.measurebrightness.emlayernr, miplevel,minx,maxx,miny,maxy,minz,maxz);
            if (bpp==3)
              if (measurewhat==2)
                emimage=squeeze(emimage(:,:,:,1)); %'Pixel brightness histograms (Red source channel)'
              end;
              if (measurewhat==3)
                emimage=squeeze(emimage(:,:,:,2)); %'Pixel brightness histograms (Green source channel)'
              end;
              if (measurewhat==4)
                emimage=squeeze(emimage(:,:,:,3)); %'Pixel brightness histograms (Blue source channel)'
              end;
            end;
            ...
            if (measurewhat==1)
              if (bpp==1)
                i=1;
                while ((i<=length(v))&&(vdata.state.lastcancel==0))
                  message={'Measuring Mean Masked Pixel Brightness ...',sprintf('Processing Cube (%d,%d,%d) of (%d,%d,%d), Segment %d of %d...',x,y,z,nrxtiles,nrytiles,nrztiles,i,length(v))};
                  set(vdata.ui.message,'String',message);
                  pause(0.001);
                  voxdata(v(i))=voxdata(v(i)) + sum(emimage(segimage==v(i)));
                  i=i+1;
                end;
              end;
              if (bpp==3)
                ...
                eir=squeeze(emimage(:,:,:,1));
                eig=squeeze(emimage(:,:,:,2));
                eib=squeeze(emimage(:,:,:,3));
                while ((i<=length(v))&&(vdata.state.lastcancel==0))
                  message={'Measuring Mean Masked Pixel Brightness RGB ...',sprintf('Processing Cube (%d,%d,%d) of (%d,%d,%d), Segment %d of %d...',x,y,z,nrxtiles,nrytiles,nrztiles,i,length(v))};
                  set(vdata.ui.message,'String',message);
                  pause(0.001);
                  voxdata(v(i),1)=voxdata(v(i),1) + sum(eir(segimage==v(i)));
                  voxdata(v(i),2)=voxdata(v(i),2) + sum(eig(segimage==v(i)));
                  voxdata(v(i),3)=voxdata(v(i),3) + sum(eib(segimage==v(i)));
                  i=i+1;
                end;
              end;
            else
              i=1;
              while ((i<=length(v))&&(vdata.state.lastcancel==0))
                message={'Measuring Masked Pixel Brightness Histograms ...',sprintf('Processing Cube (%d,%d,%d) of (%d,%d,%d), Segment %d of %d...',x,y,z,nrxtiles,nrytiles,nrztiles,i,length(v))};
                set(vdata.ui.message,'String',message);
                pause(0.001);
                h=histc(emimage(segimage==v(i)),0:255);
                if (size(h,1)>size(h,2)) h=h'; end; %wtf, histc sometimes returns rows and sometimes columns!?
                voxdata(v(i),:)=voxdata(v(i),:) + h;
                i=i+1;
              end;
            end;
          end;
  ...
  %vdata.data.measurebrightness.lastobjects=objects;
  %vdata.data.measurebrightness.lastvolume=nrvox;
  ...
  [linfo_em, res] = vdata.vast.getlayerinfo(vdata.data.measurebrightness.emlayernr);
  [linfo_seg, res] = vdata.vast.getlayerinfo(vdata.data.measurebrightness.seglayernr);
  
  if (vdata.state.lastcancel==0)
    %write surface area values to text file
    %mipfact=bitshift(1,vdata.data.measurebrightness.miplevel);
    %voxsizex=vdata.data.measurebrightness.xunit*mipfact;
    %voxsizey=vdata.data.measurebrightness.yunit*mipfact;
    %voxsizez=vdata.datameasurebrightness.zunit;
    %voxelvol=voxsizex*voxsizey*voxsizez;
    set(vdata.ui.message,'String',['Saving ' vdata.data.measurebrightness.targetfile '...']);
    
    fid = fopen(vdata.data.measurebrightness.targetfile, 'wt');
    if (fid>0)
      fprintf(fid,'%% VastTools Masked Pixel Brightness Export\n');
      fprintf(fid,'%% Provided as-is, no guarantee for correctness!\n');
      fprintf(fid,'%% %s\n\n',get(vdata.fh,'name'));
      
      fprintf(fid,'%% Source Image File: %s\n',linfo_em.name);
      fprintf(fid,'%% Source Mask File: %s\n',linfo_seg.name);
      fprintf(fid,'%% Analyzed at Mip Level %d\n',miplevel);
      
      if (measurewhat==1)
        if (bpp==1)
          fprintf(fid,'%% Mode: %s  (mean values)\n', vdata.data.measurebrightness.exportmodestring);
          fprintf(fid,'%% Columns are: Mask Segment Name,  Mask Segment ID,  Number Voxels Used,  Average Brightness [0..255]\n');
          for segnr=1:1:size(objects,1)
            seg=objects(segnr,1);
            if (nrvox(segnr)==0)
              fprintf(fid,'"%s"  %d  %d  %f\n',name{seg},seg,nrvox(segnr),0);
            else
              fprintf(fid,'"%s"  %d  %d  %f\n',name{seg},seg,nrvox(segnr),voxdata(segnr)/nrvox(segnr));
            end;
          end;
        end;
        if (bpp==3)
          fprintf(fid,'%% Mode: %s  (mean values RGB)\n', vdata.data.measurebrightness.exportmodestring);
          fprintf(fid,'%% Columns are: Mask Segment Name,  Mask Segment ID,  Number Voxels Used,  Average Brightness Red [0..255],  Average Brightness Green [0..255],  Average Brightness Blue [0..255]\n');
          for segnr=1:1:size(objects,1)
            seg=objects(segnr,1);
            if (nrvox(segnr)==0)
              fprintf(fid,'"%s"  %d  %d  %f  %f  %f\n',name{seg},seg,nrvox(segnr),0.0,0.0,0.0);
            else
              fprintf(fid,'"%s"  %d  %d  %f  %f  %f\n',name{seg},seg,nrvox(segnr),voxdata(segnr,1)/nrvox(segnr),voxdata(segnr,2)/nrvox(segnr),voxdata(segnr,3)/nrvox(segnr));
            end;
          end;
        end;
      else
        if (bpp==1)
          fprintf(fid,'%% Mode: %s  (histograms)\n', vdata.data.measurebrightness.exportmodestring);
        end;
        if (bpp==3)
          if (measurewhat==2)
            fprintf(fid,'%% Mode: %s  (histograms, red source channel)\n', vdata.data.measurebrightness.exportmodestring);
          end;
          if (measurewhat==3)
            fprintf(fid,'%% Mode: %s  (histograms, green source channel)\n', vdata.data.measurebrightness.exportmodestring);
          end;
          if (measurewhat==4)
            fprintf(fid,'%% Mode: %s  (histograms, blue source channel)\n', vdata.data.measurebrightness.exportmodestring);
          end;
        end;
        fprintf(fid,'%% Columns are: Mask Segment Name,  Mask Segment ID,  256x Voxel Count (Nr. Voxels With Brightness 0 .. 255)\n');

        for segnr=1:1:size(objects,1)
          seg=objects(segnr,1);
          fprintf(fid,'"%s"  %d',name{seg},seg);
          for i=0:31
            fprintf(fid,'  %d  %d  %d  %d  %d  %d  %d  %d',voxdata(segnr,i*8+1),voxdata(segnr,i*8+2),voxdata(segnr,i*8+3),voxdata(segnr,i*8+4),voxdata(segnr,i*8+5),voxdata(segnr,i*8+6),voxdata(segnr,i*8+7),voxdata(segnr,i*8+8));
          end;
          fprintf(fid,'\n');
        end;
      ...
      waitfor(warndlg(['WARNING: Could not open "' vdata.data.measurebrightness.targetfile '" for writing.'],'Saving volumes failed!'));
    end;
  end;
...
function [] = targetlistwindow(targetlistname, instance, inputfilename)
  ...
  if (min(size(inputfilename>0)))
    ...
    if (~isfield(targetlist,'targetlist'))
      waitfor(warndlg('This is not a valid target list file. A .MAT file with struct "targetlist" is expected.',['Error loading ' inputfilename]));
      return;
    end;
    targetlist=targetlist.targetlist;
    
    if (~isfield(targetlist,'coords'))
      waitfor(warndlg('"targetlist.coords" missing in target list file.',['Error loading ' inputfilename]));
      callback_tlquit();
      return;
    end;
...
function [] = callback_tlinsertcoords(varargin)
  ...
  try
    ...
    waitfor(warndlg(['Variable "' varname '" not found.'],'Error Importing Coordinate Matrix'));
    return;
  end;
  
  if ((size(size(mtx),2)~=2)||((size(mtx,2)~=3)&&(size(mtx,2)~=4)))
    waitfor(warndlg(['Variable "' varname '" is not a 2D matrix with three or four columns.'],'Error Importing Coordinate Matrix'));
    return;
  end;
...
function [] = callback_newsimplenavigator(varargin)
  ...
  if (~isfield(vdata.data,'exportproj'))
    waitfor(warndlg('Please render a projection image first, using Export / Export Projection Image.','Error generating Simple Navigator Window'));
    return;
  end;
...
function [] = simplenavigatorwindow(instance, inputfilename)
  ...
  if (min(size(inputfilename))>0)
    ...
    if (ismember('sndata', {vars.name})==0)
      waitfor(warndlg(['ERROR: The file "' inputfilename '" is not a VastTools Simple Navigator Image file.'],'Error loading Simple Navigator Image'));
      return;
    end;
  end;
...
function ret=getchildtreeids_seg(data,parentlist)
  %Uses the 24-column-data matrix for segmentation layers
  ...
  for p=1:1:size(pal,1)
    ...
    if (data(index,15)>0)
      i=data(index,15);
      ret=[ret i getchildtreeids_seg(data,i)]; %Add size of child tree
      while (data(i,17)>0) %add sizes of all nexts
        i=data(i,17);
        ret=[ret i getchildtreeids_seg(data,i)];
      end;
    end;
  end;
  
function ret=getchildtreeids_anno(data,parentlist)
  ...
  for p=1:1:size(pal,1)
    ...
    if (data(index,16)>0)
      i=data(index,16);
      ret=[ret i getchildtreeids_anno(data,i)]; %Add size of child tree
      while (data(i,18)>0) %add sizes of all nexts
        i=data(i,18);
        ret=[ret i getchildtreeids_anno(data,i)];
      end;
    end;
  end;
...
function ret=checkconnection()
  ...
  try
    ...
    if (findstr(lasterr,'socket write error'))
      waitfor(warndlg('ERROR: Connection to VAST lost. Please reconnect.','Error with remote connection to VAST'));
      return;
    else
      waitfor(warndlg(lasterr,'Error with remote connection to VAST'));
      return;
    end;
  end;
  if (min(size(vinfo)==0))
    waitfor(warndlg('ERROR: Requesting data from VAST failed.','Error with remote connection to VAST'));
    return;
  end;
  ret=1;