...
function[] = drawObj(dsObj,obI,obIds,v)
...
%% Draw cells
for i = 1:length(obIds)
    sub = dsObj(obIds(i)).subs;
    if ~isempty(sub)
    ...
    glob.g(v).patch(i) = renderFVnav(fv,glob.g(v).col(i,:),...
        glob.g(v).alph,glob.g(v).cid{i});
