...
subplot(3,1,1)
scatPol = scatter(SOI.offBias(useROI),roiPol,'o','filled');
...
scatPol4.CData = expIDs(useROI) * ceil(100/length(uExp));


xlim([-1 1])
ylim([-1 1])

if 0 % show point information, run, click, press enter
...
clf
subplot(3,1,1)
scatZ0 = scatter(SOI.pos(useROI,3),roiPol,'o','filled','r');
scatZ0.CData = expIDs(useROI) * ceil(100/length(uExp));
subplot(3,1,2)
scatZ = scatter(SOI.pos(useROI,3),roiPolH,'o','filled','r');
...
clf
scatPol2 = scatter(SOI.offBias(useROI),roiPol,10,'o','filled');
