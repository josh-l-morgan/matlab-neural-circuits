...
while 0
    ...
    if loadAll
        vastSubsPath='Y:\karlsRetina\CellNavLibrary_IxQ\Volumes\Final\Merge\vastSubs.mat';
    ...
    bpcSubtypes=bpcTypeDat{3};%(find(allTypes{1}==7));
    %bpcSubtypes(2)=[];
    bpcSubtypes(bpcSubtypes==0)=24;
    ...
    %4 5 8 6 are pinkish
    %3 2 7 9 10 are g->b
    %colmapOrd=[4,5,4,4,8,7,7,7,9,2,15,5,8,7,9,9,14,1,7];
    %colmapOrd=[1:15 15];
    colmapOrd=[20,40,60,70,80,170,180,190,200,195,90,230,240,125,185,185];
    %  1  2 3a 3b 4 5i 5o 5t  6  7  off xbc on  no rbc bc5
    %colmapOrd=1:15;
    %BCcolmap=HCcolmap(colmapOrd,:);
    BCcolmap=jetCmap(colmapOrd,:);
    figure();
    scatter(1:length(colmapOrd)',repmat([1],[length(colmapOrd),1]),100,BCcolmap,'filled');
    xticks([1:length(colmapOrd)])
    xticklabels(subTypeNameList);
    xtickangle(90);
    if testFigs
        f=0;
        midList=[];
        ...
        for bpcIt=1:length(bpcVxls)
            ...
            if size(curVxls,1)>bpcSizeCutoff
                ...
                curCol=repmat(BCcolmap(bpcColID(bpcIt),:),[size(curVxls,1),1]);
                %h=scatter3(curVxls(:,1),curVxls(:,2),curVxls(:,3),3,curCol,'.');
                %alpha = 0.25;
    ...
    if spinnit
        fileDir='D:\work\gif5\';
        stepAng=3;
        fileName=[fileDir 'test3'];
        %view(stepAng-1*stepAng,0);
        view(-180,0);
        drawnow
        set(gca,'visible','off')
    ...
    if voxTest
        ...
        for i=1:49 %length(bpcVxls)
            nexttile
            curVxls=bpcVxls{i};
            curProj=unique(curVxls(:,1:2),'rows');
            curProj=double(curProj);
            ...
            scatter(curProj(:,1),curProj(:,2),1,'m.');
    ...
    %% test the terries
    % make sure they ain't gettin all froggy
    testList=[1006 2033 2008 1033 3637 1084 1065 1074 1033 1208 1179];
    %drax them sklounst
    bpcItList=find(ismember(bpcCids,testList));
    bpcVoxCutoff=10000;
