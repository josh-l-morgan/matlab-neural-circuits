...
for i=1:size(typeResults,1)
    ...
    curY=find(unique(typeResults(:,2))==typeResults(i,2));
    alphaMat(curX,curY)=sum(typeResults(i,3:4));
    if resultMat(curX,curY)<.8 &&  resultMat(curX,curY)>.2
        text(curY,curX,num2str(sum(typeResults(i,3:4))),'HorizontalAlignment','center','Color','black');
    else
        text(curY,curX,num2str(sum(typeResults(i,3:4))),'HorizontalAlignment','center','Color','white');
    end
end
yticks([2:length(unique(typeResults(:,1)))]);
xticks([2:length(unique(typeResults(:,2)))]);
rgcSubTypes=unique(typeResults(:,1));
bpcSubTypes=unique(typeResults(:,2));
rgcTypeNames=curTis.cells.type.subTypeNames{1}(rgcSubTypes(2:end));
bpcTypeNames=curTis.cells.type.subTypeNames{7}(bpcSubTypes(2:end));
yticklabels(rgcTypeNames);
xticklabels(bpcTypeNames);
