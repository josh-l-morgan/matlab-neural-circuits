...
while keepGoing
    ...
    easyList=horzcat(badSynSegName',badSynName');
    ...
    badObjs=zeros(size(badIndsCln));
        badObjs(:,1)=curTis.syn.edges(badIndsCln(:,1),3);
        badObjs(:,2)=curTis.syn.edges(badIndsCln(:,2),3);
        %[b,m1,n1] = unique(a,'first');
        %[c1,d1] =sort(m1);
        %b = b(d1);
        keepInds=find(badObjs(:,1)~=badObjs(:,2));
    
    badIndsCln=badIndsCln(keepInds,:);
        
    i=1;
    figure();
    currKey='space';
    while string(currKey)~="escape" %i<=size(badIndsCln,1)%
        curCompSynIDs=badIndsCln(i,:);
        badEdges=curTis.syn.edges(curCompSynIDs,:);
        %if badEdges(1,3)==badEdges(2,3) && badEdges(1,2)==badEdges(2,2)
        %    if string(currKey)=="rightarrow"
        %        i=i+1;
        %    end
        %    if string(currKey)=="leftarrow"
        %        i=i-1;
        %    end
        %else
            badEdgesAC=aliasChecker(badEdges(:,[1 2]),aliasList);
            badPos=curTis.syn.pos(curCompSynIDs,[2 1 3]);
            ...
            output2=[badEdgesAC(1,:) badEdges(1,3) badEdgesAC(2,:) badEdges(2,3) ;...
