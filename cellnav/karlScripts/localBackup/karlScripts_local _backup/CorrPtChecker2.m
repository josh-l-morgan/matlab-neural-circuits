%% setup
analDir='Y:\karlsRetina\CellNavLibrary_IxQ\Volumes\Final\';
...
newPts=pts;
%figA=figure();
plotPhys(figA,corrPts,regionNames,i,physImgs);
%figB=figure();
%figC=figure();
%figure(figName);
hold on
while true
    ...
    elseif x==0
        ...
        scatter(x,y,50,'r+');
        [vastXds,vastYds]=allTFs{i}.transformPointsForward(x/10-10,y/10-10);
        ...
        FP=plotPlane(figB,corrPts,i);
        FC=projectEMontoPhys(figC,corrPts,regionNames,i,physImgs,vgcMat);
    end
end
end

function outFig = projectEMontoPhys(figName,corrPts,regionNames,i,physImgs,vgcMat)
figure(figName);
outFig=plotPhys(figName,corrPts,regionNames,i,physImgs);
...
vgcSubMat=sum(vgcMat(:,:,zrange(1)-zbuf:zrange(2)+zbuf),3);
...
function outFig=plotPlane(figName,corrPts,i)
outFig=1;
...
function outFig=plotPhys(figName,corrPts,regionNames,i,physImgs)
outFig=1;
curPts=corrPts{i};
imgBord=100;
figure(figName);
