...
[a,b,d]=getIPLdepth(20.65,130.3,115.3,[],[])

exportgraphics(gca,'1026_side.png','Resolution',2400);

barA=plot3([36.8 36.8],[130 100],[70 120]);
barA=plot3([6.5 6.5],[130 100],[70 120]);
...
%% Chapter4 figures
xaCidsDend=curTis.syn.edges(find(ismember(curTis.syn.edges(:,1),xaCids)),1);
xaCidsDend=unique(xaCidsDend);
%xaCidsDend=1026;
xaCidsDend(9)=[];
...
axis equal
hold on
bar1=plot3([37 27],[105 105],[110 110]);
bar2=plot3([27 27],[110 100],[105 105]);
exportgraphics(gca,'1026_spines2_top.png','Resolution',2400);


%%
allv2x=getClassConn({'amc'},{'vgc'},{'amc'},{'xac'},curTis);
allx2v=getClassConn({'amc'},{'xac'},{'amc'},{'vgc'},curTis);
posv2x=curTis.syn.pos(allv2x,:);
posx2v=curTis.syn.pos(allx2v,:);
dists=pdist2(posv2x,posx2v);
distsCheck1=pdist2(posv2x,posv2x);
distsCheck2=pdist2(posx2v,posx2v);
figure(); histogram(dists);
hold on;
histogram(distsCheck1);histogram(distsCheck2);
fromXVtoVX=min(dists,[],1);
fromVXtoXV=min(dists,[],2);
figure(); histogram(fromXVtoVX,25); hold on;
histogram(fromVXtoXV,25);
mean(fromXVtoVX)
median(fromXVtoVX)
mean(fromVXtoXV)
median(fromVXtoXV)