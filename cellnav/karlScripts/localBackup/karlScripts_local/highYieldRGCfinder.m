...
vgcAllCids=type2cid({'amc'},{'vgc'},curTis);
vgcAllCids=vgcAllCids{1};
allTargCids=find(ismember(curTis.syn.edges(:,2),vgcAllCids));
allTargType=cid2type(curTis.syn.edges(:,1),curTis);
allRGCs=find(allTargType{1}==1);
allv2r=intersect(allTargCids,allRGCs);
...
outputDat=horzcat(targRGClist,targRGCcounts,outputType{3}');
...
[~, ia, ib] = intersect(outputDatSrtd(:,1), cidCntTyp(:,1));
testOrd(ia,1) = cidCntTyp(ib,2);

outputDatSrtd=horzcat(outputDatSrtd,testOrd);
