function graphObj = compareMorph(cidLists,fvDirs,options)
%offsets,ax,labLoc
arguments
       cidLists = {2,2}
       fvDirs = {'C:\work\localDat\Volumes\Final\Analysis\fvLibrary\',...
        'C:\work\localDat\Volumes\medRes\Analysis\fvLibrary\'}
       options.offsets = [0 230 295]
       %options.depth = 0
       options.ax = gca
       options.labLoc = 'NON'
end
...
%clf
hold on
bbox=zeros(2,3,length([cidLists{:}]));
cider=1;
for u=1:length(cidLists)
    cidList=cidLists{u};
    fvDir=fvDirs{u};
for i=1:length(cidList)
    ...
    if u==2
        ...
        verts=curFV.vertices;
        newVerts=verts-coffsets;
        curFV.vertices=newVerts;
    end
