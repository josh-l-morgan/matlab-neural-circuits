...
if sanityCheck==1
    figure();
    hold on
    scatter3(INLbord(:,2),INLbord(:,3),INLbord(:,1),5,'c.');
    scatter3(GCLbord(:,2),GCLbord(:,3),GCLbord(:,1),5,'m.');
    xp=[50:10:200];
    yp=[50:10:200];
    [XP,YP]=meshgrid(xp,yp);
    zp = (INLplane.Parameters(2)*XP + INLplane.Parameters(3)*YP + INLplane.Parameters(4));
    surf(xp,yp,zp);
    zp2 = (GCLplane.Parameters(2)*XP + GCLplane.Parameters(3)*YP + GCLplane.Parameters(4));
    surf(xp,yp,zp2);
    %scatter3(double(curVGCvox(:,1)),double(curVGCvox(:,2)),double(curVGCvox(:,3)),2,'ro')
    scatter3(double(curTis.syn.pos(:,1)),double(curTis.syn.pos(:,2)),double(curTis.syn.pos(:,3)),2,'ro')
end

%% get the depth for all synapses
allSynLocs=curTis.syn.pos;
allSynEdges=curTis.syn.edges;
[zg,zi,allSynDepths]=getIPLdepth(allSynLocs(:,3),allSynLocs(:,1),allSynLocs(:,2),GCLplane,INLplane);
...
if sanityCheck==1
    figure();
    hold on
    origZ=allSynLocs(:,3).*25;
    scatter(1:length(zg),zg,'m.');
    scatter(1:length(zi),zi,'c.');
    scatter(1:length(allSynLocs(:,3)),allSynLocs(:,3),'c.');
    figure();
    scatter(1:length(allSynDepths),allSynDepths,'r.');
...
for curVGCit=1:length(vgcCidList)
    ...
    curVGCvox=getCidVox(curVGCid,10,curDSObj.dsObj,curTis);
