...
for i=1:length(allSkels)
    ...
    onBpcSynIDs=find(inputs'==1 & ismember(preTypeDat{3},bpcONsubs));
    offBpcSynIDs=find(inputs'==1 & ismember(preTypeDat{3},bpcOFFsubs));
    ...
    for j=1:length(rgcSubTypeList)
        ...
        if sanityCheck
            ...
            if fig3
                scf3=figure();
                title(['cid' num2str(curCid) ': Inputs to ' curTis.cells.type.subTypeNames{1}{curRGCsubType} ...
                    ' x VGC Input synapse'])
                hold on
                for k=1:length(curSynIDs)
                    bpcIn=infMat(bpcSynIDs,curSynIDs(k));
                    bpcOffIn=infMat(offBpcSynIDs,curSynIDs(k));
                    bpcOnIn=infMat(onBpcSynIDs,curSynIDs(k));
                    bpcInNear=bpcIn(bpcIn>0.05);
                    bpcOffInNear=bpcOffIn(bpcOffIn>0.05);
                    bpcOnInNear=bpcOnIn(bpcOnIn>0.05);
                    amcIn=abs(infMat(amcSynIDs,curSynIDs(k)));
                    amcInNear=amcIn(amcIn>0.05);
                    %scatter(repmat(k,length(bpcInNear)),bpcInNear,25,'cs','filled');
                    scatter(repmat(k,length(bpcOffInNear)),bpcOffInNear,25,'cs','filled');
                    scatter(repmat(k+.1,length(bpcOnInNear)),bpcOnInNear,25,'gs','filled');
                    scatter(repmat(k+0.25,length(amcInNear)),amcInNear,25,'md','filled');
                end
                xlabel('synapse #');
                ylabel('influence');
                set(gca, 'YDir','reverse');
                %xticks([1:length(rgcSubTypeList)]+.2);
...
for i=1:length(allSkels)
    ...
    OFFlayerSynIDs=find(curSyns.depth<0.5);
    curList=intersect(onBpcSynIDs,OFFlayerSynIDs);
    atoneLocs=vertcat(atoneLocs,curSyns.pos(curList,:));
    atoneDepths=vertcat(atoneDepths,curSyns.depth(curList));
    atoneEdges=vertcat(atoneEdges,curSyns.edges(curList,:));
end
