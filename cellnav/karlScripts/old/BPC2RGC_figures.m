...
histDat=histcounts(bpcInputSubs,[1:length(curTis.cells.type.subTypeNames{7})]);
...
targSubsNames=curTis.cells.type.subTypeNames{7}(targSubIDs);
...
xlim([4,15]);
%xline(9.5);
xline(9.5,'HandleVisibility','off');
xlabel('IPL depth');
ylabel('synapse #');
...
for i=1:length(allSkels)
    ...
    for j=1:length(rgcSubs)
        ...
        if plotAll
            ...
            %tp2=figure();
            polarscatter(angV,20-bps2rgsrtd(:,2),(20-bps2rgsrtd(:,2))*10,colV,'o','filled');
            hold on
            ax = gca;
...
for j=1:length(rgcSubs)
    ...
    polarscatter(angV,20-bps2rgsrtd(:,2),(20-bps2rgsrtd(:,2))*20,colV,'.');
    ...
    for w=1:length(typesPresent(typesPresent>0))
        ...
        text([3 3 3],[20 10 5],{'0um','10um','15um'},'HorizontalAlignment','left','VerticalAlignment','middle');
        polarplot([2*pi/360:2*pi/360:2*pi],repmat([10],360),'k:');
        polarplot([2*pi/360:2*pi/360:2*pi],repmat([5],360),'k:');
        polarplot([2*pi/360:2*pi/360:2*pi],repmat([20],360),'k:');
