...
% Last Modified by GUIDE v2.5 24-Mar-2021 12:36:37
...
glob.NA.export.handles = handles;
...
makeMPNcnv(1) % Make MPN and associated directories
...
set(handles.textOut,'String','Making cell library ...')
pause(.01)
makeMPNcnv
makeCellLibrary
%makeObjectLibrary  %% Might not be used

%%Move volume transform
WPN = glob.NA.WPN;
endStr = regexp(WPN,'\Analysis');
volDir = WPN(1:endStr-1);
if exist([volDir 'volTransform.mat'],'file');
    copyfile([volDir 'volTransform.mat'],[WPN 'fvLibrary\volTransform.mat']);
end
...
glob.NA.export.useLib = glob.vol.libNames;
set(handles.popupmenu_useFvLib,'Value',1);
if ~isempty(glob.NA.export.useLib )
set(handles.popupmenu_useFvLib,'String',glob.NA.export.useLib );
else
...
set(handles.textOut,'String','Merging fvLibraries')
pause(.01)

mergeFvLibraries
set(handles.textOut,'String','Finished merging fvLibraries')



% --- Function that updates all the fields and glob variables for the fig
...
glob.NA.export.exportName = segStr;
...
glob.NA.export.useLib = get(handles.popupmenu_useFvLib,'String');
...
% --- Executes on button press in pushbutton_datFileFig.
function pushbutton_datFileFig_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_datFileFig (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in pushbutton_makeMergedVSS.
function pushbutton_makeMergedVSS_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_makeMergedVSS (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

set(handles.textOut,'String','Making merged vss file')
mergeMatSegsToVast_cnv()
set(handles.textOut,'String','Finished merged vss file')


% --- Executes on button press in UpdateCellLibrary_pushbutton.
function UpdateCellLibrary_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to UpdateCellLibrary_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
set(handles.textOut,'String','Updating cell library ...')
pause(.01)
makeMPNcnv
updateCellLibrary
%makeObjectLibrary  %% Might not be used
...
function pushbutton_makeObjectLibrary_Callback(hObject, eventdata, handles)

global glob

set(handles.textOut,'String','Updating object library ...')
pause(.01)
makeMPNcnv
makeObjectLibrary  %% Might not be used
set(handles.textOut,'String','Finished object library.')
