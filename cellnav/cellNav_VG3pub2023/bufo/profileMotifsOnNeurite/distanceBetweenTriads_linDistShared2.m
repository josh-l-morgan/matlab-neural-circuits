...
for t = 1:tNum
    ...
    for b2 = 1:length(tri.vid(t).b2vId);
        ...
        for r = 1:length(tri.id(t).b2rId)
            b2rPos = pos(tri.id(t).b2rId(r),:);
            for v2 = 1:length(tri.vid(t).v2rId);
                ...
                dLin(c,1) = d(tri.vid(t).b2vId(b2),tri.vid(t).v2rId(v2));
        ...
        tri.sourceTri(tsNum) = t;
        tri.dEuc{tsNum} = dEuc;
        tri.dLin{tsNum} = dLin;

        forShortest = dLin + dEuc(:,2)/1000;

        minLin = min(forShortest);
        minLinId = find(forShortest == minLin,1);

        tri.dLinSmallest(tsNum,1) = dLin(minLinId);
        tri.minLin(tsNum) = minLin;
        tri.minLinId(tsNum) = minLinId;


        hasSharedRibbon = sum(dEuc(:,2)==0) ;
        tri.hasSharedRibbon(tsNum) = hasSharedRibbon;
...
fromDyad = find((tri.hasSharedRibbon) & isOnSame');
notDyad = find((~tri.hasSharedRibbon) & isOnSame');
closeNotDyad = find((~tri.hasSharedRibbon) & isOnSame' & (dEuc(:,2) < 5));
