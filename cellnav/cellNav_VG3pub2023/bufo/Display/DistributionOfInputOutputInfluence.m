...
WaN = Wa ./ repmat(sum(Wa,1),[size(Wa,1) 1]);

[a idx] = sort(WaN(1,:),'descend');
WaNS = WaN(:,idx);
...
subplot(2,1,1)
hFirst = hist(WaNS(1,:),hRange);
bar(hRange,hFirst)
ylim([0 160])

subplot(2,1,2)
hFirstTen = hist(sum(WaNS(1:10,:),1),hRange);
bar(hRange,hFirstTen)
ylim([0 160])


cumSumW = cumsum(WaNS,1);
thresh = 0.5;
threshPoint = zeros(1,size(cumSumW,2));
for i = 1:size(cumSumW,2);
    threshPoint(i) = find(cumSumW(:,i)>=thresh,1);
end
hRange = [1:2:40];
hThreshPoint = hist(threshPoint,hRange);
bar(hRange,hThreshPoint)
