...
rPol = zeros(length(sg),length(useVgc),reps);
...
for v = 1:length(useVgc)
    ...
    for r = 1:reps
        ...
        for g = 1:length(sg)
            ...
            sgClose{g} = rClose(sgIds); % Get nodes closest to synapses
        end
        for g1 = 1:length(sg) % For every cell type
