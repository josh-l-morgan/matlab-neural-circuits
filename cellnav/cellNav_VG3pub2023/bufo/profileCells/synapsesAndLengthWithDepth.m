...
clear synPos cellSynPos cellSynCount
...
goodDepth = ~isnan(nDepth);

scatter3(allPos(goodDepth,1),allPos(goodDepth,2),nDepth(goodDepth),'k','.')


notCB = nDepth > 0.1;
sum(allLengths(notCB));
...
hold on
for i = 1:length(sg)
    plot(dRange,sg(i).dCount,'color',sg(i).col)
end
...
for b = 1:length(dRange)
    d = dRange(b);
    isDeep = (allDepths>(d-bin2/2)) & (allDepths<=(d+bin2));
    nCount(b) = sum(allLengths(isDeep));
end
maxNCount = max(nCount(:));
plot(dRange,nCount/10,'color',[.3 .3 .3])

subplot(2,1,2)
cla
hold on

ei = sg(2).dCount./(sg(1).dCount + sg(2).dCount);
ei = sg(2).dCount./(sg(1).dCount );

%plot(dRange,ei/10,'color',[1 0 0])
% 
eDens =  (sg(1).dCount)./(nCount);
plot(dRange,eDens,'color',[0 .8 0])

iDens = sg(2).dCount./nCount;
plot(dRange,iDens,'color',[1 0 0])

plot(dRange,nCount/maxNCount*.1,'color',[.3 .3 .3])


plot(dRange,sg(3).dCount./nCount,'color',sg(3).col)
plot(dRange,sg(4).dCount./nCount,'color',sg(4).col)
...
offP = find(sg(3).dCount == max(sg(3).dCount),1);
onP = find(sg(4).dCount == max(sg(4).dCount),1);


isBetween = offP:onP;
between = ei(isBetween);
eiP = isBetween(find(between==max(between),1))

res = [cid mean(ei(offP-binP:offP+binP)) ...
    mean(ei(onP-binP:onP+binP)) ...
    mean(ei(eiP-binP:eiP+binP))]
