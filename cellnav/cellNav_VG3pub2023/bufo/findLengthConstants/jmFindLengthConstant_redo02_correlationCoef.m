...
filterBySNR = 0; %remove rois with low signal
filterByEdge = 1;
weightErrors = 0;
standardize = 0;
...
noise = [0:1:10];
testLengths = 1:1:200;
onScale = 1;%[1./[5:-1:2] 1 [2:5]];
offScale = 1;

numCondType = 4;

condList = zeros(length(testLengths)*length(noise)*length(onScale)*length(offScale),numCondType);
L = length(testLengths);
c = 0;
for c1 = 1:L
    for c2 = 1:length(noise);
        for c3 = 1:length(onScale)
            for c4 = 1:length(offScale)
                c = c+1;
                condList(c,:) = [testLengths(c1) noise(c2) onScale(c3) offScale(c4)];
            end
        end
    end
end
