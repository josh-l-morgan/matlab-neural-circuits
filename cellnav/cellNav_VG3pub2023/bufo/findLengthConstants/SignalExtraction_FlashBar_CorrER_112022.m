function[] = SignalExtraction_FlashBar_CorrER_112022()
...
for r = 1:NumFile
    ...
    for i = 1:length(Exps)
        ...
        %         %%Replace with single, full field ROI
        %         ROIMask = ROIMask(:,:,1) *0+1;
        ...
        %%ROI pooled response polarity
        rPol = zeros(nROI,1);
        polMean = rPol;
        meanPol = rPol;
        topThresh = 0.9; %threshold for chunk to use for high vals
        varPol = rPol;
        for rn = 1:nROI
            ...
            rMode = mode(rPix);
            %rPix = rPix-rMode;
            rOffPix = squeeze(rPix(:,TOFF,:));
            rOnPix = squeeze(rPix(:,TON,:));

            srOffPix = reshape(rOffPix,4,size(rOffPix,2)*size(rOffPix,3));
            srOffPix = sort(srOffPix,2,'ascend');
            srOnPix = reshape(rOnPix,4,size(rOnPix,2)*size(rOnPix,3));
            srOnPix = sort(srOnPix,2,'ascend');
            msrOnPix = mean(srOnPix(:,topThresh *size(srOnPix,2):end),2);
            msrOffPix = mean(srOffPix(:,topThresh*size(srOffPix,2):end),2);

            polMean(rn) = (median(msrOffPix)-median(msrOnPix))/(median(msrOffPix)+median(msrOnPix));
            repPol = (msrOffPix-msrOnPix)./(msrOffPix+msrOnPix);
            meanPol(rn) = median(repPol);

            varPol(rn) = 1-var(repPol);

            scatter(msrOnPix,msrOffPix)
            hold on
            plot([0 10],[0 10])
            xlim([0 10])
            ylim([0 10])
            ...
            rOff = mean(rOffPix(:));
            rOn = mean(rOnPix(:));
            rPol(rn) = (rOff-rOn)/(rOff+rOn);
        end
        Polarity3 = rPol;
        Polarity4 = polMean;

        %%Polarity 2
        meanOff = mean(mPixRespNrm(TOFF, :, :));
        ...
        for rn = 1:nROI
            ...
            mOff = sum(sum(rOffPix,3),2)/(size(rOffPix,3)*size(rOffPix,2));
            mOn = sum(sum(rOnPix,3),2)/(size(rOnPix,3)*size(rOnPix,2));
            mixOff = mOff(mPairs(:,1));
            mixOn = mOn(mPairs(:,2));

            mixPol = (mixOff-mixOn)./(mixOn+mixOff);
            qual(rn) = 1-var(mixPol);
        end



        ROI.Polarity1(useRois,1) = mean(Polarity' * wAmplitude)';
        ROI.Polarity2(useRois,1)  = mean(Polarity2' * wAmplitude)';
        ROI.SNR(useRois,1) = median(SNRM, 1)';
        ROI.Polarity3(useRois,1) = Polarity3;
        ROI.Polarity4(useRois,1) = Polarity4;
        ROI.qual(useRois,1) = qual;
        ROI.varPol(useRois,1) = varPol;

    end
end
ROI.ROITable = ROITable;%[ROITable, ROI.Polarity, ROI.SNR];
%save('../individualPointMasksSpread\ROI.mat','ROI')

ROI.Polarity = ROI.Polarity4;
ROI.SNR = ROI.qual;
