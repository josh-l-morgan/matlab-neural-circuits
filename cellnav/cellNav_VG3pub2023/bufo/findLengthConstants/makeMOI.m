...
for c = 1:length(cids)
    cid = cids(c);
    disp(sprintf('Making MOI for cid%d (%d of %d)',cid,c,length(cids)))
    ...
    if exist([smDir smxName],'file')
        ...
        MOI.d{c} = smx.syn2Skel.skelTopoDist;
    else
        smName = sprintf('sm_cid%d.mat',cid);
        if exist([smDir smName])
            load([smDir smName]);
            MOI.d{c} = sm.syn2Skel.skelTopoDist;
        else
            MOI.good(c) = 0;
        end
    end
    ...
    if MOI.good(c)
        ...
        if exist([swcDir nrnName],'file')
            ...
            lc = 20;
            MOI.isNeu(c) = 0;
            MOI.W{c} = exp(-MOI.d{c}/lc); % Apply length constant
            MOI.p{c}.lc = lc;
        end
    end
end
...
save([SPN 'MOI.mat'],'MOI')


%% Get polarity from model

start_mV = double(MOI.p{1}.ex.e_pas);

%%Calculate Off bias for each node of each cell
for i = 1:length(MOI.cids)
    cid = MOI.cids(i);
    targ = find(NOI.cids == cid,1);
    preSign = NOI.preSigns(targ).preSign;
    if ~isempty(preSign)
        W = MOI.W{i};
        W = W-start_mV;
        MOI.c(i).mOn = sum(W(preSign == 0,:),1);
        MOI.c(i).mOff = sum(W(preSign == 1,:),1);
        MOI.c(i).mUnk = sum(W(preSign == -1,:),1);
        MOI.c(i).offBias = (MOI.c(i).mOff-MOI.c(i).mOn)./(MOI.c(i).mOff + MOI.c(i).mOn);
    else
        MOI.c(i).offBias = [];
    end
end

%%Get polarity for ROI
MOI.offBias = []; MOI.on = []; MOI.off = [];
for i = 1:length(roiCids)
    targ = find(MOI.cids == roiCids(i),1);
    MOI.offBias(i) = MOI.c(targ).offBias(SOI.closeNode(i));
    MOI.on(i) = MOI.c(targ).mOn(SOI.closeNode(i));
    MOI.off(i) = MOI.c(targ).mOff(SOI.closeNode(i));
    MOI.unk(i) = MOI.c(targ).mUnk(SOI.closeNode(i));
end
