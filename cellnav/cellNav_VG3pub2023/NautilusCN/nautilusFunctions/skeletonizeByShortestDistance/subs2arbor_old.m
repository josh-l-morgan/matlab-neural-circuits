...
%% Get obj surface
    ...
    showPred3D(surfVox.subs,seedPath.pred,seedPath.seed)  
    ...
    showTips = pIDX(1:length(pathL.tips)/100);
    showPathL3D3(surfVox.subs,pathL,[],showTips)
    ...
    propPath = path2prop(surfVox,prop,6) %find nearest bigger property
    [sortPaths pIDX]  = sort(propPath.lengths,'descend');
    showTips = pIDX(1:length(propPath.tips)/100);
    showPathL3D3(surfVox.subs,propPath,[],showTips)    
   ...
   % prop = surfVox.path2surf.rads.atMaxAxRad;
    ...
    showPathL3D3(surfVox.subs,propPath,mProp,showTips)
