...
Requires running setUpNrnExperiment.m to generate experiment.mat file that 
contains experimental variables that will get loaded into ex

Requires running makeSM.m for any cell being test in order to generate cidX.swc
file that contains cell morphology in swc format

Requires running prepareNeps.m to generate the sm2nrn_cidX.mat file that has 
synapse information about each cell.

These functions can be run individually or through the skeletonization/analysis
window in cellNav
...
for a in range(0,len(secs_list)):
    ...
    #allSeg.append(h.allSeg(1, sec=sec))     
...
#synEdges = [1,50,100, 150,200, 250, 300]
#synEdges = [1,2,3,4,5,6,100,101,102,103]
...
for syn_num in range(0,len(synEdges)):
    ...
    syn = h.AlphaSynapse(.1,sec = sec_syn) #0.1 is the synapse position along cylindrical section 1610. The cylinder length is scaled to the range 0 - 1.
    ...
    if syn_num == 0:
        ...
        for a in range(0,len(secs_list)):
            ...
            y_sec.append(h.y3d(1, sec=sec))
            z_sec.append(h.z3d(1, sec=sec))
            #secname.append(h.secname(1, sec= sec))
            #allSeg.append(h.allSeg(1, sec=sec))            
