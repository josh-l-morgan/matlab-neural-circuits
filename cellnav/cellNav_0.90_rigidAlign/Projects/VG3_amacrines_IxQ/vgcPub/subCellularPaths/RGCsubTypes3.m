...
influenceMatRGC = zeros(length(COI.bpcGroupCids),length(tis.cids));
influenceMatSyn = zeros(length(COI.bpcGroupCids),length(tis.syn.pre));
                    ...
                    for r = 1:length(rCids)
                        ...
                        if ~isempty(isPre) & ~isempty(isPost)
                           ...
                           trackPost(isPost) = trackPost(isPost) + sum(cutEDist,1);
                            ...
                            cid2RGCgroup(targR) = rG; 
                            syn2RGCgroup(isPost) = rG;
                            pause(.01)
                        end
...
onInfluenceS = sum(influenceMatSyn(useBGroup(1:3),:),1);
offInfluenceS = sum(influenceMatSyn(useBGroup(4:end),:),1);
OFFbiasS = (offInfluenceS-onInfluenceS)./(offInfluenceS + onInfluenceS);

rL = length(useRGroup)
scatter(syn2RGCgroup,OFFbiasS,'o','k')
hold on
for i = 1:rL
    polS = OFFbiasS(syn2RGCgroup == useRGroup(i));
    scatter(useRGroup(i),median(polS),260,'+','r')
end
hold off
% 
% 
...
%     subplot(rL,1,i)
%     subplot(1,1,1)
%     brG = influenceMatRGC(useBGroup, cid2RGCgroup == useRGroup(i));
%     brS = influenceMatSyn(useBGroup, syn2RGCgroup == useRGroup(i));
%     plot(brS)
%     pause
% end
