...
for v = 1:length(useVgc)
    ...
    synType( (syn.post == cid) & (syn.preClass == 7) & (syn.preSub==3)) = 1;
    synType( (syn.post == cid) & (syn.preClass == 7) & (syn.preSub==4)) = 2;
    synType( (syn.post == cid) & (syn.preClass == 7) & (syn.preSub==5)) = 3;
    synType( (syn.post == cid) & (syn.preClass == 7) & (syn.preSub==6)) = 4;
    synType( (syn.post == cid) & (syn.preClass == 7) & (syn.preSub==7)) = 5;
    synType( (syn.post == cid) & (syn.preClass == 7) & (syn.preSub==8)) = 6;
    %synType( (syn.pre == cid) & (syn.postClass == 1) ) = 3; % rgc
    %synType( (syn.pre == cid) & (syn.postClass == 8) ) = 4; % amc out
    synType( (syn.pre == cid) & (syn.postClass == 1) & (syn.postSub == 21)) = 7; %51
    synType( (syn.pre == cid) & (syn.postClass == 1) & (syn.postSub == 23)) = 8; %4i
    synType( (syn.pre == cid) & (syn.postClass == 1) & (syn.postSub == 24)) = 9; %4ow
    synType( (syn.pre == cid) & (syn.postClass == 1) & (syn.postSub == 29)) = 10;
    synType( (syn.pre == cid) & (syn.postClass == 1) & (syn.postSub == 51)) = 11;
    synType( (syn.pre == cid) & (syn.postClass == 1) & (syn.postSub == 58)) = 12;
    ...
    for g = 1:length(synGroups);
        ...
        for b = 1:(length(bRange)-1)
            ...
            [ y x ] = find(((synDG>=(bRange(b))) & (synDG<(bRange(b)+distBin))));
            if ~isempty(x)
...
titleMat={bpcSubNames{:},rgcSubNames{:}};

for g = 1:6 %1:size(g2g,1)
    for t = 1:6 %1:size(g2g,2)
        %subplot(size(g2gN,1),size(g2gN,2),(g-1)*size(g2gN,1)+t)
        subplot(6,6,(g-1)*6+t)
        plot(bRange(1:length(bRange)-1),squeeze(g2gN(g+6,t,:)))
        ylim([0 2])
        xlim([0 50])
        title([titleMat{t} ' x ' titleMat{g+6}]);
    end
end
