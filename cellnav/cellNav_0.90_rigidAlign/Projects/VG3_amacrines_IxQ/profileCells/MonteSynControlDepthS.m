...
for v = 1:length(useVgc)
    ...
    %%Randomize
    rMinD = zeros(length(sg),length(sg),reps);
    rWD = rMinD;
    ...
    for r = 1:reps
        ...
        for g = 1:length(sg)
            sgIds = [synIds{sg(g).sT}];
            sgClose{g} = rClose(closest(sgIds));
        end
        for g1 = 1:length(sg)
            for g2 = 1:length(sg)
                d = D(sgClose{g1},sgClose{g2});
    ...
    pol(:,v) = (wD(:,1)-wD(:,2))./(wD(:,1)+wD(:,2)); %Calculate polarity
    rPol = (rWD(:,1,:)-rWD(:,2,:))./(rWD(:,1,:)+rWD(:,2,:)); % random polarity
    mrPol(:,v) = squeeze(mean(rPol,3)); %mean random polarity
    ...
    if 1 %%Show hist
        ...
        for p = 1:size(pol,1)
            ...
            hold on
            hPol(p,:) = hist(squeeze(rPol(p,:,:)),polRange);
...
sPol = (sNum(1,:)-sNum(2,:))./(sNum(1,:)+sNum(2,:));
%% Display
clf
for g = 1:length(sg)
    
    subplot(1,length(sg),g)
    hold on
...
for g = 3:length(sg);
    ...
    realSac = [realSac abs(physPol2(g-2) - realPol(isReal))];
    randSac = [randSac abs(physPol2(g-2) - randPol(isReal))];
