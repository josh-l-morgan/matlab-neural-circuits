...
%expName = 'exp_01_c'; %Good with inhib
%expName = 'exp_01_d'; %Good with inhib
%expName = 'exp_01_e'; % Good with inhib
...
for v = 1:length(vCids)
    ...
    if  exist([cellDir 'expInfo.mat'],'file')
        ...
        exciteWeight = expInfo.exciteWeight(:);
        offInhibScale = expInfo.offInhibScale(:);
...
datNum = [dat.num]
datVars = find(datNum>1);
datSing = find(datNum<2);
sd = [datVars datSing];
dimNum = length(datVars);
...
for dC = 1:d4.num
    for dR = 1:d3.num
        ...
        su = surf(mX,mY,em(:,:,dR,dC),'facecolor','interp');
        su.CData = propMat(:,:,dR,dC);
        ...
        if (dC == b4) & (dR == b3)
            ...
            scatter3(d2.ux(b2),d1.ux(b1),minError,100,'o','r','linewidth',3)
...
sc = scatter3(log10(conductances),log10(inhibWeight),meanError,'markerfacecolor','flat')
%sc = scatter(log10(conductances),log10(inhibWeight),'markerfacecolor','flat')
prop = meanError;
prop = prop-min(prop);
if max(prop)<1
    prop(prop==max(prop)) = 1;
end
prop = round(prop * 99/max(prop)+1);
pCol = jet(100);
sc.CData =pCol(prop,:); 
...
%% plot best cond each roi
sp = subplot(1,3,3); cla(sp),hold on


[sortError idxE] = sort(meanError,'ascend')

bestCon = log10(conductances(idxE))
bestIW = log10(inhibWeight(idxE))
...
mAlph = zeros(numScat,1) + .2;
mAlph(1) = .6;

for c = 1:numScat
    uPNs = uPN(:,idxE(c));
    scatter(uCN,uPNs+c/50,20,'markeredgecol',mCol(c,:),'markeredgealph',mAlph(c),...
        'markerfacealpha',0)
end
