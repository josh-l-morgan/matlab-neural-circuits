...
weightRois = 1; % compensate for uneven sampling of polarities
...
goodRoi = goodRoi & (caPol<1) & (caPol>-1);
...
elseif 0 %mean error
    ...
    rmseN = 0 - meanDif;
elseif 1 % weighted mean error
    useWeights = gWeights;
    difMatN = caPolMatN - usePredN;
    useWeights = repmat(useWeights,[1 size(difMatN,2) size(difMatN,3) size(difMatN,4) size(difMatN,5)]); 
    rmseN = 0 - sum((abs(difMatN).*useWeights),1)./sum(useWeights,1);
else
...
if 1
    ...
    if 0
        ...
        scat1 = scatter( caPolN,usePredN(:,round(bxm),round(bs1m),round(bs2m),round(bnm)),15,'filled');
        scat1.CData = SNRcol(useRoi,:)*.7;
        title(sprintf('length constant %0.2f',testLengths(bxm)))
        xlim([-1 1])
        ylim([-1 1])
        axis equal
