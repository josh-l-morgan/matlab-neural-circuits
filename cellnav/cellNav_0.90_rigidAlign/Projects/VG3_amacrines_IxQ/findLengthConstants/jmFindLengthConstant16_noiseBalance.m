        ...
        for n = 1:length(noise)
            for s = 1:length(onScale)
                for s2 = 1:length(offScale)
                    Won = W .* onMat * onScale(s); %calculate weight of all ON inputs for each roi
                    Woff = W .* offMat * offScale(s2); % same as Won

                    sumOn = sum(Won,1) + noise(n); % sum all on influences across each ROI
                    sumOff = sum(Woff,1) + noise(n); % as sumOn
