...
for v = 1:length(vCids)
    ...
    expFileNameOFF = sprintf('%sres%d.mat',cellDir,1);
    expFileNameON = sprintf('%sres%d.mat',cellDir,2);
    load(expFileNameOFF);
    nn(v).exOff = nnRes;
    load(expFileNameON);
    nn(v).exOn = nnRes;
    ...
    for i = 1:length(useNodes)
        ...
        gnn.nnON(isCid(i)) = nn(v).exOn.maxV(nTarg)+60;
        gnn.nnOFF(isCid(i)) = nn(v).exOff.maxV(nTarg)+60;
...
%% Compare nn to roi
clf, hold on
scatter(caPol(useRoi),gnn.pol(useRoi))
plot([-1 1],[0 0],'k')
plot([0 0],[-1 1],'k')
axis 'equal'
