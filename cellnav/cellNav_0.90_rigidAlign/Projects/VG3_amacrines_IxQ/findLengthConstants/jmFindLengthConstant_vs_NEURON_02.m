...
if 0
    ...
    expName = 'exOFFON_Cond03';
elseif 0
    %% ON OFF Bipolar cells. No amacrines. Diameters = 0.5 um
    expName = 'exOFFON_DiamSet_0-5';
elseif 1
    expName = 'exOFFON_inhibit_7'; %Stronglly OFF biased
end
        ...
        for con = 1:numCon
            ...
            if exist(expFileNameON,'file')
                ...
                for i = 1:length(useNodes)
                    ...
                    gnn.nnON(isCid(i),con) = nn(v,con).exOn.maxV(nTarg)+60;
                    gnn.nnOFF(isCid(i),con) = nn(v,con).exOff.maxV(nTarg)+60;
                end
            end
        end
    end
...
for v = 1:length(vCids)
    ...
    numOn(v) = sum(preSign==2);
    numOff(v) = sum(preSign==1);
    ...
    predPol = zeros(numCond,length(isCid));
    predOn = predPol;
    predOff = predPol;
    for c = 1:numCond
        lc = condList(c,1);
    ...
    predList(:,isCid) = predPol;
    predListOn(:,isCid) = predOn;
    predListOff(:,isCid) = predOff;
end
...
for c = 1:length(showLogGm)
    ...
    uP = sNPol(useRoi,targ);
    scatter(uC,uP,20,'markeredgecol',mCol(c,:),'markeredgealph',mAlph(c),...
        'markerfacealpha',0)
end
