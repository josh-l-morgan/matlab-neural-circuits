...
showType = 3
if showType ==1 % No ihibition
    cMB = [];   eMB = [];   iMB = [-inf];   osMB = [];
    cMax = [];  eMax = [];  iMax = [];  osMax = [];
    cMin = []; eMin= []; iMin = []; osMin = [];
elseif showType ==2
    cMB = [];   eMB = [-3];   iMB = [];   osMB = [];
    cMax = [-5];  eMax = [];  iMax = [];  osMax = [];
    cMin = []; eMin= []; iMin = []; osMin = [];
elseif showType == 3; % Compare all
    cMB = [];   eMB = [];   iMB = [];   osMB = [];
    cMax = [];  eMax = [];  iMax = [];  osMax = [];
    cMin = []; eMin= []; iMin = []; osMin = [];
end
...
%% find differencs
clear useError binError errorOfBins errorOfMeans errorOfVars rms predCor
for c = 1:numCon
    uPNs = uPN(:,c);
...
drawnow
scatter3(bX2,bX1,minError,100,'o','r','linewidth',3)
...
disp(bestStr)
errorStr = sprintf('min error %0.4f, cc = %0.3f',minError,predCor(bInd));
disp(errorStr)
titleStr = sprintf('%s\n%s',errorStr,bestStrS);
title(titleStr)
