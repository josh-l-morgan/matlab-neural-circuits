...
for v = 1:length(SOI.cids)
    vCid = SOI.cids(v); %Get cid of VG3 to run
    ...
    d = SOI.cell(v).d(:,useNodes); % Get matrix of distiances from all synapse nodes (Y) to all functional roi nodes (X)
    preSign = SOI.cell(v).preSign; % Get list of signs (0=ama, 1 = on Bip, 2 = off Bip) for synapses onto VG3
    ...
    onMat = preSignMat == 1; % make logical matrix of on synapses with shape of d
    offMat = preSignMat == 2; % as onMat
