...
%% Show Measured Polarities by depth
clf

uFRoi = unique(fRoi);

%%Find data of each functionally imaged region
[zGCL,zINL,fZ] = getIPLdepth(SOI.pos(:,3),SOI.pos(:,1),SOI.pos(:,2),[],[]);
polRange = [-1:.1:1];
for i = 1:length(uFRoi)
    isFRoi = find(fRoi==uFRoi(i));
    fRoiZ(i) = median(fZ(isFRoi));
    hPolFRoi(i,:) = hist(caPol(isFRoi),polRange);
    subplot(length(uFRoi),1,i)
    bar(polRange,hPolFRoi(i,:));
    title(sprintf('frame %d, depth %0.3f',uFRoi(i),fRoiZ(i)))
    xticks manual
    xticks([])
    ylim([0 30])
end
...
%% Show all correlation points All together now
clf
axis('equal')
set(gca,'clipping', 'off')
hold on
...
propCol = propCol / max(propCol);
pCol = hsvCol(round(propCol*(size(hsvCol,1)-1))+1,:);

hsvCol = hsv(6);
pCol = hsvCol([1 2 3 5 4 6 6 4 3 1 2 5],:);


for p = 1:length(SOI.cids)
    %%Show

    nTarg = find(NOI.cids==SOI.cids(p),1);
    isCid = roiCids == SOI.cids(p);
    nep = NOI.neps(nTarg).nep;
    pos = nep.pos;
    scatSkel = scatter3(pos(:,1),pos(:,2),pos(:,3),5,'o');
    scatSkel.MarkerFaceColor = [0 0 0]
    scatSkel.MarkerFaceAlpha = .1
    scatSkel.MarkerEdgeColor = [0 0 0]
    scatSkel.MarkerEdgeAlpha = 0

    for z = 1:length(uFRoi)
        useROI = find(isCid & (fRoi == uFRoi(z)));
        useN = SOI.closeNode(useROI);
        scatter3(pos(useN,1),pos(useN,2),pos(useN,3),50,...
            'o','filled','markerfacecolor',pCol(z,:),...
            'markeredgealpha',.5,...
            'MarkerEdgeColor',pCol(z,:)*.5);
...
if 1
    ...
    for d  = 1:size(usePredN,2)
        ...
        for p = 1:length(POI.testDepths)
            ...
            isPlane = (POI.plane == p) & (POI.planeDist<=maxDist);
    ...
    bestFitInd = find(pError==min(pError),1);
...
%%Plot best fit
clf
bestPred = usePredN(:,bestFitInd,round(bs1m),round(bs2m),round(bnm));
...
for p = 1:length(POI.testDepths)
    ...
    medPred(p,d) = median(pols);
    histPols = hist(pols,polRange);
    bar(polRange,histPols,'k')
    title(sprintf('length constant %0.2f, testDepth = %0.2f',...
        testLengths(bestFitInd),POI.testDepths(p)))
    scatter(POI.popPol(p),0,'r','filled')
end
...
for v = 1:length(useCid)
    vCid = useCid(v);
    useRoi = find((goodRoi>0) & (POI.poiCids==vCid));
