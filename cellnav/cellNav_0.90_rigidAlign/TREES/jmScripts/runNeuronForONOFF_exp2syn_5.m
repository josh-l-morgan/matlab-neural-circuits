...
if testType == 1 %Martix of values
...
elseif testType ==2; %list each
    
    runCids = [ 2 3 4 5 13 14 20]    
    testConductances = 6 * 10.^[-5];
    testExciteWeight = 2.6 * 10.^[-2 -2];
    testInhibWeight = 10.^[-2.5 -5];
    testOffScales = [0.1 1.25];
...
%% make tests lists for matrix texts
if (testType == 1) | (testType ==3)
    c = 0;
    clear testInhibWeight runConThisTime testExciteWeight testOffScales testConductances
    for tC = 1:length(eachCond)
        for tI = 1:length(eachInhib)
            for tE = 1:length(eachExcite)
                for tO = 1:length(eachOffScale)
                    c = c+1;
                    testConductances(c) = eachCond(tC);
                    testInhibWeight(c) = eachInhib(tI);
                    testOffScales(c) = eachOffScale(tO);
                    testExciteWeight(c) = eachExcite(tE);
                end
            end
        end
    end
...
onoffInhibScale = [1 .5];% off weight will be reset with test conditions
...
for v = 1:length(runCids)
    ...
    tree{1}.R = tree{1}.R * 0 + 1; % Make everything dendrite
    ...
    trees = t2n_writeTrees(trees,[],mtrFile);                 % transform tree to NEURON morphology (.hoc file). This only has to be done once for each morphology
    ...
    expInfo.exciteWeight = testExciteWeight;
    expInfo.offInhibScale = testOffScales;
    ...
    for con = runConThisTime
        ...
        for e = 1:length(activate)
            ...
            if ~exist(expFileName,'file')
                ...
                syn1.weight = testExciteWeight(con);
                syn2.weight = testInhibWeight(con);
                onoffInhibScale(2) = testOffScales(con);
                ...
                nnRes.syn1 = syn1;
                nnRes.syn2 = syn2;
                nnRes.onoffInhibScale = onoffInhibScale(e);
...
% jmFindLengthConstant_vs_NEURON_06
% disp('finished collecting results')
