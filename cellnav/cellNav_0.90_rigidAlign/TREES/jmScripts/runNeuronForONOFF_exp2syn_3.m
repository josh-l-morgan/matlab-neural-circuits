...
if testType == 1 % test conductances without inhibitory inputs
    ...
    showVideo = 0;
elseif testType == 2; % ihibitory test
    runCids = [ 2 3 4 5 13 14 20]
    testInhibWeight = [     10^-3     10^-4   10^-5    10^-4.5   10^-3.5 ];
    testConductances = zeros(1,length(testInhibWeight)) + 10^5;
    runConThisTime = [1:5];
    expName = 'testOFFON_changeInhib_03';
    showVideo = 0;

elseif testType == 3 %test
    runCids = 2;
    testConductances = [   10^-6];
    testInhibWeight = [10^-3.5];
    runConThisTime = [1 :length(testConductances)];
...
syn2.weight = 10^-3; % peak current in nS, CHANGED IN EACH CONDITION
...
for v = 1:length(runCids)
    ...
    for con = runConThisTime
        ...
        for e = 1:length(activate)
            ...
            if 1;%~exist(expFileName,'file')
                ...
                while 1
                    ...
                    % 
                    %     try
                    %         if isempty(allFig(f).Number)
                    %             eval(allFig(f).CloseRequestFcn);
                    %         elseif (allFig(f).Number == fig.Number)
                    %             delete(allFig(f));
                    %         end
                    %     end
                    % 
        ...
        sp = subplot(3,2,1), hold on
        plot(sp,nTime,rootV(1,:),'b')
        plot(sp,nTime,rootV(2,:),'r')


        sp = subplot(6,2,2), hold on
