...
if testType == 1 % test conductances without inhibitory inputs
    ...
    testConductances = [ 10^-3 10^-2.5 10^-3.5 10^-2  10^-4  10^-5 10^-6 10^-7 10^-1 10^-1.5];
    testInhibWeight = zeros(length(testConductances));
    runConThisTime = [1:6];
    expName = 'exOFFON_changeCond_04';
    showVideo = 0;
elseif testType == 2;
    runCids = [ 2 3 4 5 13 14 20]
    testConductances = [    10^-4   10^-4   10^-4   10^-4   10^-4];
    testInhibWeight = [     10^-6   10^-5   10^-4   10^-3   10^-2]
    runConThisTime = [1 :length(testConductances)];
    expName = 'testOFFON_changeInhib_01';
    showVideo = 0;

elseif testType == 3 %test
    runCids = 2;
    testConductances = [   10^-2.75];
    testInhibWeight = zeros(length(testConductances));
    runConThisTime = [1 :length(testConductances)];
...
syn1.weight = 10^-3; % peak current in nS

syn2.tau1 = 1;
syn2.tau2 = 60;
syn2.i = defaultConductance;%9*10^-6;%0.01;
syn2.weight = 10^-4; % peak current in nS, CHANGED IN EACH CONDITION

%%Alpha parameters
syn1.gmax = 0.001;%26 * 10^-6; %0.001; % uS
syn1.onset = 10;
syn1.onsetSTD = 10;
...
neuron.params.tstop = 40;                                  % stop simulation after this (simulation) time [ms]
neuron.params.prerun = -20; %400                           % add a pre runtime [ms] to let system settle
...
%% Show synapses
    ...
    for con = runConThisTime
        ...
        for e = 1:length(activate)
            ...
            if 1;%~exist(expFileName,'file')
                ...
                if ~useExp2Syn
                    ...
                    exGmax = cat(1,repmat(syn1.gmax,[L1 1]),repmat(syn2.gmax,[L2 1]));
                    exOnset = cat(1,repmat(syn1.gmax,[L1 1]),repmat(syn2.gmax,[L2 1]));
                   ...
                   if useTimeCourse
                    ...
                    for s = 1:length(nID1)
                        ...
                        nneuron.pp{1}.AlphaSynapse(c) = ...
                            struct('node',syn1.nodes(nID1(s)),'gmax',syn1.gmax,'onset',onsets1(s),'e',syn1.e);% add an AlphaSynapse at this location
                    end
                   ...
                   else %simplest stimulation

                       nneuron.pp{1}.AlphaSynapse = struct('node',syn1.nodes,'gmax',syn1.gmax,'onset',syn1.onset);% add an AlphaSynapse at this location
                       nneuron.record{1}.AlphaSynapse = struct('node',syn1.nodes,'record','i');      % record synaptic current

                   end


                    %nneuron.record{1}.AlphaSynapse = struct('record','i');
                else
                ...
                for f = 1:length(allFig)
                    if allFig(f).Number ~= fig.Number
                        delete(allFig(f).Number);
                    end
                end
       ...
       set(fig,'Visible','on');
        ...
        prop(isnan(prop)) = 100;
        sp = scatter3(sp,tr.X,tr.Y,tr.Z,5,'o','k','markerfacecolor','flat');
