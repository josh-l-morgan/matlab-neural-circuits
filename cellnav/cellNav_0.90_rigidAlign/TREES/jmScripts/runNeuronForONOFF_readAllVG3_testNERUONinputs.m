...
if 1 % all
    ...
    testConductances = [10^-3 10^-2.75 10^-2.5 10^-2 10^-4 10^-5  10^-7  10^-6  10^-1 10^-3.5 10^-1.5];
    runConThisTime = [1 : 6];
...
for v = 1:length(runCids)
    ...
    for t = 1                                                                   % preconfiguration to loop through several morphologies, now only one morphology is considered
        neuron.mech{t}.all.pas = struct('g',0.0003,'Ra',Ra,'e',-60,'cm',Cm);    % add passive channel to all regions and define membrane capacity [µF/cm²], cytoplasmic resistivity [Ohm cm] and e_leak [mV]
    ...
    for con = runConThisTime
        ...
        prop(isnan(prop)) = 100;
        sp = scatter3(tr.X,tr.Y,tr.Z,'o','k','markerfacecolor','flat');
