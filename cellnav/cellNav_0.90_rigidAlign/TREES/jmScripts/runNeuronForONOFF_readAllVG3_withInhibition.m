...
for v = 1:length(runCids)
    ...
    for con = runConThisTime
        ...
        for e = 1:length(activate)
            ...
            if 1;%~exist(expFileName,'file')
                ...
                for s = 1:length(syn1.nodes)
                    onsets = randn(length(syn1.nodes),1) * syn1.onsetSTD + syn1.onset;
                    nneuron.pp{1}.AlphaSynapse(s) = ...
                        struct('node',syn1.nodes(s),'gmax',syn1.gmax,'onset',syn1.onset,'e',syn1.e);% add an AlphaSynapse at this location
                end

                %nneuron.record{1}.AlphaSynapse(s) = struct('node',excite(s),'record','i');      % record synaptic current
                
                %% Make Inhibitory synapses
                c = length(nneuron.pp{1}.AlphaSynapse);
                for r = 1:syn2.repeats
                    onsets = randn(length(syn2.nodes),1) * syn2.onsetSTD + syn2.onset;
                    for s = 1 : length(syn2.nodes)
                        c = c+1;
                        nneuron.pp{1}.AlphaSynapse(c) = ...
                            struct('node',syn2.nodes(s),'gmax',syn2.gmax,'onset',syn2.onset,'e',syn2.e);% add an AlphaSynapse at this location
        ...
        for n = 1:size(synVs,2);
            plot(nTime,squeeze(synVs(1,n,:)))
        end
        
       subplot(2,1,2),hold on
        for n = 1:size(synVs,2);
            plot(nTime,squeeze(synVs(2,n,:)))
        end
