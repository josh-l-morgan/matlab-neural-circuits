...
% freeNodes = ones(nodeNum,1);
% trackNodes = zeros(nodeNum,1);
...
%         next1 = find((edges(:,2)==last(n)) & free); % find all edges in which checked node is parent
%         next2 = find((edges(:,1)==last(n)) & free); % find all edges in which checked node is parent
%         free(next1) = 0; % clear the discovered edge from being checked again
%         free(next2) = 0; % clear the discovered edge from being checked again
%         nextNode1 = edges(next1,1); %grab the child node IDs from edges
%         nextNode2 = edges(next2,2); %grab the child node IDs from edges
%         foundNodes = unique(cat(1,nextNode1,nextNode2));
%         trackNodes(foundNodes) = trackNodes(foundNodes) + 1;
%         foundNodes = foundNodes(freeNodes(foundNodes)); %only add nodes once
%         newN = cat(1,newN, foundNodes); % collect child nodes of discoverd edges
%         if ~isempty(foundNodes) % if a new node has been discovered
%             freeNodes(foundNodes) = 0;
%             pred(foundNodes) = c; %set parent to old position space
%             predOld(foundNodes) = last(n);
%         end
%     end
%
%     if ~mod(i,100)
...
type2(newID,1) = type;
meanNodeRad2(newID,1) = sm.nep.meanNodeRad;% * 0 + .01;
...
%% Make sm compliant with swc
if 0
    'hi'
    sm.skel2skel.linDist(newID,newID) = sm.skel2skel.linDist;
    sm.skel2skel.eucDist(newID,newID) = sm.skel2skel.eucDist;
    if isfield(sm,'syn2Skel')
        sm.syn2Skel.skelEucDist(:,newID) = sm.syn2Skel.skelEucDist;
        sm.syn2Skel.skelTopoDist(:,newID) = sm.syn2Skel.skelTopoDist;
        sm.syn2Skel.syn2SkelDist(:,newID) = sm.syn2Skel.syn2SkelDist;
    end
end
