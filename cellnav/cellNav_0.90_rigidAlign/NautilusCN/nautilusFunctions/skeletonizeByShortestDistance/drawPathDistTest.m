function[pathLengths] = drawPathDistTest(pred,dists,increment,tips,pos)
...
for t = 1:length(sortTips)
    ...
    tipDist(t) = 0;
    bases(t) = tip;
    col = cmap(ceil(rand*100),:)
    for r = 1:numSurf*2
         
        if tip<1,
        %scatter3(pos(tip,1),pos(tip,2),pos(tip,3),100,'o','filled','markerfacecolor',col)
            break,
