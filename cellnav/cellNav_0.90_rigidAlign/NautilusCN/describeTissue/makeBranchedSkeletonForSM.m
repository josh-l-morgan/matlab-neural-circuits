...
load('MPN.mat');
%MPN = 'Z:\Active\morganLab\DATA\KxR_P11LGN\CellNav_KxR\Volumes\HighRes2023\Merge\'

load([MPN 'obI.mat'])
load([MPN 'dsObj.mat'])

dsamp = [1 1 1];
cellSub = obj2conBranch(obI,dsObj,sm.cid,0); %Do not separate branches [ , , ,0] if there could be overlapping segmentations labeled 'brh'
...
if ~isempty(cellSub)
    ...
    nep.edges = edges(goodEdge,:);
    nep.edgeRad  = arbor.edgeProps.rad(goodEdge) * arbor.voxSize(1);
