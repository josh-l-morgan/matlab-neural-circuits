...
if exist('binder','var')
    ...
    if exist([MPN 'dat.mat'],'file'),
        ...
        try
            ...
            disp('failed to parse Google Dat')
            dat.cid = [];
        end
    else
        dat.cid = [];
...
%% Cell types
type.typeNames = {'rgc' 'tcr' 'lin' 'unk','phot','hrz','bpc','amc','mul','glia','ctx','rtn','ach','oli','ast'};
...
%% cell Subtypes
for i = 1:length(type.typeNames),type.subTypeNames{i} = {};end
type.subTypeNames{8} = {'vgc'};
type.subTypeNames{7} = {'bc1' 'bc2' 'bc3a' 'bc3b' 'bc4' 'bc5i' 'bc5o' 'bc5t' 'bc6' 'bc7' 'bc8' 'bc11' 'bcunk' 'bon' 'boff'};
type.subTypeNames{1} = {'unk' 'on' 'off' 'on/off' '1ni' '1no' '1wt' '25' '27' '28'  '2an' '2aw' '2i' '2o' ...
...
%% resolve dat and tis types and subtypes

%%Add new names to type
tTypeNames = tis.cells.type.typeNames;
if isfield(dat,'typeNames')
dTypeNames = dat.typeNames;

for i = 1:length(dTypeNames)
   targ = find(strcmp(tTypeNames,dTypeNames{i}),1);
   if ~isempty(targ)
       d2tTypeNames(i) = targ;
   else
       tTypeNames = cat(2,tTypeNames,dTypeNames(i));
       %tis.cells.type.subTypeNames{i} = {};
       d2tTypeNames(i) = length(tTypeNames);
   end
end

dType = dat.type;
dType(dType>0) = d2tTypeNames(dType(dType>0));
dat.type = dType;
tis.cells.type.typeNames = tTypeNames;

%%Add New names to subType and change dat subtype IDs

for t = 1:length(tTypeNames);
    
    if t>length(tis.cells.type.subTypeNames)
        tis.cells.type.subTypeNames{t} = {};
    end
    tSubTypeNames = tis.cells.type.subTypeNames{t};
    dSubTypeNames = dat.subTypeNames{t};
    
    for i = 1:length(dSubTypeNames)
        targ = find(strcmp(tSubTypeNames,dSubTypeNames{i}),1);
        if ~isempty(targ)
            d2tSubTypeNames(i) = targ;
        else
            tSubTypeNames = cat(2,tSubTypeNames,dSubTypeNames(i));
            d2tSubTypeNames(i) = length(tSubTypeNames);
        end
    end
    
    dType = dat.type;
    dSubType = dat.subType;
    dChange = (dType==t) & (dSubType>0);
    dSubType(dChange) = d2tSubTypeNames(dSubType(dChange));
    dat.subType = dSubType;
    
    tis.cells.type.subTypeNames{t} = tSubTypeNames;
end
...
%% Define synapse type by synapse tag
for o = 1:length(tis.syn.obID)
     nam = obI.nameProps.names{tis.syn.obID(o)};
     if sum(regexp(nam,'syn'))
         tis.syn.edgeTag{o} = 'syn';
     elseif sum(regexp(nam,'rib'))
         tis.syn.edgeTag{o}  = 'rib';
     elseif sum(regexp(nam,'junc'))
         tis.syn.edgeTag{o}  = 'junk';
     end
end
