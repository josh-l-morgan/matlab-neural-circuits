...
%%Check confidence coeficient
means = mean(cis,3);
predDif = repmat(aPrioriPrecision',[1,reps]);
predHits = abs(means)<=predDif;
predPerform = mean(predHits,2);
...
s3.XLim = [min(tickRange) max(tickRange)];
    ...
    plot([0 pnum+1],[0 0],'color',[0 0 0],'LineStyle','- -')
    ...
    for p = 1:pnum
        ...
        g = randn(ns(p),1);
        [a b c] = ttest(g);
        sw(p) = swarmchart(ax,g*0+p,g,80,'MarkerEdgeColor','none','markerfacecolor',[.2 .2 1]);
