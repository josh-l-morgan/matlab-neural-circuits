...
if ~isfield(param,'convertToPercent')
...
for td = 1:testNum
    ...
    if param.useMean % Generate distribution using randn normal distribution
        ...
        np = length(rPool);
        for r = 1:reps
            r1(:,r) = rPool(randperm(np,n1));
            r2(:,r) = rPool(randperm(np,n2)) + testDif(td);
        end
    end
...
res.standardDifferenceRange = [md-seDif*2 md+seDif*2];
res.pooledDifferenceError = seDif2;
res.pooledDifferenceRange = [md-seDif2*2 md+seDif2*2];
