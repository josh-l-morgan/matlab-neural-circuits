...
synDir = ['D:\LGNs1\Analysis\seedBouton\seedBoutons4\'];
...
col = [.5 .5 .5];
    ...
    try viewProps = rmfield( viewProps,'mask'), catch err, end
    showCellNames = cat(2,num2cell(showCell));

    viewProps.cellId = num2cell([allMembers allMembers(11)]);
...
%     col2 = [1 0 0; 0 1 1; 0 1 1; 0 1 1];
    ...
    image(uint8(I_axons))
     ...
     if 1
        rotDir = 'D:\LGNs1\Analysis\movies\fourSeeds\testDir11\'
...
image(uint8(cropSyn * 1)),pause(.01)
    ...
    %{
imwrite(uint8(showSyn),sprintf('%ssynPos_%s.png',synDir,'201glom_dim2'))
