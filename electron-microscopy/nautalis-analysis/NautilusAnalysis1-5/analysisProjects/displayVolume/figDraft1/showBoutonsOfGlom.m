...
synDir = ['D:\LGNs1\Analysis\seedBouton\seedBoutonsGlom201_dim1\'];
...
viewProps.viewWindow = [mid(2) - rad(1)  mid(1) - rad(2)  mid(3) - rad(3) ; ...
    ...
    lookupCol = [1 0 0; 0 1 0; 1 0 1; 1 1 0; 0 .5 1; 1 1 0];
    glomList = getList_glomID();
    for i = 1:size(lookupCol,1)
        
        isGlom = glomList(glomList(:,2) == i,1);
        for a = 1:length(isGlom);
            targ = find(showAx == isGlom(a));
            if ~isempty(targ)
                
                col2(targ,:) = lookupCol(i,:);
