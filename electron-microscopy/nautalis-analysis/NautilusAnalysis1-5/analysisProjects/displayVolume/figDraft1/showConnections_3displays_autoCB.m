...
MPN = 'D:\LGNs1\Segmentation\VAST\S8\S8_ds16\CellBodies\matOut_AutoCB\'

load([SPN 'cbObj.mat'])
clear dsObj
for i = 1:length(cbObj.PixelIdxList)
  [y x z] = ind2sub(cbObj.ImageSize, cbObj.PixelIdxList{i});
    dsObj(i).subs = [y x z];
end
...
if 1 % object IDs
    ...
    showCellNames =[1:length(cbObj.PixelIdxList)];
...
if 1
   rotDir = 'D:\LGNs1\Analysis\movies\autoCB\autoCBRot2\'
   if ~exist(rotDir,'dir'),mkdir(rotDir), end
   degs = [90];
   degs = [0:1:359];
   I_stack = showRotation(viewProps,degs,rotDir);
...
if 0
    ...
    image(uint8(I))
    %%
    %   imwrite(uint8(I),[cellPicDir 'autoCB_max.png'])
