...
showCell = [903 907 109 909 919 194 106 129 123];
showCell = [1025        2035   127];%        127         268         266         267];
...
allPost = [conTo(:).tcrList];

pop{1} = intersect(conTo(1).rgcList,useClade);
pop{2} = intersect(conTo(2).rgcList,useClade);
pop{3} = intersect(conTo(3).rgcList,useClade);
pop{4} = intersect(conTo(4).rgcList,useClade);

colTable = [1 0 0; 0 1 0; ; 1 0 1; 0 1 1]*.3;
...
prePop{1} = [useClade];
postPop{1} =  [setdiff(setdiff(allPost,useClade),targCells)];
...
viewProps.viewWindow(1) = viewProps.viewWindow(1)*.5;
...
if 0
    ...
    getDeg = [0:10:180];
    clear I_rot;
    movieDir = 'D:\LGNs1\Analysis\movies\mixedClade\rot90_test\';
    mkdir(movieDir)
    saveFile = 0;
    c = 0;
...
%{
imwrite(uint8(showSyn*.2),[cellPicDir 'clade1_TCRDim2.png'])
%}
