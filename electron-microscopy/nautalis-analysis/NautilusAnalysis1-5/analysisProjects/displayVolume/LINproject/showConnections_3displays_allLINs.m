...
allEdges = obI.nameProps.edges
 ...
 linList = obI.cell.name(obI.nameProps.lin(obI.cell.mainObID))
 clear postLin preLin numPreLin numPostLin
for i = 1:length(linList)
    postLin{i} = preTo(allEdges,linList(i));
    preLin{i} = postTo(allEdges,linList(i));
    numPreLin(i) = length(preLin{i});
    numPostLin(i) = length(postLin{i});
end
...
postTarg = preTo(allEdges,pickLin);
preTarg = postTo(allEdges,pickLin);
%%Reference lists

%[postTarg] = getList_125rand;
showCell = [pickLin; postTarg(:,1)];
%showCell = linList

showCellNames = cat(2,num2cell(showCell));

%col = [1 1 1; repmat([1 0 0],[size(preTarg,1) 1]); repmat([0 1 0],[size(postTarg,1) 1])];
col = [10 10 10;colMap(randperm(size(colMap,1),length(showCell-1)),:)]
...
if 1
    ...
    if 0
        rotDir = 'D:\LGNs1\Analysis\movies\LIN\rand60from125d\'
    ...
    if 0
        ...
        image(uint8(I))
        %%
        %  imwrite(uint8(I),[cellPicDir '204pre.png'])
       %% 
    end
    ...
    %{
imwrite(uint8(showSyn),[cellPicDir '204pre.png'])
    %}
