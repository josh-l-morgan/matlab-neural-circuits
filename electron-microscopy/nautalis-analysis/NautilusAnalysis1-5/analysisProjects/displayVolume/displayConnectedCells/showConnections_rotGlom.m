...
showCell = [conTo(1).preLIN conTo(2).preLIN conTo(1).postLIN conTo(2).postLIN  ];
...
record = 1;
frameDir = 'C:\Users\joshm\Documents\myWork\myPublications\LGNs1\figures\movies\rotation\lin2\';
if ~exist(frameDir,'dir'),mkdir(frameDir),end
while 1
    f = 0;
    for d = 1:length(I_rot)
        image(uint8(I_rot{d}*1)),pause(.001)
        if record 
            f = f+1;
            imageName = sprintf('%sframe%3.0f.png',frameDir,f);
            imwrite(uint8(I_rot{d}*1),imageName);
        end
