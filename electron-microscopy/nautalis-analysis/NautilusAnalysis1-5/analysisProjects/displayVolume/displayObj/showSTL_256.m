...
trackProgress = cell(length(uCellId),1);
nothing = [];



parfor i = 1:length(uCellId)
    ...
    if ~exist(fileNameOBJ,'file')
        ...
        if ~isempty(allSub)
            ...
            fileNameNot = sprintf('%s%08.0f.mat',objDir,i);
            saveNothing(fileNameNot)
...
%             end
  ...
  %  val = get(prog,'UserData')
%     val = val+1
%     set(prog,'UserData',val)
%     
%     i
%     trackProgress{i} = 1;
%     trackProgress{i}
    %disp(sprintf('%d of %d meshed',sum(trackProgress),length(trackProgress)))
end
...
for i = 1:256;
   fileNameOBJ = sprintf('%s%03.0f.obj',objDir,i);
   mergeOb = find(viewProps.ob256==i);
   f = []; v = [];vcount = 0;
   for m = 1:length(mergeOb)
       fv = allFV{mergeOb(m)}
       f = cat(1,f,fv.faces);
       f = f + vcount;
       v = cat(1,v,fv.vertices);
       vcount = vcount+size(v,1);
   end
   if ~isempty(mergeOb)
    vertface2objmtl(v,f,fileNameOBJ,num2str(i),col(mergeOb(1),:));
   end
   disp(sprintf('saved %d of 256',i))
end
