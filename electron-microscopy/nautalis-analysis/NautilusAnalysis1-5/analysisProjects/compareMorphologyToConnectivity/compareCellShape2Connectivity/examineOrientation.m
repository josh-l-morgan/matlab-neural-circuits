...
for i = 1:length(cellList)
    ...
    if (size(rawObjectSubs,1)>100) & (~isempty(rawSeed))
        ...
        if pass
            ...
            lookRange = [5 25];
            [cellOri pc] = princompSkeletons(cellStruct.arbor,seedSub,lookRange);
            pause(.1)
...
%% get ori
firstOri = zeros(length(cellArbors),3);
clear oris
for i = 1:length(cellArbors)
    ...
    coef = cellArbors(i).pc.Coef;
    firstOri(i,:) = abs(coef(:,1))';
    oris(i,:,:) = abs(coef);
    
    for p = 1:3
        maxOri(i,p) = find(coef(:,p) == max(coef(:,p)));
    end
...
histOriBin = [0:.03:1];
histOri = hist(firstOri(:,3),histOriBin);
bar(histOriBin,histOri,'k')

mean(firstOri(:



histOri = hist(oris(:,2,2),histOriBin);
bar(histOriBin,histOri,'k')


cellList(find(firstOri(:,3)<.5))

oriMatch = cat(2,oris(:,1,3), oris(:,2,2), oris(:,3,1));
mean(oriMatch,1)
std(oriMatch,1)
...
epsName = 'D:\LGNs1\Analysis\dendriticMorphology\out5\labeledLatRatNumbered.eps'
print(gcf, epsName, '-depsc2','-painters','-r1200')
pngName = 'D:\LGNs1\Analysis\dendriticMorphology\out5\labeledLatRat.png'
