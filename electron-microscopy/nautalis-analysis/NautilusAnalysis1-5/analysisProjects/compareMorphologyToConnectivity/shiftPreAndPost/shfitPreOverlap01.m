%%Observe how overlap changes when position of presynaptic cells are
%%shifted relative to post.  Copied code from filteredSkelOverlap07.m
...
for shifted = 1:length(shiftList)
    shiftX = shiftList(shifted);
    disp(sprintf('checking shift %d of %d',shifted,length(shiftList)))
    tic
...
%             shiftX = shiftList(shifted);
            ...
            parfor p = 1: length(cellList)
                ...
                pairDists = zeros(size(preNodes,1),size(postNodes,1));
                for n = 1:size(preNodes,1)
                    pairDists(n,:) = sqrt((postNodes(:,1)-preNodes(n,1)).^2 + (postNodes(:,2)-preNodes(n,2) + shiftX).^2 + ...
                        (postNodes(:,3)-preNodes(n,3)).^2);
                end
                
                histDists = histc(pairDists(:),histBin);
                shiftCellHists{i,p,shifted} = histDists;
...
skelOverlapPred.shiftCellHists = shiftCellHists;
