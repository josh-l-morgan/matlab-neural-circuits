...
postDists = sqrt((postAnchors(:,1)-targPos(1)).^2 + ...
    (postAnchors(:,2)-targPos(2)).^2 + ...
    (postAnchors(:,3)-targPos(3)).^2);
...
cb2net.postAnchors = postAnchors;
%}



%% Compare postDists to cbDists



%% look for overlap between post segmentations and cb labels
...
for post = 1: length(postList)
    ...
    dsSubs = round(scaleSubs(dsSubs,1./cbScale));
    ...
    if showOverlap
        ...
        middleX = mode(dsSubs(:,2));
        middleY = mode(dsSubs(:,1));
        rangeYX = 100;
        
        sampCell = double(viewVol(max(1,middleY-rangeYX):middleY+rangeYX,max(1,middleX-rangeYX):middleX+rangeYX,max(1,middleZ-rangeZ):min(dsMaxSub(3),middleZ + rangeZ)));
        sampCB =  double(cbObj.vol(max(1,middleY-rangeYX):middleY+rangeYX,max(1,middleX-rangeYX):middleX+rangeYX,max(1,middleZ-rangeZ):min(dsMaxSub(3),middleZ + rangeZ))>0);
        clear colSamp
...
cbDat = cbObj;
cbDat.cbID = cbID;
cbDat.cbPos = cbCenters;
save([SPN 'cbDat.mat'],'cbDat')
