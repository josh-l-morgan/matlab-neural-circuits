...
useList = obI2cellList_allLabels(obI,targCell);
...
props = regionprops(cbObj,'centroid','area');
rawCentroids= cat(1,props.Centroid); 
cent2UM =  [128 * .0046 128 * .004    16 * .030]
centroids(:,1) = rawCentroids(:,2) * cent2UM(1);
centroids(:,2) = rawCentroids(:,1) * cent2UM(2);
centroids(:,3) = rawCentroids(:,3) * cent2UM(3);
centroids = round(centroids);

distMat  = zeros(size(postAnchors,1),size(centroids,1));
xMat = zeros(size(postAnchors,1),1); yMat = xMat; zMat = xMat;


maxAll = ceil(max(cat(1,centroids,postAnchors),[],1));
centVol = zeros(maxAll);
ancVol = zeros(maxAll);

centVol(sub2ind(maxAll,centroids(:,1),centroids(:,2),centroids(:,3))) = 1;
ancVol(sub2ind(maxAll,round(postAnchors(:,1)),round(postAnchors(:,2)),round(postAnchors(:,3)))) = 1;

clear showCol
showCol(:,:,1) = sum(centVol,3) * 300;
showCol(:,:,2) = sum(ancVol,3) *300;
showCol(:,:,3) = 0;
image(uint8(showCol))


for i = 1:size(postAnchors,1)
   distVec =      sqrt((centroids(:,1)-postAnchors(i,1)).^2 + ...
    (centroids(:,2)-postAnchors(i,2)).^2 + ...
    (centroids(:,3)-postAnchors(i,3)).^2)'; 
  distMat(i,:) = distVec; 
  
  minTarg = find(distVec == min(distVec),1);
  
  xMat(i) =   (centroids(minTarg,1)-postAnchors(i,1));
  yMat(i) = (centroids(minTarg,2)-postAnchors(i,2));
  zMat(i) =  (centroids(minTarg,3)-postAnchors(i,3));


end

minList = min(distMat,[],2);




checkMat = distMat;
anc2CB = zeros(size(postAnchors,1),1);
cbID = zeros(size(centroids,1),1);
for i = 1:size(postAnchors,1)
    [targY targX] = find(checkMat == min(checkMat(:)),1);
    anc2CB(targY) = targX;
    cbID(targX) = targY;
    anc2CBdist(targY) = distMat(targY, targX);
    checkMat(targY,:) = 100000;
    checkMat(:,targX) = 100000;
    
end

minList = min(distMat,[],2);

traceCB.cellNames = postList;
traceCB.postAnchors = postAnchors;
traceCB.CBcentroids = centroids;
traceCB.cbID = cbID;
traceCB.anc2CB = anc2CB;
traceCB.anc2CBdist = anc2CBdist;

save([SPN 'traceCB.mat'],'traceCB')
