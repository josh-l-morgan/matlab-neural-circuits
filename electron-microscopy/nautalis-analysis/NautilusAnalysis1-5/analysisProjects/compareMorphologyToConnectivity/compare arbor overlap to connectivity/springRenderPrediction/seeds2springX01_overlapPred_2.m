...
%     
%     springList = obI2cellList_seedInput_RGC_TCR(obI,seedList);
%     %seedPref = seedPreferences(seedList,springList);
%     %allEdges = obI.nameProps.edges(:,[2 1]);
%     
%     fileName = sprintf('%sskelOverlapPred.mat',MPN);
%     load(fileName,'skelOverlapPred')
%     springList = skelOverlapPred.springList;
%     
% end
...
con = springList.con;
syns = con2syn(con);
allEdges = [springList.preList(syns(:,1))' springList.postList(syns(:,2))']

springDir = 'D:\LGNs1\Analysis\springDat\modularityOverlap01\';
...
% allEdges = [];
% for i = 1:length(y)
%     newEdge = [springList.preList(y(i)) springList.postList(x(i))];
...
if 0 %zero seeds
    for i = 1:length(seedList)
        con2(:,find(springList.postList == seedList(i))) = 0;
    end
end
...
nodeIDs = [springList.preList(useOn) setdiff(springList.postList,seedList)];
nodeType = [springList.preList(useOn)*0+1 setdiff(springList.postList,seedList)*0+2];


nodeIDs = [springList.preList(useOn) (springList.postList(useIn))];
nodeType = [springList.preList(useOn)*0+1 (springList.postList(useIn))*0+2];
...
springDat = springParameters_X01_overlapPred(springIn);
...
for rerun = 1: 1
    ...
    if 0
        ...
        imageName = sprintf('%sspringRun_%03.0f_pred100.png',springDir,rerun);
        print(gcf,imageName,'-dpng','-r1024','-opengl','-noui')
        
        epsName = sprintf('%sspringRun_%03.0f_pred100.eps',springDir,rerun);
