...
for i = 1:length(axSkel)
    ...
    for b = 1:length(skel.bones);
        ...
        for e = 1:size(bone.edges,1);
            ...
            L = L + edgeLength;
            lengths(e) = edgeLength;
            mids(e,:) = mean(edgeNodes,1);
        end
        disp(L)
        boneLengths(b) = L;
        if L>minLength
            keepNodes = cat(2,keepNodes,bone.nodes);
            allMids = cat(1,allMids,mids);
            allLengths = cat(2,allLengths,lengths);
        end
    end
...
for i = 1:length(axList)
    ...
    for p = 1: length(cellList)
        ...
        for m = 1:length(minDist)
        closeEnough = pairDists<=minDist(m);
        passPre = sum(closeEnough,2);
        passPost = sum(closeEnough,1);
        
        axOverlap(i,p,m) = sum(axLengths{i}(find(passPre)));
        cellOverlap(i,p,m) = sum(cellLengths{p}(find(passPost)));
        end
...
axCellLength = axOverlap+cellOverlap;
...
skelOverlapPred.minDist = minDist;
skelOverlapPred.axOverlap = axOverlap;
skelOverlapPred.cellOverlap = cellOverlap;
skelOverlapPred.axCellLength = axCellLength;
