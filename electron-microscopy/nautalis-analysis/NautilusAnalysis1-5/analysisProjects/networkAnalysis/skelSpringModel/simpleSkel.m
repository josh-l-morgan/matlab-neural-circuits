function[skel] = simpleSkel(skelin,mergeSize);
%%
...
skel.offset = skelin.offset ;
skel.fsize = skelin.fsize;
skel.surfSeed = skelin.surfSeed;
...
tips = [bones.tip];
bases = [bones.base];
fixedNode = zeros(size(skelin.node2surf));
fixedNode(tips) = tips;
fixedNode(bases) = bases;

for b = 1:length(bones);
    L(b) = sum(bones(b).edgeLengths);
end
%useBones = L>(mergeSize*.3);
useBones = L>0;

useBones = find(useBones);
...
%     newBones(b).rawNodes = nodeIDlist;
    ...
    for r = 1:numRaw
        ...
        if makeNode
            ...
            heldSubs = skelin.node2subs(holdVox,:);
    ...
    baseSub = skelin.node2subs(oldBone.base,:);
...
skel.node2subs = skelin.node2subs(node2surf,:);
