...
springDir = 'D:\LGNs1\Analysis\springDat1dim\springConGraph2\'
...
seedCol = [1 0 0; 0 1 0;  0 0 1; 0 .3 1; 0 .3 1; .5 .5 0 ; .4 .4 .4; .4 .4 .4] ...
    * .6; 

nodeCol = zeros(nodeNum,3);
for s = 1:length(seedList)
    useCol = seedCol(s,:);
    try nodeCol(find(nodeIDs==seedList(s)),:) = seedCol(s,:);,end
    for n = 1:nodeNum
...
springDat = springParameters_1Da(springIn);
...
for rerun = 1: 1
    ...
    allResults{rerun} = runSprings1D(springDat);
