function[results] = runSprings(springDat)
    ...
    targY = nodes.type' * 0;
    targY(nodes.type == 1) = fsize * .8;
    targY(nodes.type == 2) = fsize * .4;
    targY(nodes.type == 3) = fsize * .8;
    targY(nodes.type == 4) = fsize * .4;
...
%     preIDs = useList.preList;
    ...
    for r = 1:reps
        ...
        netRad = fsize/2;
        centerForce(abs(centDifX)>(netRad)) =  centerForce(abs(centDifX)>(netRad))*10000;
        centerForce(abs(centDifY)>(netRad)) =  centerForce(abs(centDifY)>(netRad))*10000;
