...
cladeNum = 6;
%members = getList_erosionGlom(cladeNum);

members = getList_cladePicks(cladeNum);
% [nodeIDs idx] = intersect(nodeIDs,members);
...
if 1
   ...
   [cellList cellProps] = getList_nodeXYcol; 
   for i = 1:length(cellList)
       targ = find(nodeIDs == cellList(i));
       if ~isempty(targ)
          nodeCol(targ,:) = cellProps(i,:); 
...
springDat = springParameters_X01_figDraftBigPropSmall(springIn);
...
itag = sprintf('posYX_sixCladeRemodB%d',cladeNum);
 imageName = sprintf('%sspringRun_%03.0f.png',springDir,rerun);
    imageName = sprintf('%s%s.png',springDir,itag);
    %print(gcf,imageName,'-dpng','-r1024','-opengl','-noui')
