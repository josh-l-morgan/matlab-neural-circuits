...
if loadData
    ...
    allJuncs = obI.nameProps.juncs(:,[1 2]);
end

springDir = 'D:\LGNs1\Analysis\springDat\highlightGiants4\'
...
juncCells = unique(allJuncs((allJuncs>0)&(allJuncs<1000)));
...
for i = 1:length(nodeIDs)
    isPost = preTo(allJuncs,nodeIDs(i))
    if ~isempty(isPost)
        juncTo  = [juncTo isPost(:,1)'];
    end
    
    isPre = preTo(allJuncs,nodeIDs(i))
    if ~isempty(isPost)
        juncTo  = [juncTo isPre(:,1)'];
    end
end

juncTo = unique(juncTo(juncTo>0));


otherCells = setdiff(juncTo,nodeIDs);
nodeIDs = [nodeIDs otherCells];
nodeType = [nodeType otherCells*0+3];
...
springIn.allJuncs = allJuncs;
springDat = springParameters_fiveSeedsJuncs(springIn);
