...
springDir = 'D:\LGNs1\Analysis\springDat\highlightGiants3\'
...
labelCells = mtaCount(:,1);
for i = 1:length(labelCells)
    
    targ = find(nodeIDs==labelCells(i));
    if ~isempty(targ)
   nodeCol(targ,:) = [1 1 1] * mtaCount(i,2)>0;%nodeCol(targ,:) + [.5 * mtaCount(i,2) 0 .5];  
   nodeType(targ) = 3;
    end
end
...
springDat = springParameters_fiveSeedsMTA(springIn);
