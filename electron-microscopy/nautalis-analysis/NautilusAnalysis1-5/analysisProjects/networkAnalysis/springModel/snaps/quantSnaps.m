...
%% Run rendered snapping
%load('D:\LGNs1\Analysis\springDat\results\springRender_10.mat')
...
groupNum = max(cellGroups,[],2);

histBinGN = 1:50;
clear groupNumGreat1
for i = 1:size(cellGroups,1)
    
   stepHist = hist(cellGroups(i,:),histBinGN) ;
   groupNumGreat1(i) = sum(stepHist>1); 
   
end
groupNum = groupNumGreat1;

groupNum = groupNum - min(groupNum)+1;
subplot(2,1,1)
plot(groupNum)

subplot(2,1,2)
histBinGN = 1:max(groupNum);
histGN = hist(groupNum,histBinGN);
bar(histBinGN(1:20),histGN(1:20))
...
%checkIDs = checkIDs(randperm(length(checkIDs)));
[checkIDs checkProp]  = getList_cbArea(MPN);
[checkIDs checkProp]  = getList_axSeedCon();

[checkIDs checkProp] = getList_prinCompRats;

[checkIDs checkProp] = getList_pcLatRats;
checkProp = checkProp(:,1);

[checkIDs checkProp] = getList_meanDiam;
    [checkIDs checkProp] = getList_sholl;
    ...
    seedList = [ 108 201 109 903 907];
    useList = obI2cellList_seedInput_RGC_TCR(obI,seedList);
   [checkProp] = getList_seedColor(seedList,nodeOrder);
    checkIDs = nodeOrder;
...
useCol = zeros(length(nodeOrder),1);
    for n = 1:length(nodeOrder)
       ...
       if sum(useList.preList == nodeOrder(n))
           targ = [];
       end
       if isempty(targ)
           nodeCol(n,:) = [0 0 0];
