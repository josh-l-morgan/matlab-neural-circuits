...
for r = 1:reps
    ...
    useK = k(ceil(r/reps*length(k))); %% select noise epoch
    springForce =  dists .* con * useK;
    ...
    useDispserse = disperse(ceil(r/reps*length(disperse)));
    if r>=startRepulse
        repulsion = (dists<minDist) * repulse + (fsize./dists) * useDispserse/nodeNum;
    else
        repulsion = (fsize./dists) * useDispserse/nodeNum;
    end
