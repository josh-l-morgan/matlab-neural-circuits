function[] = viewManyCells(MPN,cellList);
...
I_preTargCell = showCellTop(obI,dsObj,cellId,col,Dim,fsize);
I_preTargCellSum = showCellSum(obI,dsObj,cellId,col,Dim,fsize);

sumDir = [MPN 'sumImages\rgc\'];
%cells2Dir(obI,dsObj,cellId,Dim,sumDir);
image(I_preTargCell*1.5)


%I_targCell = showCell(obI,dsObj,targCell,[1,1,1],Dim);
I_targCellSum = showCellSum(obI,dsObj,targCell,[1,1,1],Dim,fsize);
...
I_partCell = showCellTop(obI,dsObj,partCells,col,Dim,fsize);
I_partCellSum = showCellSum(obI,dsObj,partCells,col,Dim,fsize);
