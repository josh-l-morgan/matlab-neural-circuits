function[] = tif2point(SPN,planeStep)
    ...
    if 1 %~exist([TPN 'parseTiles.mat'],'file')
        ...
        parfor i = 1:length(imageNames)
            ...
            if isempty(yPos)
                ...
                parseTiles(i).z = str2double(nam(sPos(end)+2:dotPos(end)-1));
                parseTiles(i).col = 0;
                parseTiles(i).row = 0;
                ...
                parseTiles(i).name = nam;
                parseTiles(i).z = str2double(nam(sPos(end)+2:yPos(end)-1));
                parseTiles(i).col = str2double(nam(xPos(end)+2:dotPos(end)-1));
                parseTiles(i).row = str2double(nam(yPos(end)+2:xPos(end)-1));
...
for plane = 1:planeStep :numPlanes
