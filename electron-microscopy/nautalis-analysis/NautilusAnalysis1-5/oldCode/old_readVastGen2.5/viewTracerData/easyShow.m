function[Ifull] = easyShow(MPN,viewType);
...
%%
boarder = 20;
Ifull = zeros(fsize(1) + fsize(3)+boarder, fsize(2) + fsize(1)+boarder,3,'uint8')+256;
Ifull(fsize(3) + 1+boarder:fsize(3) + fsize(1)+boarder,1:fsize(2),:) = Iall{3};
Ifull(1:fsize(3),1:fsize(2),:) = Iall{1};
Ifull(1:fsize(3),fsize(2)+1+boarder:fsize(2)+fsize(1)+boarder,:) = Iall{2};
