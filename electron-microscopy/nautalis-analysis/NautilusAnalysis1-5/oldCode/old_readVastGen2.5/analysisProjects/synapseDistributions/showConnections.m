...
showCell = [108]
showCellNames = cat(2,num2cell(showCell),'jm frag 203');
...
%% labeled synapses
prePop{2} = [obI.cell.name];
postPop{2} =  [];

prePop{3} = [1000:2000];
postPop{3} =  [];

prePop{1} = [obI.cell.name];
postPop{1} =  [];
...
showSyn(~maskSyn) = I_topSum(~maskSyn)/5;
rotSyn = imrotate(showSyn,pi);
image(uint8(rotSyn*1))

%% Draw cell bodies and connections


%{
imwrite(uint8(showSyn),[cellPicDir '601view2.png'])
%}
