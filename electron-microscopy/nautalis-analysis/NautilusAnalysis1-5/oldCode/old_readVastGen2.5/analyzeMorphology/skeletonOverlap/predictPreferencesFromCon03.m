...
%% bar preferences
clf
conBin = [min(synPref(isPrefInfo)): .5 :max(synPref(isPrefInfo))+1];

histSame = histc(sameSeedPref,conBin)/length(sameSeedPref);
histDif = histc(difSeedPref,conBin)/length(difSeedPref);
...
clf
conBin = [min(con(isPrefInfo)):max(con(isPrefInfo))+1];

scatter(sameSeedPred,sameSeedCon,'r','filled')
hold on
scatter(difSeedPred,difSeedCon,'b','filled')
plot([0 10],[0 10],'k')
