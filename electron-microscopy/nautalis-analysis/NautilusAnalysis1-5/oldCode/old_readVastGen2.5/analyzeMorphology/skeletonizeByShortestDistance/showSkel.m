...
predIm  = zeros(skel.minMax{2}(dims(1)),skel.minMax{2}(dims(2)));
...
for t = 1:length(skel.tips)
    ...
    for r = 1:length(skel.nodes)
        ...
        tSub = subs(tip,:);
        predIm(tSub(dims(1)),tSub(dims(2))) = 1;% predIm(tSub(1),tSub(2)) + 1;
...
col(:,:,2) = edgeIm + bridgeIm - predIm/2;
col(:,:,1) = predIm;
col(:,:,3) = predIm*0;
