...
%% Get obj surface
    ...
    %% Consolidate paths
    %[surfSkel] = shortenAllPaths(surfVox);
    %centralPath = getCentralPaths(surfVox);
%     [skel] = shortenAllPaths(surfVox);
%     nodeCount = skel.numNodes;
...
%%
   ...
   showMaxProp(moveObj,nodeCount(manyNode.vox));
    ...
    longTipReps = 5;
    disp('Find long tips')
%     [manyNode] = countMaxAndDist(surfVox,skel.numNodes,longTipReps);
%     manyNodeness = manyNode.prop;
    ...
    %tipProp = tipLengths>(longTipReps*5);
    %showMaxProp(moveObj,tipProp*400  + (longTipness>0)*20);
    showMaxProp(moveObj,tipProp*1000  + 20);
 %}
...
%     %Find bones for every voxel
%     [surfSkel] = shortenPath(surfVox);
%     
% %     
% %     [surfSkel] = selectBonesForSkel(surfSkel);
% %     surfVox.surfSkel = surfSkel;
%     
%     
...
%     showMaxProp(moveObj,mod(surfSkel.prop,256));
%     showMaxProp(moveObj,nodeCount+20);
% 
