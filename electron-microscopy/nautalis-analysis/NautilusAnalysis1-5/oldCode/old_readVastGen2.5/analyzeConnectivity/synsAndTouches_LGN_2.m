...
synMat = gridArbor.synMat;
touchMat = gridArbor.touchMat;

load([TPN 'gridSubs.mat'],'gridSubs')
synMat = gridSubs.synMat;
touchMat = gridSubs.touchMat;
spineparentids = 1:size(touchMat,1);

scatter(touchMat(:),synMat(:))
...
%         combIDs = y* max(dendVal*2) + dendVal;
        ...
        numUniques = sum(makeSyn(:)>0);
        randUniques(r) = numUniques;
        randMults(r) = totSyn-numUniques;
