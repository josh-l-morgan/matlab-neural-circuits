classdef VASTControlClass < handle
  ...
  %%%% VERSION: March 18, 2015 - for VAST Lite RC2+
  ...
  methods
    ...
    function [layerinfo, res] = getlayerinfo(obj, layernr)
      ...
      res=processerror(obj);
      if (res==1)
        if (obj.nrinints==7)&&(obj.nrinuints==1)&&(obj.nrindoubles==3)
    ...
    function [segdata, res] = getallsegmentdata(obj)
      ...
      while (writepos<expectedlength) %(max(size(indata))<expectedlength)
        ...
        %pause(0.01);
        %disp(sprintf('%d of %d ..',writepos,expectedlength));
      end;
      ...
      if (res==1)
        ...
        for i=1:1:uid(1)
          segdata{i}.id=i-1;
          segdata{i}.flags=uid(sp); %obj.inuintdata(2);
          segdata{i}.col1=uid(sp+1); %obj.inuintdata(3);
          segdata{i}.col2=uid(sp+2); %obj.inuintdata(4);
          segdata{i}.anchorpoint=id(sp+3:sp+5); %obj.inintdata(1:3); x,y,z
          segdata{i}.hierarchy=uid(sp+6:sp+9); %obj.inuintdata(5:8); parent,child,prev,next
          segdata{i}.collapsednr=uid(sp+10); %obj.inuintdata(9);
          segdata{i}.boundingbox=id(sp+11:sp+16); %obj.inintdata(4:9);  x1,y1,z1,x2,y2,z2
    ...
    function [segdatamatrix, res] = getallsegmentdatamatrix(obj)
      ...
      if (res==1)
        ...
        for i=1:1:uid(1)
          segdatamatrix(i,1)=i-1; %segdata{i}.id=i-1;
          segdatamatrix(i,2)=uid(sp); %segdata{i}.flags=uid(sp); %obj.inuintdata(2);
          segdatamatrix(i,3)=bitand(bitshift(uid(sp+1),-24),255); %bid((sp+1)*4); %segdata{i}.col1=uid(sp+1); %obj.inuintdata(3);
          segdatamatrix(i,4)=bitand(bitshift(uid(sp+1),-16),255); %bid((sp+1)*4+1);
          segdatamatrix(i,5)=bitand(bitshift(uid(sp+1),-8),255); %bid((sp+1)*4+2);
          segdatamatrix(i,6)=bitand(uid(sp+1),255); %bid((sp+1)*4+3);
          segdatamatrix(i,7)=bitand(bitshift(uid(sp+2),-24),255); %bid((sp+2)*4); %segdata{i}.col2=uid(sp+2); %obj.inuintdata(4);
          segdatamatrix(i,8)=bitand(bitshift(uid(sp+2),-16),255); %bid((sp+2)*4+1);
          segdatamatrix(i,9)=bitand(bitshift(uid(sp+2),-8),255); %bid((sp+2)*4+2);
          segdatamatrix(i,10)=bitand(uid(sp+2),255); %bid((sp+2)*4+3);
          segdatamatrix(i,11:13)=id(sp+3:sp+5); %segdata{i}.anchorpoint=id(sp+3:sp+5); %obj.inintdata(1:3); x,y,z
          segdatamatrix(i,14:17)=uid(sp+6:sp+9); %segdata{i}.hierarchy=uid(sp+6:sp+9); %obj.inuintdata(5:8); parent,child,prev,next
          segdatamatrix(i,18)=uid(sp+10); %segdata{i}.collapsednr=uid(sp+10); %obj.inuintdata(9);
          segdatamatrix(i,19:24)=id(sp+11:sp+16); %segdata{i}.boundingbox=id(sp+11:sp+16); %obj.inintdata(4:9);  x1,y1,z1,x2,y2,z2
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % GETSEGIMAGERAW = 20;
    function [segimage, res] = getsegimageraw(obj, miplevel,minx,maxx,miny,maxy,minz,maxz)
    ...
    function [segimage,res] = getsegimageRLEdecoded(obj, miplevel,minx,maxx,miny,maxy,minz,maxz,surfonlyflag)
      ...
      if (res==0)
        ...
        %Decode RLE
        %segimage=uint16(zeros((maxx-minx+1)*(maxy-miny+1)*(maxz-minz+1),1));
...
%         na(sva+1)=na(sva+1)+double(sna)';
    ...
    function [segimage,values,numbers,res] = getsegimageRLEdecodedcountunique(obj, miplevel,minx,maxx,miny,maxy,minz,maxz,surfonlyflag)
    ...
    function [segimage,values,numbers,bboxes,res] = getsegimageRLEdecodedbboxes(obj, miplevel,minx,maxx,miny,maxy,minz,maxz,surfonlyflag)
      %tic
      [segimageRLE,res] = getsegimageRLE(obj, miplevel,minx,maxx,miny,maxy,minz,maxz,surfonlyflag);
    ...
    function [emimage,res] = getemimage(obj, layernr,miplevel,minx,maxx,miny,maxy,minz,maxz)
      ...
      if (res==1)
        if (size(emimageraw,2)==(maxx-minx+1)*(maxy-miny+1)*(maxz-minz+1))
          %One byte per pixel
          if (minz==maxz)
            emimage=permute(reshape(emimageraw,maxx-minx+1,maxy-miny+1),[2 1]);
          else
            emimage=permute(reshape(emimageraw,maxx-minx+1,maxy-miny+1,maxz-minz+1),[2 1 3]);
          end;
        else
          %Three bytes per pixel
          if (minz==maxz)
            emimage=flipdim(permute(reshape(emimageraw,3,maxx-minx+1,maxy-miny+1),[3 2 1]),3);
          else
            emimage=flipdim(permute(reshape(emimageraw,3,maxx-minx+1,maxy-miny+1,maxz-minz+1),[3 2 4 1]),4);
          end;
        end;
      else
        emimage=emimageraw;
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [screenshotimage,res] = getscreenshotimage(obj, miplevel,minx,maxx,miny,maxy,minz,maxz,collapseseg)
      ...
      if (res==1)
        if (minz==maxz)
          screenshotimage=flipdim(permute(reshape(screenshotimageraw,3,maxx-minx+1,maxy-miny+1),[3 2 1]),3);
        else
          screenshotimage=flipdim(permute(reshape(screenshotimageraw,3,maxx-minx+1,maxy-miny+1,maxz-minz+1),[3 2 4 1]),4);
        end;
      else
...
%       end;
    ...
    function parse(obj,indata)
      ...
      obj.inintdata=int32(obj.inintdata); %typecast(obj.inintdata,'int32');
      obj.inuintdata=uint32(obj.inuintdata); %typecast(obj.inuintdata,'uint32');
    end;
  end;
end