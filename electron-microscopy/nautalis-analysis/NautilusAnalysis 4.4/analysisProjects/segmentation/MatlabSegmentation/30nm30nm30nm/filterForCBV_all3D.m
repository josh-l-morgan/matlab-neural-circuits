...
TPN = 'D:\LGNs1\testAlign\s8Sample_filt4_all3DCB_testCompressionPNG\';
mkdir(TPN)
...
skern = gaus3d([100 100 100],1);
ckern = gaus3d([100 100 100],5);
hkern = ckern-skern;
...
for i = 1:size(ratCB,3)
    newName = sprintf('CB_%05.0f.png',i)
   %imwrite(uint16(labCB(:,:,i)),[TPN newName],'bitdepth',16)
       imwrite(uint8(labCB(:,:,i)),[TPN newName],'Compression','none')
