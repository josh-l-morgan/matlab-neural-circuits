...
%% Edge
subplot(1,2,1)
gKern1 = gaus3d([5 5 1],2);
c1 = fastCon(I,gKern1);
image(fitH(c1))
threshs= [.5:.1:.5]; % thresolds to check for edges.  later scaled
sizes = .5:.5:.5;  % sizes to check for edges
%eI = sumEdge(c1,threshs,sizes,0); % robustish edge detection

eI = edge(c1,'Canny',[.01 .3],1);
