function[p] = findClosestesPropInNep(nep,prop,aveReach);
...
for c = 1:length(cellNodes)
    ...
    for s = 1:length(synTarg)
        ...
        prop(skelTarg,:) = repmat(cellProp,[length(skelTarg) 1]); %add prop to skel
        useProp(skelTarg) = 1; %mark skel as having a prop
    end
end
...
%     hold on
   %disp(sprintf('checking %d of %d skel nodes',i,length(skelNodes)))
    start = skelNodes(i);
...
for i = 1:length(skelNodes)
    ...
    if isempty(sampNodes)
        ...
        p.closest(skelNodes(i)) = sampNodes(find(sampDists == min(sampDists),1));
    end
