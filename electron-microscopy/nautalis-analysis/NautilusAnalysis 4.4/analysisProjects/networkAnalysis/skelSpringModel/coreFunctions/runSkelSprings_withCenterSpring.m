...
elseif param.usePos
    ...
    nodeX(nodes.usePos>0) = nodePos(:,1)*fRat;
    nodeY(nodes.usePos>0) = fsize - nodePos(:,2)*fRat;
...
kern1 = tent((fsize*fres)*2+1)*(1/sum(pushes));
kern2 = tent(5*fres)*1;
kern = addKern(kern1,kern2);
...
%     
    ...
    if r>=startRepulse
        ...
        %fieldG = fastCon(fieldDis,kern1);
                fieldG = convn(fieldDis,kern1,'same');
    ...
    sumPushX = nodeGx .* isPushed *  -disperse ;%
    sumPushY = nodeGy .* isPushed *  -disperse ;
...
%     
    ...
    nodeXV = (nodeXV + sumPullX + sumPushX ); %% add forces
    nodeYV = (nodeYV + sumPullY+ sumPushY );
    ...
    showFieldG = uint8(cat(3,fieldG,fieldDis*1000,fieldG));
    image(showFieldG)
    pause(.1)
    %%Display
    ...
    toc
        ...
        for g = 1:groupNum
            ...
            sg = scatter(nodeX(group(g).ind),nodeY(group(g).ind),group(g).size,group(g).ind,...
                 group(g).marker,'filled');
