function[nep] = colorNep_minDist(nep,obI)
...
if 1 %find density
    ...
    p = findClosestesPropInNep(nep,prop,aveDist);
    
    isClose = p.closest>0;
    pos1 = nep.nodePos(isClose,:);
    pos2 = nep.nodePos(p.closest(isClose),:);
    
    dists = sqrt((pos1(:,1)-pos2(:,1)).^2 + (pos1(:,2)-pos2(:,2)).^2 + ...
        (pos1(:,3)-pos2(:,3)).^2);
    
    scatter(dists,p.minDist(isClose),'.')
    hold on
    plot([min(dists) max(dists)],[min(dists) max(dists)])
    hold off
    
    closeVals = p.minDist(isClose);
    closerRange = [0:5:100];
    histClose = hist(closeVals,closerRange);
    ratClose = histClose/sum(histClose)
    sum(ratClose(1:10))
    sum(ratClose(1:4))
    bar(closerRange,ratClose,'k')
...
if 1 %average of skel node prop
    nep.edgeCol(isSkel>0,1:3) = conCol(ceil(meanProp(isSkel>0)),:);
end
