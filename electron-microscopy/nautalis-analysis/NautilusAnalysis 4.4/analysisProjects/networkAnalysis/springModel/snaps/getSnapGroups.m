...
springDir = 'D:\LGNs1\Analysis\springDat\cellProperties\Snaps1\';
...
load([springRes 'res_snapLongNoSeedsAll_2.mat'])
...
if 1
    ...
    [colorList colPropRaw2] = seedMarkov(MPN);
    colPropRaw = colPropRaw2(:,1);

    colorList = nodeIDs(nodeType==2);
...
if 0
    nodeCol = nodeCol * 0;
   %[colorList2 colPropRaw2] = getAttributes(obI);
    [colorList2 colPropRaw2] = seedMarkov(MPN);
    ...
    for i = 1:length(nodeIDs)
        ...
        if ~isempty(targ)
            ...
            nodeCol(i,:) = colPropRaw2(targ,:);
            nodeType(i) = nodeType(i) + 2;
        end
    end
...
if 0
    ...
    for i = 1:length(labelCells)
        targ = find(nodeIDs==labelCells(i));
        nodeCol(targ,:) = nodeCol(find(nodeIDs==labelCells(i)),:) + .5;
