...
[checkIDs checkProp] = getList_meanDiam;
    ...
    load('D:\LGNs1\Analysis\nautDat\nodeIDCol_possitions.mat');
    checkIDs = nodeIDCol.IDs;
    cellProp = nodeIDCol.Colors;

%% Reshape color
% 
%     nodeProp = [];nodePropRef = [];
%     for i = 1:length(nodeIDs)
%         targ = find(checkIDs == nodeIDs(i));
%         if ~isempty(targ)
%             if length(targ)>1
%                 'too many targets'
%                 checkIDs(targ)
%             end
%             nodeCol = [nodeCol; checkProp(targ(1),:)];
...
%     
%     
%     sortVal = sort(abs(colProp(:)-50),'descend');
%     satVal = max(sortVal(round(length(sortVal)*.05)),1);
%     colMat = colorWheel2d(100,satVal);
%     for c = 1:size(colProp,1)
...
if 0
    ...
    itag  = 'rend10_position_glomSnap2';
