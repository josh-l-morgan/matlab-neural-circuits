...
%load('D:\LGNs1\Analysis\springDat\results\res_snapLongNo4Seeds3.mat')

% load('D:\LGNs1\Analysis\springDat\results\moreSpace_ALL.mat')
% load('D:\LGNs1\Analysis\springDat\results\moreSpace_ALL_lowNoise.mat')
...
nodeCol(find(nodeIDs == 130),:) = [1 1 1]
...
%{
save([springRes 'res_snapLong01_baseClade2.mat'],'cladeResults')
...
if 0
    ...
    clear members targ
    for i = 1:length(x)
        dists = sqrt((yPos-y(i)).^2 + (xPos-x(i)).^2);
        targ(i) = find(dists == min(dists));
        members{i} = cladedNodes{targ(i)};
    
    end
    cladePick.x = x;
    cladePick.y = y;
    cladePick.clades = targ;
    cladePick.members = members;
    members
    if 0
        save('.\data\clade\cladePick_six2.mat','cladePick')
    end
%%
end
