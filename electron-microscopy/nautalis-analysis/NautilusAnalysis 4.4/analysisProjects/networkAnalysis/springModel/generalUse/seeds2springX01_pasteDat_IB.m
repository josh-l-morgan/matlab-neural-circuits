...
if 1
    ...
    colNum = sum(nodeType == 1);
   colorTable = [ 0 .2 1; 1 .2 0; 0 1 1.5; 1 0 1.5]*2 
  
   
colMap = hsv(256);
colMap = cat(1,[0 0 0],colMap);
preCol(nodeType == 1,:) = colMap(ceil((1:colNum)*256/colNum),:);


for i = 1:size(preCol,1)
   targ = find(nodeIDs == useList.preList(i));
    if ~isempty(targ)
        nodeCol(targ,:) = preCol(i,:);
        nodeType(targ) = nodeType(targ) + 2;
    end
end

for i = 1:length(useList.postList)
    postCol = mean(preCol(con2(:,i)>0,:),1);
    targ = find(nodeIDs == useList.postList(i));
    if ~isempty(targ)
        nodeCol(targ,:) = postCol;
    end
end

showCol = zeros(1,size(preCol,1),size(preCol,2));
showCol(1,:,:) = preCol;
...
springDat = springParameters_X01_snap(springIn);
...
save([springRes 'IB_binSyn.mat'],'result')
