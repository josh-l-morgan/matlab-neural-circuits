...
springDir = 'D:\LGNs1\Analysis\springDat\cellProperties\ResAll5\';
...
%con2(find(useList.preList == 2003),:) = 0;
...
%     colProp = ceil(colProp./repmat(max(colProp,[],1),[size(colProp,1) 1]) * 99)+1;
    %colProp = nodeProp-repmat(min(nodeProp,[],1),[size(nodeProp,1) 1]);
    colProp = nodeProp;
