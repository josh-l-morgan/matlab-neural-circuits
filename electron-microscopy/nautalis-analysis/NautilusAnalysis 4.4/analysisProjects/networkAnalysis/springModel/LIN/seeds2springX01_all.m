...
allEdges(allEdges(:,2) == plusOne,3) = 1;
   ...
   nodeType = useList.nodeType;
   nodeIDs = useList.nodes;
   nodeNum = length(nodeIDs);
...
if 0
    ...
    nodeCol = nodeCol * .8;
    nodeCol(sum(nodeCol,2) == 0,:) = .5;
    
    seedCol = [1 0 0; 0 1 0; .5 0 1 ; 0 .5 1; 1 1 1];
     for s = 1:length(seedList)
        targ = find(nodeIDs == seedList(s));
        nodeType(targ) = 5;%nodeType(targ)+10;
