...
if loadData
    ...
    useList = obI2nodes_rtl1(obI,seedList);
    checkList = obI2cellList_seedInput_RGC_TCR(obI,seedList);
    useList = checkList;
...
con = useList.con;
checkNodeIDs = unique([useList.preList useList.postList]);

if 0
sumEdge = sum(con,1)' + sum(con,2);
nodeIDs = useList.nodes(sumEdge>0);
nodeType = useList.nodeType(sumEdge>0);


% nodeType = [useList.preList*0+1 useList.postList*0+2];

nodeIDs = intersect(nodeIDs,checkNodeIDs)

else
   
    nodeIDs = checkNodeIDs;
    nodeType = checkNodeIDs * 0+1;
...
save([springRes 'withFourSeeds.mat'],'result')
