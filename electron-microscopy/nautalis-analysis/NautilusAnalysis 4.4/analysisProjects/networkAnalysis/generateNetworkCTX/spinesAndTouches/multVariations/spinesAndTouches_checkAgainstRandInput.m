...
rawSynMat = singlespinesynapsematrix>0;
touchMat = singlespinetouchmatrix>0;
spineparentids;

totSyn= sum(rawSynMat(:));
...
spineOrder = randperm(size(touchMat,2));
synMat = rawSynMat * 0;
for i = 1:length(spineOrder)
    pickS = spineOrder(i);
   pickA = find(touchMat(:,pickS));
   useA = pickA(ceil(rand*length(pickA)));
   synMat(useA,pickS) = synMat(useA,pickS) + 1;
