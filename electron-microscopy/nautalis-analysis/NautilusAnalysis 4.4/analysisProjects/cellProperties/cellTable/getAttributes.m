function[cellList cellCol] = getAttributes(obI);
...
for c = 1:3
    cellCol(:,c) = cellCol(:,c) - min(cellCol(:,c));
    cellCol(:,c) = cellCol(:,c)/max(cellCol(:,c));
end
...
% 
% cellCol(:,1) = (cellPos(:,1) - center(1)+75)/150;
% cellCol(:,2) = (cellPos(:,2) - center(2)+75)/150;
% cellCol(:,3) = (cellPos(:,3) - center(3)+75)/150;
