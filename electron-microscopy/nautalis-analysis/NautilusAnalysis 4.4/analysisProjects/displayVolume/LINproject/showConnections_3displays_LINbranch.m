...
showCell = [125 unique([9017  527 9018 156 527 397 9013 394 125 527 394 ...
    9014 9015 156 9015 9016 395 396 9019 9020 9021])];


showCellNames = cat(2,num2cell(showCell));

%col = [1 1 1; repmat([1 0 0],[size(preTarg,1) 1]); repmat([0 1 0],[size(postTarg,1) 1])];
col = [[10 10 10] ; colMap(randperm(size(colMap,1),length(showCellNames)-1),:)]
...
viewProps.viewWindow = [1650  850  900 ; ...
                        2000  1100  fsize(3) ];
                    
  viewProps.viewWindow = [1650  1  1200 ; ...
                        2000  fsize(2)  fsize(3) ];
...
mid = [12271, 15116, 7411]


mid = mid ./obI.em.dsRes .* obI.em.res([2 1 3]) .* [4 4 1] / 1000;
xyzPos = [foundX  foundY foundZ].* obI.em.dsRes ./obI.em.res([2 1 3])./[4 4 1] * 1000;

mid = round([mid(1)/4 mid(2)/4 mid(3)/1]);
rad = [500 500 500];
viewProps.viewWindow = [mid(2) - rad(1)  mid(1) - rad(2)  mid(3) - rad(3) ; ...
    mid(2) + rad(1)  mid(1) + rad(2)  mid(3) + rad(3) ];

viewProps.viewWindow = [1500  1000  5000 ; ...
    2000  1500  9200 ];
...
if 1
    ...
    if 0
        rotDir = 'C:\LGNs1\Analysis\movies\LIN\branch125d\'
    ...
    if 0
        ...
        image(uint8(I))
        %%
        %   imwrite(uint8(I),[cellPicDir 'fourSeedSeed_full1.png'])
