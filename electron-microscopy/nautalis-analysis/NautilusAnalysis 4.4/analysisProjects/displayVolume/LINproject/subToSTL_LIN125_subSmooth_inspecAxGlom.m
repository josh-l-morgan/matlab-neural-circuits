...
obMovDir = 'D:\LGNs1\Analysis\movies\subLin125_BranchGlom_3\'
...
if region == 1
    isLocal = [5119  224  ];%[9079 353 468 221 362 5119];%[ 9078 805 399 426 436 423 9003 434 819 349 492 9193];% 221 353 402]; %% shaft synapses1
...
cellList = cat(2,125, group1, group2, group3, group4, group5);
%cellList = [125];
...
%col = [rainBow; [1 1 1]]
groupCol = [1 0 0; 0 1 0; 0 0 1; 1 .5 0; .7 .7 .7; .4 .4 .4];

if region == 1
    groupAlph = [1 .5 1 0 0 0 0 0 ];
else
    groupAlph = [1 .5 .5 .5 .5 .5];
    groupAlph = [1 1 1 1 1 1];
    groupAlph = [1 .6 1 1 1 1 .4];
...
col = [groupCol(1,:);repmat(groupCol(2,:),[length(group1) 1])
    ...
    repmat(groupCol(5,:),[length(group4) 1]);repmat(groupCol(6,:),[length(group5) 1])];
alph = [groupAlph(1);repmat(groupAlph(2),[length(group1) 1]);repmat(groupAlph(3),[length(group2) 1]);...
    repmat(groupAlph(4),[length(group3) 1]); repmat(groupAlph(5),[length(group4) 1]); repmat(groupAlph(6),[length(group5) 1])];
...
if region == 1
	...
	crop = [target - [100 100  100]; target + [100 100 100]];
    crop = [1641.9    0120.6    1243.4;  1841.9    0320.6    1443.4]
    crop = [1670.9    0120.6    1243.4;  1841.9    0320.6    1400.4]
...
if markersOn
    ...
    %
    marker(1).sub = getSynAnchors(obI, rgcs, 125);
    marker(2).sub = getSynAnchors(obI, 125, tcrs );
    marker(3).sub = getSynAnchors(obI,rgcs, tcrs);
    marker(4).sub = getSynAnchors(obI,lins, lins);
...
for i = 5;%1:length(az);
   ...
   % view([startAngle(1) az(i)+startAngle(2)])
