...
for useTri = prim125;%1:size(tri.triCell,1);
...
for d = 1:length(degs);
    ...
    rotDir = 'C:\Users\jlmorgan\Documents\LIN\images\rotSynDistribution11\';
    rotFold = sprintf('%striad_%04.0f\\',rotDir,useTri);
    iNam = sprintf('tri%04.0f_rot%04.0f.png',useTri,d);
    if ~exist(rotFold,'dir'),mkdir(rotFold),end
    writeImage = uint8(showSynTrack(1000:2500,500:2000,:));
    imwrite(uint8(writeImage ),[rotFold iNam])
