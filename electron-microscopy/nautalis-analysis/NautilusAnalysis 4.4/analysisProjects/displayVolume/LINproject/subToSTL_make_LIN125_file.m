...
for i = 1:length(cellList)
    ...
    vSize = max(smallSub,[],1)+1;
    smallVol = zeros(vSize,'uint8');
    inds = sub2ind(vSize,smallSub(:,1),smallSub(:,2),smallSub(:,3));
    smallVol(inds) = 1;  
    se = strel('sphere',2);
    filtVol = imdilate(smallVol,se,'same');
    
    %largeVol = imresize(smallVol,downSamp,'bicubic');
    largeInds = find(filtVol>0);
    [y x z] = ind2sub(vSize,largeInds);
    smallSub = [y x z];
    ...
    if isempty(smallSub)
        ...
        pause(.01)
        tag = 'cell3'
        fileNameFV = sprintf('%sPatch_dSamp%d_%s_%d.mat',objDir,downSamp,tag,obName);
        save(fileNameFV,'fv')
