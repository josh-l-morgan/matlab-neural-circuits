...
%manual measurement of diameter is ~10 x 7

cb = s(4).sub;

scatter3(cb(:,1),cb(:,2), cb(:,3),'.')
axis equal
...
maxR = sqrt((ex1(1)-ex2(1)).^2 + (ex1(2)-ex2(2)).^2 + (ex1(3)-ex2(3)).^2)/2;
minR = sqrt((min1(1)-min2(1)).^2 + (min1(2)-min2(2)).^2 + (min1(3)-min2(3)).^2)/2;

area = pi * minR * maxR


sum(sqrt((sm.skelPos(sm.skelEdges(:,1),1)-sm.skelPos(sm.skelEdges(:,2),1)).^2 + ... 
    (sm.skelPos(sm.skelEdges(:,1),2)-sm.skelPos(sm.skelEdges(:,2),2)).^2 + ...
    (sm.skelPos(sm.skelEdges(:,1),3)-sm.skelPos(sm.skelEdges(:,2),3)).^2))
...
%     sub = subCell{1};
    ...
    if ~isempty(sub)
        ...
        if isempty(smallSub)
            ...
            fv.vertices = fv.vertices * downSamp/5; %rescale to micrometers
