...
rotDir = 'C:\Users\jlmorgan\Documents\LIN\images\outputLINs_07\';
...
%% syns
syns = mot.syn.syns;
preSyns = syns(syns(:,1) == 125,:);
postTarg = preSyns(:,2);
synPostRGC = intersect(postTarg,rgcs);
synPostTCR = intersect(postTarg,tcrs);
synPostLIN = intersect(postTarg,lins);

postSyns = syns(syns(:,2) == 125,:);
preTarg = postSyns(:,1);
synPreRGC = intersect(preTarg,rgcs);
synPreTCR = intersect(preTarg,tcrs);
synPreLIN = intersect(preTarg,lins);
...
group{1} = [125];
group{2} = [356]; %setdiff(synPostLIN(12),125);
group{3} = [208];%setdiff(synPreLIN,125);
group{4} = [209];%setdiff(unique([diTargTCR; triTargTCR]),125);
group{5} = [904]; 
groupCol = [5 5 5; 0 5 0; 5 0 0; .5 .5 5; 5 0 5];
...
degs = 0;%[1:1:360];
for d = 1:length(degs);
    sprintf('rendering angle %d (%ddeg) of %d',d,degs(d),length(degs))
