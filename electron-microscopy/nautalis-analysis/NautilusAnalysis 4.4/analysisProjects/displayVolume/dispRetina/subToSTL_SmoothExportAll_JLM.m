...
obMovDir = [WPN 'stlRot\test\']
objDir = [MPN 'stlFiles\lowRes']
...
if 1
    ...
    cellList = [unique(obI.nameProps.cellNum) ];%'frag' }
    col = hsv(length(cellList))
    alph = ones(length(cellList),1);
    
else
    
    group1 = [];%intersect(Pre(:,1)', isLocal);
...
aL = lightangle(0,45) ;
trackCells = [];
allStl.vertices = []; allStl.faces = []; stlCol = [];
...
for i = 1:length(cellList)
    ...
    if ~isempty(sub)
        ...
        if isempty(smallSub)
            ...
            if writeSTL
                L = size(allStl.vertices,1);
                stlwrite(fileNameSTL,fv.faces,fv.vertices);
                allStl.vertices = cat(1,allStl.vertices, fv.vertices);
                allStl.faces = cat(1,allStl.faces,fv.faces + L);
                stlCol = cat(1,stlCol,repmat(col(i,:),[size(fv.faces,1) 1]));
            end
...
if writeSTL
    fileNameSTL = sprintf('%sdSamp%d_all.stl',objDir,downSamp);
    stlwrite(fileNameSTL,allStl.faces,allStl.vertices,'facecolor',stlCol);
end
