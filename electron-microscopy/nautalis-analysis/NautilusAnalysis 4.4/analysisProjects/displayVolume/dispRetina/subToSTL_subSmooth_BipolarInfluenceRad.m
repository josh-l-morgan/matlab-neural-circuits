...
cid = 4
load(sprintf('%s\\SMs\\sm_cid%d.mat',WPN,cid))

load([MPN 'obI.mat'])
load([MPN 'dsObj.mat'])
load([WPN 'tis.mat'])

obMovDir = [WPN '\movies\IxQ\cid4_VG4_B_VGfunc32\'];
...
vgcs = [2 3 4 10];
bpcON = tis.cells.cids(find((tis.cells.type.typeID' == 7) & (tis.cells.type.onOff(:,1))))
bpcOFF = tis.cells.cids(find((tis.cells.type.typeID' == 7) & (tis.cells.type.onOff(:,2))))
bpcUnk = tis.cells.cids(find((tis.cells.type.typeID' == 7) & (sum(tis.cells.type.onOff,2) == 0)))





inputs{1} = bpcON; %on cells
inputs{2} = bpcOFF; %off cells
prop = synInfluence(sm,inputs);
prop(1,:) = prop(1,:)/max(prop(1,:));
prop(2,:) = prop(2,:)/max(prop(2,:));
propCol = cat(2,prop(2,:)',prop(1,:)',prop(2,:)'*0);
propCol = propCol + .2;



maxProp = max(prop,[],1);
propAlph = maxProp/max(maxProp(:));
propAlph(propAlph<.2) = .2;
propInd  = prop(2,:)./(sum(prop,1))*100;
cmap = jet(100);
propCol = cmap(ceil(propInd),:);
...
propPos = sm.skel.pos;
propPos = propPos(:,[3 1 2]) *20/downSamp;
...
if 0 % color a group of cells with colormap
    colorGroup = [4];
    colorMapGroup = {'hsv'};
...
for i = 1:length(cellList)
    ...
    if ~isempty(sub)
        ...
        if isempty(smallSub)
          ...
          if 1 
              p = mapSkelPropIntoPatch(p,propPos,propCol,propAlph);
          end
