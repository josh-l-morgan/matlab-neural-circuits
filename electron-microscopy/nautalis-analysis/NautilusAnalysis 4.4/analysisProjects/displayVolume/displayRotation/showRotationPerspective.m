function[I_rot] = showRotationPerspective(viewProps,getDeg,movieDir)
...
for d = 1:length(getDeg)
   ...
   % fileName = sprintf('%srot_%05.0f_deg%d.png',movieDir,c, getDeg(d));
    fileName = sprintf('%srot_deg%05.0f.png',movieDir, getDeg(d));

    if ~exist(fileName,'file')
        
        disp(sprintf('Rotating view %d of %d',d,length(getDeg)));
        viewProps.degRot = getDeg(d);
        I = stereoCellsAndMoreFull_PS(viewProps);
        %I = uint8(I.^.7 * 4.5);
