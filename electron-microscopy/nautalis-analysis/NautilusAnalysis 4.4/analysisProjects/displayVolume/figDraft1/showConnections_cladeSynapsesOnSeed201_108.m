...
prePop{1} = [intersect(cladePick.members{1},useRGCs)];
postPop{1} =  [intersect(cladePick.members{1},useTCRs)];

prePop{2} = cladePick.members{1};
postPop{2} = [108];

prePop{3} = cladePick.members{1};
postPop{3} = [201];
% 
% prePop{9} = cladePick.members{2};
% postPop{9} = [108];

synCol  = [ 0 0 0; 0 2 2; 2 0 2; 0 0 0; 0 0 0; 0 0 0; 2 0 0; 0 1 0];
...
%{
imwrite(uint8(showSyn*.2),[cellPicDir 'offSeedClade16_d3.png'])
%}
