...
if 1
    ...
    col = [rainBow; [1 1 1]]
    col = [0 1 0;repmat([1 0 0],[length(RGC) 1]);repmat([0 0 1],[length(TCR) 1])]
    col = repmat([1 1 1],[length(cellList) 1]);
...
if 0
    ...
    useCells = [conTo(1).rgcList conTo(1:4).targ];
    %useCells = [145 160 158 157];
    col = getList_seedColor(targCells,useCells);
    goodCol = find(sum(col,2)>0);
    useCells = useCells(goodCol);
    col = col(goodCol,:);
...
if 0 % show everything
   %useOb = [1:5]*100;
    ...
    colmap = hsv(256);
    cellNames = obI.cell.name;
   cellNum = length(cellNames);
   cellRand = ceil(rand(cellNum,1) * 256);
   cellCol = colmap(cellRand,:);
   showCellNames = cat(2,obI.nameProps.names);
  %showCellNames = {'cell 617'}
   L = length(showCellNames);
   obIDs = 1:L;

   obRand = ceil(rand(L,1)*256);
   col = colmap(obRand,:);
   for c = 1:length(cellNames)
       obTarg = obI.cell.obIDs{c};
       col(obTarg,:) = repmat(cellCol(c,:),[length(obTarg) 1]);
       obRand(obTarg) = cellRand(c);
   end
   
   if exist('useOb','var')
   showCellNames = showCellNames(useOb);
   col = col(useOb,:);
    obRand = obRand(useOb);
end
    
    viewProps.obIDs = obIDs;
   viewProps.ob256 = obRand;
...
if 0
    rotDir = 'D:\LGNs1\Analysis\movies\fourSeeds\seed4fullTCR_saturate2\'
...
if 0
    %% render objects
    objDir = 'D:\LGNs1\Analysis\movies\objExports\allObs_seedA_01\'
    if ~exist(objDir,'dir'),mkdir(objDir), end
    %showSTL_256(viewProps,objDir)
    showSTL(viewProps,objDir)

   % imwrite(uint8(I_topSum),[objDir '2dExample.png'])
end
...
if 0
    ...
    image(uint8(I))
    %%
    %   imwrite(uint8(I),[cellPicDir 'gray_TCR.png'])
