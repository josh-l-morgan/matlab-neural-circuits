...
synDir = ['D:\LGNs1\Analysis\seedBouton\mixedCells\axonPreferenc3\'];
...
viewProps.viewWindow = [mid(2) - rad(1)  mid(1) - rad(2)  mid(3) - rad(3) ; ...
    ...
    %% Draw boutons
    preList = [prePop{1} prePop{2}];
    col2 = repmat([1 0 0],[length(prePop{1}),1]);
    col2 = cat(1,col2,repmat([0 1 0],[length(prePop{2}),1]));
...
%     
    ...
    if 1
        ...
        imwrite(uint8(I_boutons2),sprintf('%sboutonsOnly%d.png',synDir,targCells))
