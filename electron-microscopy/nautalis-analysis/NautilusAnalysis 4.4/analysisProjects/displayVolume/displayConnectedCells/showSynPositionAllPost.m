...
for c = 1:length(allPostCells)


showCell = allPostCells(c);
...
showSyn(~maskSyn) = I_topSum(~maskSyn)/3;
%image(uint8(showSyn))



%{
imwrite(uint8(showSyn),[cellPicDir '201_3syn_view2.png'])
%}

cropSyn = showSyn;
[y x] = find(sum(cropSyn,3)>0);
cropSyn = cropSyn(min(y):max(y),min(x):max(x),:);
image(uint8(cropSyn)),pause(.01)
