...
rotDir = 'C:\Users\jlmorgan\Documents\LIN\images\p5\test3\';
...
synPos = cat(1,mot.syn.synPos{:,:});

%% triads
if length(mot.tri.triCell)
tri = mot.tri;
triCell = tri.triCell;
...
for g = 1:length(groupCol);
    ...
    if rndCol(g)
        gcol = cmap(randperm(size(cmap,1),length(group{g})),:);
    else
        gcol = repmat(groupCol(g,:),[length(group{g}) 1]);
    end
    col = cat(1,col,gcol);
end
...
viewProps.viewWindow = [300 300 0; 1200 1200 fsize(3)];
