function varargout = cellNavGuide_31(varargin)
% CELLNAVGUIDE_31 MATLAB code for cellNavGuide_31.fig
%      CELLNAVGUIDE_31, by itself, creates a new CELLNAVGUIDE_31 or raises the existing
%      singleton*.
%
%      H = CELLNAVGUIDE_31 returns the handle to a new CELLNAVGUIDE_31 or the handle to
%      the existing singleton*.
%
%      CELLNAVGUIDE_31('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CELLNAVGUIDE_31.M with the given input arguments.
%
%      CELLNAVGUIDE_31('Property','Value',...) creates a new CELLNAVGUIDE_31 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before cellNavGuide_31_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to cellNavGuide_31_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help cellNavGuide_31

% Last Modified by GUIDE v2.5 16-Jun-2020 11:17:09
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @cellNavGuide_31_OpeningFcn, ...
    'gui_OutputFcn',  @cellNavGuide_31_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before cellNavGuide_31 is made visible.
function cellNavGuide_31_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to cellNavGuide_31 (see VARARGIN)

% Choose default command line output for cellNavGuide_31
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes cellNavGuide_31 wait for user response (see UIRESUME)
...
function varargout = cellNavGuide_31_OutputFcn(hObject, eventdata, handles)
...
try 
    glob.p.preSyn1.delete
end
try 
    glob.p.preSyn2.delete
end
if val
...
glob.syn.g(L) = glob.syn.g(1);


groupNames ={glob.syn.g(L).name};
...
try synfo.p.delete
    ...
    if ~isempty(synfo.synType)
        isType = find(tis.syn.synType == synfo.synTypeID);
        isSyn = intersect(isSyn,isType);
    end
    
    pos = tis.syn.synPosDS(isSyn,[2 1 3]) * glob.em.dsRes(1);
    set(glob.handles.mainAx, 'NextPlot', 'add')
    synfo.p = scatter3(pos(:,1),pos(:,2),pos(:,3),glob.syn.g.markerSize,...
    ...
    preStr =  get(glob.handles.popupmenu_synPreGroup,'String');
    preStr = preStr{get(glob.handles.popupmenu_synPreGroup,'Value')};
    postStr = get(glob.handles.popupmenu_synPostGroup,'String');
    postStr = postStr{get(glob.handles.popupmenu_synPostGroup,'Value')};
    typeStr = get(glob.handles.popupmenu_synSynapseType,'String');
    typeStr = typeStr{get(glob.handles.popupmenu_synSynapseType,'Value')};

    synfo.name = sprintf('%s TO %s %s',preStr, postStr, typeStr);
