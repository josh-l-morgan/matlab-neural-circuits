function[predSyn synNum] = overlapFracPredictSynOfSubset(varargin);
...
for i = 1:size(conDists,1)
    for p = 1:size(conDists,2)
        ...
        nearNodes(i,p) = sum(pairCount(histBin<=binRes));
        %predSyn(i,p) = sum(pairCount(1:length(nodeFrac))'.*nodeFrac);
    end
end

predSyn = nearNodes/sum(sum(nearNodes(testMask>0)))*synNum;
