function[prefStat] = checkPreference(sourceCon,predSyn,seedTarg,useCon,randomizePick)


useAx = find(sum(useCon(:,seedTarg),2));
pickPref = 1:size(sourceCon,1);

if randomizePick
    pickPref(useAx) = useAx(randperm(size(useAx,1))); %choose new order of axons for determining seed preference
end
...
%% Group preferences
sameSeedPred = predSyn(useCon & mask.tracePost & isSame);
difSeedPred = predSyn(useCon & mask.tracePost & isDif);
sameSeedPref = synPref(useCon & mask.tracePost & isSame & mask.predOne);
difSeedPref = synPref(useCon & mask.tracePost & isDif & mask.predOne);
...
prefStat.sameSeedPref = sameSeedPref;
prefStat.difSeedPref = difSeedPref;
