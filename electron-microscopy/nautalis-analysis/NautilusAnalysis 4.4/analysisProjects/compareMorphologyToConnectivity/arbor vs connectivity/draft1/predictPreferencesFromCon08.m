...
if 1
   ...
   pickPref = 1:size(sourceCon,1);
    for s = 1:length(seedCells)
        ...
        simMat = sqrt(sourceCon .* repmat(sourceCon(pickPref,targ),[1 size(sourceCon,2)]));
...
for i = 1: length(axList)
    ...
    for s = 1:length(seedCells)
        targ = find(cellList==seedCells(s));
        simMat = sqrt(sampCon .* repmat(prefCon(:,targ),[1 size(sourceCon,2)]));
...
sameSynPred = overlapPredictSynOfSubset(skelOverlapPred,binRes,isSame);
sameSynError1 = conError(con,predSyn,isSame)
sameSynError2 = conError(con,sameSynPred,isSame)

difSynPred = overlapPredictSynOfSubset(skelOverlapPred,binRes,isDif);
...
return
%% Survival curve preferences

survSame = cumsum(histSame(length(histSame):-1:1))
survSame = survSame(length(survSame):-1:1);
survDif = cumsum(histDif(length(histSame):-1:1))
survDif = survDif(length(survDif):-1:1);

plot(conBin,survSame,'b')
hold on
plot(conBin,survDif,'r')
hold off
