...
%MPN = GetMyDir
if 1
    load('.\data\Morphology\pcWorkSpace_cardSholl.mat')
else
...
[colIDs colProp] = getList_erosionGlomCol;
[colIDs colProp] = getList_cladeCol;

colIDs = [ 108 201 903 907];
colProp = [ 1 0 0; 0 1 0; 1 0 1; 0 1 1];
colProp = [ 1 0 0; 0 1 0; 1 0 1; 0 1 1];
...
for i = 1:length(cellArbors)
    ...
    yxPos = ceil(yxPos);
    if 0 %use seed cell connectivity
    ...
    elseif size(colProp,2) == 1
        ...
        if isempty(prefTarg)
            ...
            listCellProps(i) = colProp(prefTarg);
            useCellProps(i) = 1;
        end
...
scatter(cellProps(useCellProps>0,1),listCellProps(useCellProps>0))
getExtremes = find((useCellProps>0)' & (cellProps(:,1)>.6))
cellList(getExtremes)
