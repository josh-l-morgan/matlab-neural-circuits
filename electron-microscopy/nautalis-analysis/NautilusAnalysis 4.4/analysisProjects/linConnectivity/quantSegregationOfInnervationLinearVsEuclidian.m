...
eucDists = sm.skel2skel.eucDist;
linDists = sm.skel2skel.linDist;
bigDif = linDists > (eucDists * 3);
...
matchedEuc = eucDists(goodCheck & (procTypeMatch == 11) & samePost);
unMatchedEuc = eucDists(goodCheck & (procTypeMatch == 11) & difPost);
matchedLin = linDists(goodCheck & (procTypeMatch == 11) & samePost);
unMatchedLin = linDists(goodCheck & (procTypeMatch == 11) & difPost);

scatter(unMatchedEuc,unMatchedLin,'k','.')
hold on
scatter(matchedEuc,matchedLin,'r','.')
hold off
...
for i = 1:length(bins)-1
    ...
    sprintf('checking bin %d of %d',i,length(bins)-1)
    subplot(1,1,1)
   ...
   useDistEuc =  (eucDists >= bins(i)) & (eucDists < bins(i+1)) ;
   useDistLin = (linDists >= bins(i)) & (linDists < bins(i+1)) ;
...
%    ssPostDif(i) = sum(sum( useDist &  goodCheck & (procTypeMatch == 33) & difPost));
   ...
   ttPostMatchLin(i) = sum(sum( bigDif & useDistLin &  goodCheck & (procTypeMatch == 11) & samePost));
   ttPostAllLin(i) = sum(sum( bigDif &  useDistLin &  goodCheck & (procTypeMatch == 11)));
   ttPostDifLin(i) = sum(sum( bigDif &  useDistLin &  goodCheck & (procTypeMatch == 11) & difPost));
   
   ttPostMatchEuc(i) = sum(sum( bigDif &  useDistEuc &  goodCheck & (procTypeMatch == 11) & samePost));
   ttPostAllEuc(i) = sum(sum( bigDif &  useDistEuc &  goodCheck & (procTypeMatch == 11)));
   ttPostDifEuc(i) = sum(sum(  bigDif & useDistEuc &  goodCheck & (procTypeMatch == 11) & difPost));
...
plot(bins(2:end),ttPostMatchEuc./ttPostAllEuc,'k')
plot(bins(2:end),ttPostMatchLin./ttPostAllLin,'r')
...
plot(bins(2:end),ttPostAllEuc,'k')
plot(bins(2:end),ttPostAllLin,'r')
...
plot(bins(2:end),ttPostMatchEuc,'k')
plot(bins(2:end),ttPostMatchLin,'r')
...
plot(bins(2:end),ttPostMatchEuc./(ttPostMatchEuc + ttPostDifEuc),'k')
plot(bins(2:end),ttPostMatchLin./(ttPostMatchLin + ttPostDifLin),'r')
