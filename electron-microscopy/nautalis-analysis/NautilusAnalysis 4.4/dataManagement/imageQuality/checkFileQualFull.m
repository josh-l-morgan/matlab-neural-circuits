...
sigThresh = .5; %Only look at darkest sigthresh of image
...
if 0
    ...
    fullImage = double(imread(checkFile));
    [ys xs] = size(fullImage);
    Is = ones(floor(ys/3),floor(ys/3));
    Id = zeros(size(Is,1),size(Is,2),length(yshift));
    [y x] = find(Is);
    y = y*3-2;
    x = x*3-2;
    for i = 1:length(yshift)
        ind = sub2ind([ys xs],y+yshift(i),x+ xshift(i));
        %Is = I([1 + yshift(i) : 3 :ys],[1 + xshift(i) :3:xs]);
        Is(:) = fullImage(ind);
        Id(:,:,i) = 255-Is;
    end
