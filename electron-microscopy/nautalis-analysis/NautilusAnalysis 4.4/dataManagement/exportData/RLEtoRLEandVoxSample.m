...
zStep = 1;%16; %100 for mip3, 20 for mip 1
%xyStep = 4096;
xStep = xSize-1;%16*16*16;
yStep = 1;%16*16*16;
miplevel = 0;
checkMip = 2;
...
for z = 500:length(zSteps);
    ...
    for y = 5000:length(ySteps);
        ...
        getReg = [  1                   1+1000;...
                    ySteps(y)           ySteps(y)+yStep - 1;...
                    zSteps(z)         zSteps(z)+zStep - 1];
        ...
        secRLE = cat(2,secRLE,rleimg);
        
        vLast = 0;
        for v = 1:length(rleimg)/2
            runVal = rleimg(v*2-1);
            runLength = rleimg(v*2);
            vStop = vLast + runLength;
            if runVal
                newV = [repmat(runVal,[1 runLength])' [vLast+1:vStop]' ...
                    repmat(getReg(2,1),[1 runLength])' ...
                    repmat(getReg(3,1),[1 runLength])' ];
                secV = cat(1,secV,newV);
            end
            vLast = vStop;
        end %end all vals
    end %end run y
    ...
    secV
    allV = cat(1,allV,secV);
    
    
     secRLEFileName = sprintf('secRLE_%05.0f.mat',getReg(3,1));
     secVoxFileName = sprintf('secVox_%05.0f.mat',getReg(3,1));         
     save(secVoxFileName,'secV');
     save(secRLEFileName,'secRLE');
end % end run z
