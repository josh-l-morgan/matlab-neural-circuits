...
allBBox2 = allBBox;
allBBox(1:2,1:2) = allBBox(1:2,1:2)/checkDown;
...
for z = 1:length(zSteps);
    ...
    %%Get region low res plane
            getReg2 = allBBox2;
            getReg2(1:2,1) = 1;
            getReg2(3,1:2) = [zSteps(z)-1 zSteps(z)+zStep-2];
        ...
        lastX = 0;
                ...
                for r = 1:length(rls)
                    ...
                    [dy dx dz] = ind2sub(regSize2,runID);
                    if oIDs(r)>0
                        c = c+1;
                        eachSubs{c} = [dy' dx' dz'];
                       % Iv2(runID) = oIDs(r);
                ...
                subs = cat(1,eachSubs{:});
            scatter3(subs(1:100:end,1),subs(1:100:end,2),subs(1:100:end,3),'.')
