...
clear all
TPN = GetMyDir;
TargetDir = 'C:\Users\joshm\Documents\MyWork\myData\ParrallelBeam';
downSamp = 1;
inams = getPics(TPN);
...
%%%
for h = 1:7
    H = ones(maxBeam(1)+ height,maxBeam(2)+width,'uint8') * 256;
for i = 1:length(inams)
    if hex(i) == h
    ...
    imwrite(H,[TargetDir '\hex' num2str(h) '.tif'],'Compression','none')
end %end hexes
colormap gray(256)
image(H)
