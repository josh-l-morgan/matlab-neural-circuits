...
%% sum edges
threshs= [.1:.1:.3]; % thresolds to check for edges.  later scaled
sizes = 1:1:5;  % sizes to check for edges
sI = sumEdge(smI,threshs,sizes,1); % robustish edge detection
...
tI = watershed(dI2,4);
cmap = rand(max(tI(:))+1,1)*255+1; %random colormap
image(cmap(tI+1)),pause(.01)

tProps = regionprops(tI,dI2,'MinorAxisLength','MajorAxisLength',...
    'PixelIdxList','ConvexHull','MeanIntensity','Centroid','MinIntensity',...
    'ConvexImage','BoundingBox','Orientation');
minAx = [tProps.MinorAxisLength];
majAx = [tProps.MajorAxisLength];
ori = [tProps.Orientation];
meanIntensity = [tProps.MeanIntensity];

tape = find((minAx < 120 ) & (majAx > 200) & ((90 -abs(ori))<10));

showI = I * 0;
for i = 1:length(tape)
   showI(tI == tape(i)) = 1; 
end
image((tI == 0) * 1000 + showI * 100),pause(.01)
...
%% Grab sections
sections = find((minAx > 10) & (majAx < 50));

inTape = [];
for i = 1 : length(tape)
    inTape = unique([inTape; wI(tI == tape(i))]);
end
