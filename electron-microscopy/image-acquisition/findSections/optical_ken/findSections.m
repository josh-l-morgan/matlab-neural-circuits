...
%% Set Variables
tapeMin = 1; %min length
tapeMax = 500; %max width
majThresh = 60; %max section length
minThresh = 10; %min section width
...
%% Filter I 
I = medfilt2(I,[11 11]);
gKern = gaus3d([25 25 20],1);
I = fastCon(double(I),gKern);

%% resize
scl = 3000/max(size(I));
I =  imresize(double(I),[size(I,1)*scl,size(I,2)*scl]);
[ys xs] = size(I);
I = max(I(:)) - I;
...
% 
% vidObj = VideoWriter([TPN 'secOv1.avi']);
% close(vidObj);
% open(vidObj);
% 
% 
% for i = 1:length(sections)
%    
%     ct = xProps(sections(i)).Centroid;
%     
%      sPic = Iraw(ct(1)-100:ct(1)+100,ct(2)-100:ct(2)+100);
%     image((sPic -40) * 2),pause(.01)
%     %sPics{i} = sPic;
%      currFrame = getframe;
