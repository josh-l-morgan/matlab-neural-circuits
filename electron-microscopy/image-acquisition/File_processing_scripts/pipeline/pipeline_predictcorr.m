function param=pipeline_predictcorr(param)
...
%           stransx=smtx(1,3)*param.downscale.scale; 
%           stransy=smtx(2,3)*param.downscale.scale;
%           smtx(1,3)=0; smtx(2,3)=0; 
%           
%           tmtx=squeeze(param.rigid.absrigidmatrix(slice-1,trow,tcol,:,:));
%           ttransx=tmtx(1,3)*param.downscale.scale; 
%           ttransy=tmtx(2,3)*param.downscale.scale;
%           tmtx(1,3)=0; tmtx(2,3)=0; 
%           %itmtx=inv(tmtx);
%           %itmtx(1,3)=-ttransx; itmtx(2,3)=-ttransy;
%           rmtx=tmtx*inv(smtx); %relative transformation matrix
% %           rmtx(1,3)=stransx-ttransx;
% %           rmtx(2,3)=stransy-ttransy;
%           
%           rtrans=[stransx-ttransx stransy-ttransy 1]';
%           vrtrans=tmtx*rtrans;
%           rmtx(1,3)=vrtrans(1);
%           rmtx(2,3)=vrtrans(2);
          ...
          for gy=1:1:gridheight
            for gx=1:1:gridwidth
              ...
              rtg=rmtx*tg;
              corresp_tx(slice,srow,scol,ts,trow,tcol,gy,gx)=rtg(1)+midx; %tgridx(gy,gx);
              corresp_ty(slice,srow,scol,ts,trow,tcol,gy,gx)=rtg(2)+midy; %tgridy(gy,gx);
            end;
          end;
...
param.predictcorr.corresp_tx=corresp_tx;
param.predictcorr.corresp_ty=corresp_ty;
param.predictcorr.corresp_tphi=corresp_tphi;
