function param=pipeline_computelocalcorr(param)
...
nroftslices=param.predictcorr.nroftslices; %should be 3: previous, this, next
gridheight=param.predictcorr.gridheight;
gridwidth=param.predictcorr.gridwidth;
patchwidth=param.predictcorr.patchwidth/lparam.downscalefactor;
patchheight=param.predictcorr.patchheight/lparam.downscalefactor;
...
corresp_tx=(param.predictcorr.corresp_tx-1)/lparam.downscalefactor+1;
corresp_ty=(param.predictcorr.corresp_ty-1)/lparam.downscalefactor+1;
corresp_tphi=param.predictcorr.corresp_tphi;

%The following variables will store the newly computed corresponding point information
...
XI=ones(patchheight,1)*(1:patchwidth); XI=XI-patchwidth/2;
YI=(1:patchheight)'*ones(1,patchwidth); YI=YI-patchheight/2;
        ...
        for trow=1:1:nroftrows
          for tcol=1:1:nroftcolumns
            ...
            if (ts==2)&&(trow==srow)&&(tcol==scol)
              ...
              corresp_ctx(slice,srow,scol,ts,trow,tcol,:,:)=corresp_sx(slice,srow,scol,ts,trow,tcol,:,:);
              corresp_cty(slice,srow,scol,ts,trow,tcol,:,:)=corresp_sy(slice,srow,scol,ts,trow,tcol,:,:);
              ...
              for gy=1:1:gridheight
                for gx=1:1:gridwidth
                  ...
                  %clipping
                                if sx1<1 sx1=1; end;
                                if sy1<1 sy1=1; end;
                                if sx2>size(simage,2) sx2=size(simage,2); end;
                                if sy2>size(simage,1) sy2=size(simage,1); end;
                  spatch=simage(sy1:sy2,sx1:sx2);
                  
                  %Cut out corresponding image region out of target image (rotated)
                  tgridx=corresp_tx(slice,srow,scol,ts,trow,tcol,gy,gx);
                  tgridy=corresp_ty(slice,srow,scol,ts,trow,tcol,gy,gx);
                  tphi=corresp_tphi(slice,srow,scol,ts,trow,tcol);
                  
                  %Check whether the four patch corners are inside the image
                  cornersx=[-patchwidth/2 patchwidth/2 patchwidth/2 -patchwidth/2];
                  cornersy=[-patchheight/2 -patchheight/2 patchheight/2 patchheight/2];
                  ...
                  if (min(rotcornersx)>=1)&&(min(rotcornersy)>=1)&&(max(rotcornersx)<=size(timage,2))&&(max(rotcornersy)<=size(timage,1))
                    ...
                    if (max(rotcornersx)<1)||(max(rotcornersy)<1)||(min(rotcornersx)>size(timage,2))||(min(rotcornersy)>size(timage,1))
                      ...
                      if (abs(ctx)<(patchwidth*(1-lparam.minoverlap)))&&(abs(cty)<(patchheight*(1-lparam.minoverlap)))
                  ...
                  if lparam.showimages==1
                    if tpatchfits
                      ...
                      if (ctx==0)&&(cty==0)
                        ...
                        hold on;
                        drawpoly([1 patchwidth patchwidth 1]-ctx,[1 1 patchheight patchheight]-cty,'r');
                        hold off;
                      end;
                      %                 else
                      %                   figure(17);
                  ...
                  if (tpatchfits==1)
                    ...
                    smx1=max(1+transx,1); smx2=min(transx+patchwidth,patchwidth);
                    smy1=max(1+transy,1); smy2=min(transy+patchheight,patchheight);
                    smx=(smx1+smx2)/2;
                    smy=(smy1+smy2)/2;
                    tmx1=max(1-transx,1); tmx2=min(-transx+patchwidth,patchwidth);
                    tmy1=max(1-transy,1); tmy2=min(-transy+patchheight,patchheight);
                    tmx=(tmx1+tmx2)/2;
                    tmy=(tmy1+tmy2)/2;
                    rotmx=cos(tphi)*(tmx-patchwidth/2)-sin(tphi)*(tmy-patchheight/2)+tgridx;
                    rotmy=sin(tphi)*(tmx-patchwidth/2)+cos(tphi)*(tmy-patchheight/2)+tgridy;
                    
                    if lparam.showimages==1
                      figure(18);
                      hold on;
                      drawpoly([1 patchwidth patchwidth 1]+transx,[1 1 patchheight patchheight]+transy,col);
                      plot(smx,smy,'k*');
                      plot(smx,smy,'wo');
                      hold off;
                    end;
...
param.computelocalcorr.corresp_ctx=(corresp_ctx-1)*lparam.downscalefactor+1;
param.computelocalcorr.corresp_cty=(corresp_cty-1)*lparam.downscalefactor+1;
param.computelocalcorr.corresp_cmax=corresp_cmax;
...
param.computelocalcorr.corresp_cvar075=corresp_cvar075; %*lparam.downscalefactor;
