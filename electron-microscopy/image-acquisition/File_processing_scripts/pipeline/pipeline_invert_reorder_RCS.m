function pipeline_invert_reorder_RCS(param)
%This script is made to reorder and rename raw images
%Naming scheme in original image is in the order row, column, slice (for multiple tiles only)
  ...
  for slice=startslice:1:endslice
    for row=1:1:param.nrofrows
      for column=1:1:param.nrofcolumns
        name=sprintf(param.baseorigname,row,column,lparam.sourceslice(slice)); filename=sprintf('%s%s',param.origdir,name);
        if exist(filename,'file')
          txt=sprintf('Loading image %s ...',filename);
          disp(txt);
          image=imread(filename);
