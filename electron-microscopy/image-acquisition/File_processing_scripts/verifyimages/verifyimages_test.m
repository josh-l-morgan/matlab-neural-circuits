function verifyimages_test(sourcedir,sourcefiletemplate)
%verifyimages.m
...
% targetdir_downscaled='./downscaled/S1Column_downscaled/w12/';
% downscaledtargetsize=2048;
% downscaledfileextension='_downscaled.png';
% downscaledfiletype='png';
% targetdir_cropped='./center/S1column_center/w12/';
...
%   disp(txt);
  ...
  k=getkeywait(2); %getkeywait(60);%Wait 60 seconds for keypress
  ...
  for f=1:1:nroffiles
    ...
    if exist(filenamewithpath,'file')==0
      ...
      %%%%%%%%%%%%
      if length(find(vdiffmean)) == 0 %vdiffmean==0
        txt=sprintf('%s white image:', filename);
        disp(txt);
        outlierspos_WhiteImage(overallptr+f) = 1; 
      else
        txt=sprintf('%s non-white image:', filename);
        disp(txt);
        outlierspos_WhiteImage(overallptr+f) = 0;
      end;
...
fprintf(fid,'\r\n');
for f=1:1:size(outlierspos,2) %nroffiles
    %outlierspos_WhiteImage(f)
    name=overallfilelist{f};
    if outlierspos_WhiteImage(f) == 1
        fprintf(fid, 'White image found in image %s',name);
        fprintf(fid,'\r\n');
    else
