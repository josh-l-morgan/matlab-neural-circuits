...
function boldify24(h,g)
%BOLDIFY24	Make lines and text bold for standard viewgraph style.
...
% *** UNCLASSIFIED ***
% $Id: boldify24.m,v 1.3 2002/07/03 13:12:44 gronosky Exp $
...
for i=1:length(ha)
  if (strcmp(get(ha(i),'Type'),'axes') == 1)
    ...
    set(ha(i),'FontSize',24); 		% Tick mark and frame format
    ...
    set(get(ha(i),'XLabel'),'FontSize',24); % 24-pt labels
    set(get(ha(i),'XLabel'),'FontWeight','Bold');
    set(get(ha(i),'XLabel'),'VerticalAlignment','top');

    set(get(ha(i),'YLabel'),'FontSize',24); % 24-pt labels
  ...
  for j=1:length(hc)
    ...
    if chtype(1:4) == 'text'
      sz = get(hc(j), 'FontSize');
      if sz < 12
	set(hc(j),'FontSize',12); % Do not resize text larger than 12 pt
      end
