function param=photostagemap_ovintermediateupdatestagemap(param)
...
lparam=param.ovintermediateupdatestagemap;

for w=param.processwafers
  nrofslices=size(param.generatestagemap.linearslicepos{w},1);
  if lparam.useregionalign==1
    gabsmatrixcount=param.ovintermediateregionabsolutealign.gabsmatrixcount{w};
    gabsmatrix=param.ovintermediateregionabsolutealign.gabsmatrix{w};
  else
    gabsmatrixcount=param.ovintermediateabsolutealign.gabsmatrixcount{w};
    gabsmatrix=param.ovintermediateabsolutealign.gabsmatrix{w};
  end;
  ...
  if lparam.overridepixelsizemu==0
    ovpixelsizenm=param.intermediateoverviewimages.pixelsize_nm(w,1,1); %Use st1 sec1 to get pixel size scale factor
...
%     end;
%   end;
  stsec=param.alignintermediateoverviewimages.stsec{w};
