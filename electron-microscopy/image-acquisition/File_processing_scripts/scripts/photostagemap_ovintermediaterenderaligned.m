function param=photostagemap_ovintermediaterenderaligned(param)
%This function renders a stack of aligned overview images (aligned within-wafer only)
%This will only render the 'good' slices (only those have been aligned)
%By Daniel Berger for MIT-BCS Seung / Harvard Lichtman, March 2010

lparam=param.ovintermediaterenderaligned;
...
%   end;
  ...
  for i=1:1:nrofslices
    sourceimagename=[param.intermediateoverviewimages.targetdirectory sprintf(param.intermediateoverviewimages.filenametemplate,w,stsec(i,1),stsec(i,2))];
    ...
    if lparam.useregionalign==1
      rmat=param.ovintermediateregionabsolutealign.gabsmatrix{w}{i};
    else
      rmat=param.ovintermediateabsolutealign.gabsmatrix{w}{i};
    end;
