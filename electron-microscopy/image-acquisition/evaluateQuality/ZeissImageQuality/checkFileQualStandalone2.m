function[tile maxId] = checkFileQualStandalone(checkFile)
%%Quick Image Quality Check
%%by Josh Morgan, Jeff Lichtman Lab, Harvard 2012

%% user variables
dsamp = 200; %number of kernal samples in each dimension 
sigThresh = .1; %Only look at darkest sigthresh of image LGNS = 0.01
top = .5; % select top X of sample based on contrast differences LGNs = 0.5
...
if (length(useVals)>5) & ~acquisitionFail  %check quality if there is enough information in the image
    sortMean = sort(useVals,'ascend');
    threshVal = sortMean(end  - (fix(length(sortMean)*.01))- (fix(length(sortMean)*(.01+sigThresh))));
