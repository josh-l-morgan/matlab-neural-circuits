...
datDir = [TPN 'dat\'];
if ~exist(datDir),mkdir(datDir),end
...
sm.Set_PassedTypeSingle('AP_CONTRAST',25);
sm.Set_PassedTypeSingle('AP_BRIGHTNESS',50);

sm.Set_PassedTypeSingle('DP_STAGE_BACKLASH',1);

clear previousWrite
stripNum = 20;
sm.Fibics_AcquireImage(BEW,BEH,BEdwell,[datDir 'testImage.tif']);
...
for x = 1:stripNum
    ...
    if exist(previousWrite,'file')
        ...
        I = imread(previousWrite);
        Irs = imresize(I,[size(I,1),size(I,2)/20],'nearest','Antialiasing',1);
        clear I
        imwrite(Irs,[isoDir num2str(x-1) '.tif'],'Compression','none');
        clear Irs
        toc
    end
    tic
...
save([TPN 'dat\fibSettings.mat'],'fibSet')
%% Make image
%buildSnapshotStageEdge(TPN)
