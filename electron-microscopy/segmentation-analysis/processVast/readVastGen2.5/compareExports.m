...
nam1 = [imageDir{1} allDir(1).parseTiles(1).name];
        ...
        recVol1 = zeros(ys,xs,zs,'single');
        recVol2 = zeros(ys,xs,zs,'single');


tilePos2 = cat(2,[allDir(2).parseTiles.row]',[allDir(2).parseTiles.col]',[allDir(2).parseTiles.z]');
for i = 1:length(allDir(1).parseTiles)
    ...
    if ~isempty(targ)
        ...
        if sumDif
            disp(sprintf('found %d differences',sumDif))
            ind = find(difImage);
            sparseTiles(i).ind = single(ind);
            sparseTiles(i).id1 = single(image1(ind));
            sparseTiles(i).id2 = single(image2(ind));
