...
nbndfaces=0;
bndfaces=zeros(10000,3); %[];

for k=1:nz
  for j=1:ny
    pvtx=[];
    for i=1:nx
      %Expand bndfaces in steps of 1000 as necessary
      % Voxel(j,i,k)
      ...
      if (voxels(j,i,k)==id)
        ...
        if (nbndfaces>=size(bndfaces,1)-20)
          bndfaces=[bndfaces; zeros(10000,3)];
        end;
        ...
        if (max(size(pvtx))==0)
          pvtx=ones(1,8)*((nx+1)*(ny+1)*(k-1) + (nx+1)*(j-1)); % + i);
          pvtx(2)=pvtx(1)+1;
          pvtx(3)=pvtx(3)+(nx+1);
          pvtx(4)=pvtx(3)+1;
          pvtx(5)=pvtx(5)+(nx+1)*(ny+1);
          pvtx(6)=pvtx(5)+1;
          pvtx(7)=pvtx(5)+(nx+1);
          pvtx(8)=pvtx(7)+1;
        end;
...
%         vtx=ones(1,8)*((nx+1)*(ny+1)*(k-1) + (nx+1)*(j-1) + i);
%         vtx(2)=vtx(1)+1;
%         vtx(3)=vtx(3)+(nx+1);
%         vtx(4)=vtx(3)+1;
%         vtx(5)=vtx(5)+(nx+1)*(ny+1);
%         vtx(6)=vtx(5)+1;
%         vtx(7)=vtx(5)+(nx+1);
%         vtx(8)=vtx(7)+1;
        ...
        indx=[i-1 j-1 k-1];
        rc=offset+sf.*indx+sf/2;
        
        vtxcrd=[[-0.5 -0.5 -0.5];[0.5 -0.5 -0.5];[-0.5 0.5 -0.5];[0.5 0.5 -0.5];[-0.5 -0.5 0.5];[0.5 -0.5 0.5];[-0.5 0.5 0.5];[0.5 0.5 0.5]];
        for v=1:1:8
          vtxcrd(v,:)=vtxcrd(v,:).*sf + rc;
        end;
...
bndfaces=bndfaces(1:nbndfaces,:); %prune back to actual size
...
[bndfaces_ren,orig_vertex_id]=voxel_renumbervertex(bndfaces);
