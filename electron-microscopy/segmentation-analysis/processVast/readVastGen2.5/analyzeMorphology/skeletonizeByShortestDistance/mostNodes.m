%function[skel] = mostNodes(surfVox)
...
[pathL1] = drawPathDist(seedPath);

showMaxProp(moveObj,pathL1.owned+10);
showMaxProp(moveObj,pathL1.pathDist+10);

pause(.01)
%% find path to voxel that is path to greatest distnce tip
reps = 5;
%[path2maxDist] = passMaxAndDist(surfVox,pathDist,reps);
[path2maxDist] = passMaxAndDist(surfVox,pathL1.pathDist,reps);
...
%% for each vox on path from tip, enter distance of longest source tip
[pathL2] = drawPathDist(newPred,seedPath.dist);


colormap gray(256)
showMaxProp(moveObj,pathL2.owned+10);
showMaxProp(moveObj,pathL2.pathDist+10);
