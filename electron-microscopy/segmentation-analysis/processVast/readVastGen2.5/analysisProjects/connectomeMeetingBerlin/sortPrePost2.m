...
shuffleGraph = postGraph2;
newOrder = sortPost;

image(shuffleGraph * 20)
...
for y = 1:length(newOrder)
    for x = 1:length(newOrder)
        newVal = shuffleGraph(y,x);
        oldY = find(sortPost == newOrder(y));
        oldX = find(sortPost == newOrder(x));
        oldVal = postGraph2(oldY,oldX);
        if (oldVal - newVal)
            'bad val'
            break
        end
...
%%hGraph
rGraph = zeros(length(sortPre),length(newOrder));
for i = 1:size(edges,1)
   targPre = find(sortPre == edges(i,2) );
   targPost = find(newOrder == edges(i,1));
   if ~isempty(targPre) & ~isempty(targPost);
      rGraph(targPre,targPost) = hGraph(targPre,targPost)+1; 
   end
end
sum(rGraph(:))
