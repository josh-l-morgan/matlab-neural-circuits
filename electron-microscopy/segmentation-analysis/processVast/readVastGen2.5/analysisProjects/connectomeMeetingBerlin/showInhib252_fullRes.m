...
%%Axons plus dim TC
showCellNames = num2cell(intersect(cellList,[108 201 252  2000:2099]));

specialCells = [108 201 252];
showCellNames = num2cell(setdiff(cellList,specialCells));
col = colMap(ceil((1:length(showCellNames))*256/length(showCellNames)),:);
col = col(randperm(size(col,1)),:);
col = col/20;


showCellNames = cat(2,showCellNames,num2cell(specialCells))
col = cat(1,col,[1 0 0;0 1 0 ; 0 0 1]);


showCellNames = num2cell(specialCells);
col = [0 1 0;0 0 1 ; 1 0 0]/3;
...
viewProps.viewWindow = [1  1  1 ; ...
    fsize(1)  fsize(2)  fsize(3) ];
...
%{
imwrite(uint8(I_topSum),[cellPicDir 'glomA_types2.png'])
%}
