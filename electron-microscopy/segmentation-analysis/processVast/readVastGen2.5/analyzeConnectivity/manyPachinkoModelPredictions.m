...
meanDist = mean(randSynDist,1);

sortRandDist = sort(randSynDist,'ascend');
targ95 = ceil(reps * 0.975);
top95 = randSynDist(targ95,:);
bottom95 = randSynDist(end-targ95+1,:);

modRes.reps = reps;
modRes.targ95 = targ95;
modRes.meanDist = meanDist;
modRes.top95 = top95;
modRes.bottom95 = bottom95;


randMore = randSynDist - repmat(top95,[reps 1]);
randMore(randMore<0) = 0;
countRandMore = sum(randMore,2)/sum(synMat(:));
realMore = synMat(:) - top95';
realMore(realMore<0) = 0;
ratMore = sum(realMore)/sum(synMat(:))
