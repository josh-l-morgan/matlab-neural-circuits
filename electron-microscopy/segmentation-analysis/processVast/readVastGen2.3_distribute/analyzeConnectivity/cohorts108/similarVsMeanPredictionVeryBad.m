...
for pre = 1:size(normMat,1)
    ...
    simList = sum(abs(minMat),2);
    simOther = simList(otherPre).^5;
    mostSim = find(simOther == max(simOther),1);
    for post = 1:size(normMat,2)
        mostSimMat(pre,post) = normMat(mostSim,post);
    end
end
...
blankMat = normMat;
blankMat(:,1) = 0;
scatter(meanMat(:,1)*100+rand(size(meanMat(:,1)) ,1)*.01,normMat(:,1)*100,'lineWidth',2);
hold on

scatter(simMat(:,1)*100,normMat(:,1)*100,'r','lineWidth',2);
xlim([0 100])
ylim([0 100])

%scatter(mostSimMat(:,1)*100,normMat(:,1)*100,'g','lineWidth',2);
hold off
