...
threshs= [.9];%[.8:.1:.8]; % df = [.3:.1:.5], thresolds to check for edges.  later scaled
sizes = [  1 2 3 4 ];  % sizes to check for edges

%%Watershed variables
gKern2 = gaus3d([30 30 1],3); %df = ([ 20 20 1], 3)
% gKernSurround = gaus3d([60 60 1],5)*1.5;
% gKern2 = gKernCenter - gKernSurround;
% gKern2 = gKern2 - mean(gKern2(:));
% plot(gKern2(round(size(gKern2,1)/2),:)),pause(.01)
watMin =20; %df = 10, minimum intensity change for watershed
...
for i = 1: length(wy)
    ...
    cons = unique(wI(sub2ind2D(size(wI),nyx(:,1),nyx(:,2))));
    cons = cons(cons>0);
...
%     sInd = sub2ind2D(size(I),dyx(:,1),dyx(:,2));
%     sVal = c2(sInd);
%     sEdge = uI(sInd);
%     eVal = sVal(sEdge>0);
%     eVal = sVal;
