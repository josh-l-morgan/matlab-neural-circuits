...
%%Edge varibles
gKern1 = gaus3d([1 1 1],1); %kernal for edge convolution
threshs= [.6];%[.8:.1:.8]; % df = [.3:.1:.5], thresolds to check for edges.  later scaled
sizes = [ 1 2];  % sizes to check for edges

%%Watershed variables
gKern2 = gaus3d([5 5 1],2); %df = ([ 20 20 1], 3)
watMin =50; %df = 10, minimum intensity change for watershed
...
newBorders = imdilate(newBorders,strel('disk',1));
