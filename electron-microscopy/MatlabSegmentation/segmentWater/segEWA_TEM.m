...
threshs= [.9:.1:.9]; % df = [.3:.1:.5], thresolds to check for edges.  later scaled
...
dKern(:) = sqrt(((y-kSize/2)).^2 + ((x - kSize/2)).^2);
...
for i = 1:length(wy)
    ...
    if ~isempty(eVal)
        newBorders(wy(i),wx(i)) = mean(mVal)-mean(eVal);
    end
end
subplot(1,1,1)
