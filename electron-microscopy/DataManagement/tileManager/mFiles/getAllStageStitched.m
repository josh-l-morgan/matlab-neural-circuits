...
for s = 1:length(us.sec)
    waf = num2str(us.sec(s).wafNum);
    sec = num2str(us.sec(s).wafSec);
    
    
    for t = 1:16
        path = us.sec(s).paths{t};
        if ~isempty(path)
            break
        end
    end
    slashs = regexp(path,'\');
    secDir = path(1:slashs(end-2));
...
for w = 1:length(wafers)
    ...
    if exist(stitchedWafDir(1:end-1),'dir')
        stitchedWafDir
        pause
        rmdir(stitchedWafDir(1:end-1),'s')
    end
