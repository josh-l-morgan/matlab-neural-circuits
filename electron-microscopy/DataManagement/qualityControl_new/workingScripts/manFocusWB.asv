...
fig = gcf;
sid = 1:length(wif.sec);
colormap gray(256)
xs = wif.sec(1).tile(1).Info.ImageWidthInPixels;
ys = wif.sec(1).tile(1).Info.ImageHeightInPixels;
q = 0;
readI = 1; readS = 1;
maxRes = 500;
...
maxSize = wif.sec(1).tile(1).Info.ImageHeightInPixels;
subReg = [round(xs/2 - isize/2) round(xs/2 - isize/2) + isize-1];
subRegX = subReg; subRegY = subReg;
s = 1;
secNam = wif.sec(s).name;
maxSec = length(wif.sec);

numTile = length(wif.sec(s).tile);
good = zeros(1,numTile); rate = good;
I = 255-imread([WPN wif.sec(s).name wif.sec(s).tile(t).name],'PixelRegion',{subRegY,subRegX});

downSamp = imread([WPN wif.sec(s).name wif.sec(s).ov]);  %Get overview image
%downSamp = imread([wif.dir(1:end-1) 'shaped\downsamp\' wif.secNam{s} '.tif']);

%% Get images of mosic quality

if exist([WPN 'wQual.mat'])
    load([WPN 'wQual.mat'])
else
    wQual = checkAllFocus(WPN);
end
qual = wQual.sec(s);
...
scatter(wif.sec(s).tile(t).col,wif.sec(s).tile(t).row);
failed = fails{s};
rated = rates{s};
...
rc = [[wif.sec(s).tile.row]' [wif.sec(s).tile.col]'];
...
while ~q  % While not quitting
    ...
    if isstruct(pressed) %was there a keypress?
        ...
        if readS
            ...
            scatter(blankX,blankY,'.','SizeData',500,'MarkerEdgeColor',[.1 .1 .1])
            scatter(wif.sec(s).tile(t).col,wif.sec(s).tile(t).row,'k');
            scatter(wif.sec(s).tile(t).col,wif.sec(s).tile(t).row,'.','w');
            failed = fails{s};
            rated = rates{s};
if ~isempty(rated),scatter(rated(:,2),rated(:,1),'*','g'),end
if ~isempty(failed),scatter(failed(:,2),failed(:,1),'x','r'),end
        ...
        if  readI
            ...
            I = 255-imread([WPN wif.sec(s).name wif.sec(s).tile(t).name],'PixelRegion',{useRegY,useRegX});
            readI = 0;
