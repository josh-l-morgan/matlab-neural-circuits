%function[userEval] = manFocus(TPN,tile);
...
I = 255-imread([TPN wif.sec(s).tile(t).name],'PixelRegion',{subRegY,subRegX});

downSamp = imread([TPN wif.sec(s).ov]);  %Get overview image
%downSamp = imread([wif.dir(1:end-1) 'shaped\downsamp\' wif.secNam{s} '.tif']);

%% Get images of mosic quality


load([TPN secNam '\qual.mat']);
showMos = qual.mos.qualMos;
...
rc = [wif.sec(s).row wif.sec(s).col];
...
while ~q  % While not quitting
    ...
    if isstruct(pressed) %was there a keypress?
        ...
        if readS
            ...
            scatter(blankX,blankY,'.','SizeData',500,'MarkerEdgeColor',[.1 .1 .1])
            scatter(wif.sec(s).col(t),wif.sec(s).row(t),'k');
            scatter(wif.sec(s).col(t),wif.sec(s).row(t),'.','w');
            failed = fails{s};
            rated = rates{s};
if ~isempty(rated),scatter(rated(:,2),rated(:,1),'*','g'),end
if ~isempty(failed),scatter(failed(:,2),failed(:,1),'x','r'),end
        ...
        if  readI
            ...
            I = 255-imread(wif.sec(s).tile(t).name,'PixelRegion',{useRegY,useRegX});
            readI = 0;
