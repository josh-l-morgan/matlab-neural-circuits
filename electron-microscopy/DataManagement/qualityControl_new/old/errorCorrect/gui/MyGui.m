...
%% Make Figure
RG = figure('Visible','off','Position',[10,10,1000,520]);
...
if exist('.\temp\RG_LastFile.mat')
    ...
    [Cell,Results,Status,Combo]=LoadData(DPN);
...
%% Display Results
    function ShowResults(source,eventdata)
        ...
        if isstruct(Results)
           ...
           DotA1 = uicontrol('style','text','Position',[Rx+120,Ry+30,40,20],...
                'String',Results.Arbor(1).Dots);
           DotDendA1 = uicontrol('style','text','Position',[Rx+170,Ry+30,40,20],...
                'String',round(1000*Results.Arbor(1).DotDend)/1000);
           ...
           if size(Results.Arbor)>1 
               ...
               DotA2 = uicontrol('style','text','Position',[Rx+120,Ry+5,40,20],...
                  'String',Results.Arbor(2).Dots);
               DendA2 = uicontrol('style','text','Position',[Rx+170,Ry+5,40,20],...
                  'String',round(1000*Results.Arbor(2).DotDend)/1000);        
           end
    ...
    function Run_Callback(source,eventdata)
        ...
        if exist([DPN '\data\DotStats']), anaNN; end
        Status.Running=0;
        ShowResults
        Display = axes('Units','pixels','Position',[10,10,500,500]);
        image(Combo)
