clear all
[TFN TPN] = GetMyFile;
%% Turn examin quality of em image
...
colormap gray(256)
        ...
        I1 = double(imread(Inam,'PixelRegion',{[ 1 dsampy ys],[1 dsampx xs]}));
        Id = zeros(size(I1,1),size(I1,2),length(yshift));
        
        %grab samples
        for i = 1:length(yshift)
            Is = double(imread(Inam,'PixelRegion',{[ 1+yshift(i) dsampy ys],[1 + xshift(i) dsampx xs]}));
...
%% Record data        
        focSec.quality = quality; 
        focSec.percentSaturation = percentSat;
        focSec.meanGlob = meanGlob;
        focSec.meanDifs = meanDifs; 
        focSec.scaledDifs = scaledDifs;
        focSec.sdOverGlob = scaledDifs/meanGlob;
        focSec.focusTarget = focTarg;
%% Report
