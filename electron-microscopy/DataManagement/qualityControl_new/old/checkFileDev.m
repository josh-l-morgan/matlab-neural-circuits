function[tile] = checkFileQuality(checkFile)
...
colormap gray(256)
    ...
    Im = mean(Id,3);
    vals = sort(Im((sumSats<1)),'ascend');
    lowThresh = vals(round(length(vals)*.1));
    highThresh = vals(round(length(vals)*.90));
    Idev = std(Id,1,3);
    lowDev = mean(Idev(Im<=lowThresh));
    devVals = sort(Idev(:),'ascend');
    %highDev = mean(devVals(end-10:end));
    highDev = mean(Idev(Im>=highThresh));
    Dev = (highDev-lowDev)/lowDev;
