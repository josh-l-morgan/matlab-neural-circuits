...
yshift = [ 1 1 1 0 2]; %shifts for contrast measurement
xshift = [ 1 0 2 1 1];
...
for s = 1 : length(wif.sec) % run sections
    ...
    for t = 1:length(wif.sec(s).tile)
        ...
        difC = median(vals(1:fix(length(vals)/4)));
        %difC = median(vals(:));
        ...
        difx = mean(sIx(1:fix(length(Ix)/25)));
        dify = mean(sIy(1:fix(length(Iy)/25)));
        con = difC;
        ...
        focSec(s).tile(t).difX1X2Y1Y2XY1 = [mean(Ix(:)) mean(Iy(:))];
        focSec(s).tile(t).focY = mean(sIy);
        focSec(s).tile(t).focX = mean(sIx);
