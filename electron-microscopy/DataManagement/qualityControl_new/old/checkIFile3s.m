function[tile] = checkIFile3s(checkFile)
...
colormap gray(256)
    ...
    Sats = (Id==255) | (Id == 0);
    sumSats = sum(Sats,3);
    vals = maxId(sumSats<2);
    %image(meanId)
    sortMean = sort(vals,'descend');
    sigThresh = .20;
    threshVal = sortMean(fix(length(sortMean)*sigThresh));
    useSig = find((maxId>=threshVal) & (sumSats<2));
    image(fitH(maxId>=threshVal))
    pause(1)
    ...
    quality = min(vertQual,horzQual)/range;
%     
