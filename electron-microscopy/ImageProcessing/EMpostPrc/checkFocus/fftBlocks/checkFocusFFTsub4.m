...
minSigArea = .1; % proportion of image zthat must have signal
...
useMFI = useMFI(useMFI>3);
if isempty(useMFI)
    'no difference found in reference images'
    useMFI = 10:20;
end

goodMFI = allMFIs(1,useMFI);

%% Judge reference images
for i = 1:2
    listYX = memListYX{i};
    mfIs = memMFIs{i};
    ...
    refHighFFT(i) = median(sortFM(1:grab));
end
...
for i = 1:length(iNam);
    ...
    %allMFIs(i,:) = mean(mfIs,1);
     ...
     subplot(2,1,2)
     showMos = fftMos;
     showMos = showMos - min(showMos(:));
     showMos = showMos * 255/max(showMos(:));
     image(showMos),pause(.1)
    ...
    highFFT(i) = (highFFT(i)-refHighFFT(1))/abs(refHighFFT(1)-refHighFFT(2));
