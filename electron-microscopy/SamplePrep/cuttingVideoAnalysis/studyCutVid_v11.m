...
tic
strip = zeros(nFrames,rw-lw+1,3,'uint8');
grabFs = 1000
for i = 1: grabFs: nFrames
    tic
    if ~mod(i,10)
        sprintf('Reading frame %d of %d.', i,nFrames)
        
    end
    frame = read(vidObj, [i min(i+grabFs - 1]);  %read first few images
    sy = (i-1)+ 1;
    strip(sy :(sy +size(frame,4) - 1),:,:) = permute(mean(frame(ty:by,lw:rw,:,:),1),[4 2 3 1]);
    toc
end
toc
