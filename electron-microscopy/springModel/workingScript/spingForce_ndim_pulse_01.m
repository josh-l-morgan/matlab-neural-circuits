...
g=gca();
hold on;
g=scatter3(posMat(:,1),posMat(:,2),posMat(:,3),500,'.k');
axis tight;
drawnow;
...
debug=0;
colormap(jet);
testColor=ind2rgb((conListReal(:,3).*(1/(round(max(conListBig(:,3)),5)))),jet);
...
if writeBool==1
    vidObj=VideoWriter('testing2.avi');
    open(vidObj);
    vidObj.FrameRate=15;
end
            ...
            for l=1:(nodeNum/2)
                if mod(l,lineNeurPerc)==0
                    ...
                    for s=1:length(synList(:,1))
                        ...
                        g=line(prePostPos(:,1),prePostPos(:,2),prePostPos(:,3),'Color',testColor(l,:),'LineWidth',1);  %conListReal(l,3),
