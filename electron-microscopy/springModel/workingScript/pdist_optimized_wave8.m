...
posMat=rand(length(nodeList),dimensions).*500;
...
colormap(jet);
testColor=ind2rgb((conListReal(:,3).*0.9),jet);
...
%         h=scatter3(posMat(7,1),posMat(7,2),posMat(7,3),1000,'.r');
        if lineBool==1
            conListBig=conListReal(conListReal(:,3)>0.004,:);
            colormap(jet);
            testColor=ind2rgb((conListBig(:,3).*0.9),jet);
            for l=1:(nodeNum/2)
                if mod(l,197)==0
                    preID=nodeList(l,:);
                    ...
                    for s=1:length(synList(:,1))
                        ...
                        h=line(prePostPos(:,1),prePostPos(:,2),prePostPos(:,3),'Color',testColor(l,:),'LineWidth',1);  %conListReal(l,3),
