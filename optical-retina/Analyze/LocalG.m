...
for i=1:size(RD,2)
    ...
    if exist([Raid '\' char(RD(i)) '\data\Local.mat'])
    load([Raid '\' char(RD(i)) '\data\Local.mat'])
    Master(i).Local=Local; end
...
for i = 1: size(Master,2)
   LDD5=Master(i).Local(2).Mid.LocalDD(:,5); %Get local densities for first arbor
   if size(Master(i).Local,2)>2
        LDD5b=Master(i).Local(3).Mid.LocalDD(:,5);
        LDD5=[LDD5 ; LDD5b];
   end
   LDD5=sort(LDD5);
   Top5=LDD5(round(.95*size(LDD5)):size(LDD5));
   meanTop5(i)=mean(Top5)
end
...
%subplot(4,1,1)
[n x]= hist(PlotMe(Ages==5),0:bin:.4); %% Show All P5
plot(x,n,'r'),title('P5','FontSize',12);
hold on
%subplot(4,1,2)
[n x]= hist(PlotMe(Ages==7),0:bin:.4);
plot(x,n,'m'),title('P7','FontSize',12);
%subplot(4,1,3)
[n x]= hist(PlotMe(Ages==12),0:bin:.4);
plot(x,n,'b'),title('P12','FontSize',12);
%subplot(4,1,4)
[n x]= hist(PlotMe(Ages>30),0:bin:.4);
...
E(1)=std(PlotMe(Ages==5))/sqrt(sum(Ages==5));
E(2)=std(PlotMe(Ages==7))/sqrt(sum(Ages==7));
E(3)=std(PlotMe(Ages==12))/sqrt(sum(Ages==12));
E(4)=std(PlotMe(Ages>30))/sqrt(sum(Ages>30));


errorbar([5 7 12 35],[mean(PlotMe(Ages==5)) mean(PlotMe(Ages==7)) ...
    mean(PlotMe(Ages==12)) mean(PlotMe(Ages>30))],E,'r')
...
%scatter(Ages(BIs),MeanDD(BIs),'g')
scatter(Ages(BIs),Arbor1DD(BIs),'b')
scatter(Ages(BIs),Arbor2DD(BIs),'r')

legend('ON','OFF','BI ON arbor', 'BI OFF arbor')
