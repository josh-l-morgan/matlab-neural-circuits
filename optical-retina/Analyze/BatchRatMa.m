...
for i=1:size(Map,2)
    ...
    if ~strcmp(Map(i).IMfolder,'?')&& exist([Map(i).TPN 'data'])
        ...
        %%Check Status
        clear Status
            if exist([TPN 'data\Status.mat'])
                load([TPN 'data\Status.mat'])
            else 
                Status.Fix=0;
                Status.Ra=0;
                Status.Ma=0;
                Status.Dr=0;
                Status.Running=0;
                save([TPN 'data\Status.mat'],'Status')
            end
            
            %make sure field exists
            if ~isfield(Status,'Dr')
                Status.Dr=0;
            end
            
            %Select file that is not running
            if ~Status.Running  %and if the program is not running
             Status.Running=1 %register program as running
             save([TPN 'data\Status.mat'],'Status')
             
                %% if its been processed to the point of needing ratioing ect...     
                if exist([TPN 'data\BigFilled.mat']) && ...
                        exist([TPN 'data\AllSeg.mat']),

                    %% run analysis functions and update status
