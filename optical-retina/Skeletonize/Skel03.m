...
TPN=GetMyDir;
TPNm=[TPN 'mask\'];
dTPNm=dir(TPNm); dTPNm=dTPNm(size(dTPNm,1));

if ~exist([TPN 'temp']), mkdir([TPN 'temp']),end

Ir=tiffread2([TPNm dTPNm.name])
zs=size(Ir,2); [ys xs] = size(Ir(1).data);
D = zeros(ys,xs,zs,'uint8');
for i = 1: zs
    D(:,:,i)=Ir(i).data;
end
...
AllSeg=zeros(0,3,2); % Create full Variable for Segments
