...
for o=1:ob  %Run for All objects
    ...
    Fnew=(D*0)>0;
    Left=D>0;
    Fnew(Sold(1),Sold(2),Sold(3))=1;
    count=1;
    Block=ones(3,3,3);
   while sum(Left(:))>0  %while there are new voxels to do

    Left=Left & ~Fnew;
    Fdi=imdilate(Fnew,Block);
    Fnew=Fdi & Left;
    %image(sum(Fnew,3)*30),pause(.01) 
    ...
    if exist('Snew','var'), Sold=Snew; clear Snew; end  %update skeleton points list: new becomes old
     ...
     if mod(SG,30)==0,  image((sumD>0)*255 - (sum(Left,3)>0)*100),pause(.01),end
