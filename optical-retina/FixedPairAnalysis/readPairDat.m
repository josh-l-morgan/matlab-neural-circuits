...
%%  Compare synapse number (bar graphs)
useC = [1 2 4];
for t = 1:length(useC)
    ...
    %useNow = intersect(useNow,useS([dat{useS,colSyn}]>0));
    %X = [dat{useNow,colPolyArea}] * 0.0603^2 ;
    Y21 = [dat{useNow,colSyn}];

    barScat2(Y9,Y21)
    P = ranksum(Y9,Y21);
    title(sprintf('%s P = %.3f',cType,P));
    ys = ylim;
    ylim([0 max(20,ys(2))])
end

hold off
%%  Compare num Territory (bar graphs)
useC = [1 2 4];
maxYs = 0;
subplot(1,1,1)
for t = 1:length(useC)
    ...
    useNow = intersect(useNow,myG);
    %useNow = intersect(useNow,useS([dat{useS,colSyn}]>0));
    Y9 = [dat{useNow,colNumTer}] * 0.0603^2 ;
    ...
    useNow = intersect(useNow,myG);
    %useNow = intersect(useNow,useS([dat{useS,colSyn}]>0));
    Y21 = [dat{useNow,colNumTer}] * 0.0603^2 ;
...
% end
% 
% xlswrite('showSyn',showSyn,sprintf('P%.0f',useAges(a)))
% end
...
for t = 1:length(useC)
    ...
    %X = [dat{useNow,colPolyArea}] * 0.0603^2 ;
    Y9 = [dat{useNow,colSyn}];
    hist(Y9,xAx,'EdgeColor','r','FaceColor','r');
    hold on
    ...
    hist(Y21,xAx,'EdgeColor','b','FaceColor','w')
    P = ranksum(Y9,Y21);
    title(sprintf('%s P = %.3f',cType,P));
    ylim([0 15])
end
...
for t = 1:length(useC)
    ...
    useNow = intersect(useNow,find(appoed));
    useNow = intersect(useNow,[eval(cType)]);
    useNow = intersect(useNow,G{10});
    %useNow = intersect(useNow,useS([dat{useS,colSyn}]>0));
    X9 = [dat{(useNow),colNumAppo}] ;
    Y9 = [dat{useNow,colSyn}];
    ...
    useNow = intersect(useNow,G{10});
    %useNow = intersect(useNow,useS([dat{useS,colSyn}]>0));
    X21 = [dat{(useNow),colNumAppo}];
    Y21 = [dat{useNow,colSyn}];
    h = barScat3(Y9./X9,Y21./X21); 
    ylim([0 4])
%     xlim([0 3])
    hold on
