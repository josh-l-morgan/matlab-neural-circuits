...
for k = 1:NumCells
    ...
    switch Cell.Type
        ...
        mDtA(c)=sum(DotDist(Ter>0))/sum(Ter(:)>0);
        mDdA(c)=sum(DendDist(Ter>0))/sum(Ter(:)>0);
        mDDA(c)=mDtA(c)/mDdA(c);    
...
CellNum=size(mDdA(Mono),1);
TabL=cell(CellNum,8);
TabL(:,1)=num2cell(mDdA(Mono));
TabL(:,2)=num2cell(mDtA(Mono));
TabL(:,3)=num2cell(DD(Mono));
TabL(:,4)=num2cell(Rdend(Mono));
TabL(:,5)=num2cell(Rdot(Mono));
TabL(:,6)=num2cell(RDD(Mono));
TabL(:,7)=num2cell(Age(Mono));
TabL(:,8)=num2cell(Name(Mono));
...
TabLs=[];
for i = 1: size(Ages,1)
   TabLs=[TabLs;TabL(Age(Mono)==Ages(i),:)]; 
end
...
TabL(:,4)=num2cell(Rdend(BIs));
TabL(:,5)=num2cell(Rdot(BIs));
TabL(:,6)=num2cell(RDD(BIs));
