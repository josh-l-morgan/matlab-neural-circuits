...
figure
%%Map Raid folders
Raid='\\128.208.64.36\wonglab\Josh\Analyzed\Output2';
Raid=GetMyDir
RaidDir=dir(Raid);
...
for i=1:size(RD,2)
    ...
    if exist([Raid '\' char(RD(i)) '\find\SG.mat'])
    load([Raid '\' char(RD(i)) '\find\SG.mat'])
    Master(i).SG=SG; end
end
...
for i=1:size(Master,2), c(i)= ~isempty(Master(i).SG); end
Master=Master(c); clear c
