...
for k = 8:size(UseCells,1)
    ...
    [ys xs c] = size(Im); zs=size(Idir,1);
                ...
                DrawSeg(DrawSeg<1)=1;  %get rid of non indexables
...
Cell.Name=TPN(size(TPN,2)-5:size(TPN,2)-1);

%%Make side view
clear DDO1 DDO2
DDO1(:,:,1)=SegOrtho1*50;
DDO1(:,:,2:3)=cat(3,DotOrtho1,DotOrtho1)*256;
DDO2(:,:,1)=SegOrtho2'*50;
DDO2(:,:,2:3)=cat(3,DotOrtho2',DotOrtho2')*256;
maxy=max(size(DDO1,1),size(DDO2,1))
maxx=max(size(DDO1,2),size(DDO2,2))
DDO=zeros(maxy*2,maxx,3,'uint8');
DDO(1:size(DDO1,1),1:size(DDO1,2),:)=DDO1;
DDO(size(DDO1,1)+1:size(DDO1,1)+size(DDO2,1),1:size(DDO2,2),:)=DDO2;
image(DDO)

%Create image index

KPN='\\128.208.64.36\wonglab\Josh\Analyzed\Output2\Images\Ortho\'
if exist([TPN 'Cell.mat'])
    load([TPN 'Cell.mat'])
    Name=[KPN 'P' Cell.Age '_' Cell.Type '_' Cell.Name '.tif'];
else
    Name=[KPN  Cell.Name '.tif'];
end
if isempty(find(Name=='?'));
    imwrite(DDO,Name,'Compression','none')
end
