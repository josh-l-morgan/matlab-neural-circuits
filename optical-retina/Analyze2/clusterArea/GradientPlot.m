...
for k = 1:size(UseCells,1)
    ...
    NormBDD(k,:)=resample(BDD,90,size(BDD,2));
    NormBDD(k,:)=NormBDD(k,:)/mean(NormBDD(k,:));
    
    NormBdot(k,:)=resample(Bdot,90,size(Bdot,2));
    NormBdot(k,:)=NormBdot(k,:)/mean(NormBdot(k,:));
    
    NormBdend(k,:)=resample(Bdend,90,size(Bdend,2));
    NormBdend(k,:)=NormBdend(k,:)/mean(NormBdend(k,:));    
    
    GradPlot.NormBDD=NormBDD;
    GradPlot.NormBdot=NormBdot;
    GradPlot.NormBdend=NormBdend;
