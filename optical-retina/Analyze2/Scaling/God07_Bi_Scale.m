...
%%Sum Arbors
DA=sum(TotLengthA,2)./sum(Area,2);
PA=sum(DotNumA,2)./sum(Area,2);
PD=sum(DotNumA,2)./sum(TotLengthA,2);
...
CompareAges(Age,DdOA(:,1),Age,DdOA(:,2))
CompareAges(Age,DtOA(:,1),Age,DtOA(:,2))
CompareAges(Age,DD(:,1),Age,DD(:,2))

[R P] = corrcoef(DdOA(:,1),DdOA(:,2))
[R P] = corrcoef(DtOA(:,1),DtOA(:,2))
[R P] = corrcoef(DD(:,1),DD(:,2))

%%Scatter Arbors
scatter(DdOA(:,1),DdOA(:,2))
scatter(DtOA(:,1),DtOA(:,2))
scatter(DD(:,1),DD(:,2))
...
PlotAges(Age,mean(Rdot,2))
PlotAges(Age,mean(Rdend,2))
PlotAges(Age,mean(RDD,2))


stats(DdOA(Age==7))

ranksum(DD(Age==7,1),DD(Age==12,1))
ranksum(PD(Age==7),PD(Age==12))

ranksum(RDD(Age>30,2),RDD(Age==12,2))

ranksum(mean(RDD(Age==7,:),2),mean(RDD(Age>7,:),2))

ranksum(mean(Rdend(Age>30,:),2),mean(Rdend(Age<30,:),2))
