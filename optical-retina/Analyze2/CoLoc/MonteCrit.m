

colormap = gray(256)
TPN=GetMyDir


%% Get Presynaptic field

'reading Presynaptic Image'
Idir=dir([TPN 'I']); Idir=Idir(3:size(Idir,1));
Ip=imread([TPN 'I\' Idir(1).name]);
[ys xs c] = size(Ip);
zs=size(Idir,1);
I=zeros(ys,xs,zs,'uint8');

for i = 1: zs
   Ip=imread([TPN 'I\' Idir(i).name]);
   I(:,:,i)=Ip(:,:,3);
end
'done reading image'

'median filtering image'
for i = 1: zs     
    I(:,:,i)=medfilt2(I(:,:,i));
end
'median filtering done'

Imax=max(I,[],3);
image(Imax),pause(.1)


%% Get Post Synaptic Field
'Get Synapse info'
load([TPN 'Dots.mat'])
load([TPN 'find\SG.mat'])
Pass=SG.passF;

%%
SE=strel('ball',3,3);
Cube=ones(5,5,5);
for y=1:5;for x=1:5;for z=1:5;
            Cube(y,x,z)=dist([y x z],[3 3 3]);
end, end, end
Cube=Cube<2.5;
for i = 1: Dots.Num
    VoxP=Dots.Vox(i).Pos;
    mins=min(VoxP,[],1);
    maxes=max(VoxP,[],1);
    DiSpaceV=zeros(maxes-mins+5);
    clear DiVox
    for d = 1:3
        DiVox(:,d)=VoxP(:,d)-mins(d)+1;
    end
    DiVoxInd=sub2ind(size(DiSpaceV),DiVox(:,1)+2,DiVox(:,2)+2,DiVox(:,3)+2);
    DiSpaceV(DiVoxInd)=1;
    DiSpace=DiSpaceV;
    DiSpace=imdilate(DiSpace,Cube);
    %DiSpace=imdilate(DiSpace,SE);
    DiPerim=bwperim(DiSpaceV,26);
    DiSpaceSurround=DiSpace-DiSpaceV+DiPerim;
    [Sy Sx Sz]=ind2sub(size(DiSpace),find(DiSpaceSurround));
    Sur=[Sy+mins(1)-3,Sx+mins(2)-3,Sz+mins(3)-3];
    SurroundA(i)={Sur};
    if mod(i,100)==0; PercentDoneSurrounding=i/Dots.Num*100, end
end




%% Get Dendrites
'Get Dendrites'
mdir=dir([TPN 'mask']); mdir=mdir(3:size(mdir,1));
Dend=imread([TPN 'mask\' mdir(1).name]);
[ys xs]=size(Dend); zs=size(mdir,1);
Dend=zeros(ys,xs,zs,'uint8');
for i = 1:zs
    Dend(:,:,i)=imread([TPN 'mask\' mdir(i).name]);
end
Dend=Dend==1;
Dend=bwperim(Dend,6);
sDend=uint8(sum(Dend,3));


%% Measure Real Surrounds

%%Find all in bounds Surrounds
InBounds=ones(1,Dots.Num);
for i = 1:Dots.Num
    Check=cell2mat(SurroundA(i));
    if sum(Check(:)<1) | sum(Check(:,1)>ys) | sum(Check(:,2)>xs) | sum(Check(:,3)>zs)
        InBounds(1,i)=0;
    end
    Check=Check(~((sum(Check<1,2)) |(Check(:,1)>ys) | (Check(:,2)>xs) | (Check(:,3)>zs)),:);
    Surround(i)={Check};
end


InB=find(InBounds);
siz=[ys xs zs];
clear Top Top5 Mean Peak Sum Blues
for i =1: Dots.Num
   Check=cell2mat(Surround(i));
   CheckI=sub2ind(siz,Check(:,1),Check(:,2),Check(:,3));
   Voxes(i)=size(CheckI,1);
   Blue=I(CheckI);
   Blue=sort(Blue);
   Blues(i)={Blue};
   Sum(i)=sum(Blue);
   Mean(i)=mean(Blue);
   Peak(i)=max(Blue(:));
   Top(i)=mean(Blue(fix(.75*size(Blue,1)):size(Blue,1)));
   Top5(i)=mean(Blue(max(1,size(Blue,1)-10):size(Blue,1)));
end

%% Run Monte from Dot
% Run=find(Pass);
% Reps=100;
% for i = 1:size(Run,1)
%     id = Run(i);
%     ID(i)=id;
%     
%     %%Get relative surround
%     rSur=cell2mat(Surround(id));
%     for d = 1:3
%         rSur(:,d)=rSur(:,d)-round(mean(rSur(:,d)));
%     end
%     
%     %%Restrict Dendrite search area
%     [fdy fdx fdz]=ind2sub(siz,find(Dend));
%     fDend=[fdy fdx fdz];
%     for r = 1:Reps  %Run Monte reps
%         while 1 %Search for viable spot
%             Pick=fix(rand*size(fdy,1))+1;
%             NewSur=[rSur(:,1)+fDend(Pick,1) rSur(:,2)+fDend(Pick,2) rSur(:,3)+fDend(Pick,3)];
%             if ~(sum(NewSur(:)<1) | sum(NewSur(:,1)>ys) | sum(NewSur(:,2)>xs) | sum(NewSur(:,3)>zs))
%                 break, end
%         end
%         
%        CheckI=sub2ind(siz,NewSur(:,1),NewSur(:,2),NewSur(:,3));
%        Blue=I(CheckI);
%        Blue=sort(Blue);
%        %Blues(r)={Blue}; %%!!!!!!!!!!!!!!!!!!!! Ditch later or eat Mem
%        mcSum(r)=sum(Blue);
%        mcMean(r)=mean(Blue);
%        mcPeak(r)=max(Blue(:));
%        mcTop(r)=mean(Blue(fix(.75*size(Blue,1)):size(Blue,1)));
%        mcTop5(r)=mean(Blue(max(1,size(Blue,1)-10):size(Blue,1)));
%        
%     end
%     
%     
%     P.id(i)=id;
%     P.Top(i)=sum(mcTop>=Top(id))/Reps;
%     P.Top5(i)=sum(mcTop5>=Top5(id))/Reps;
%     P.Sum(i)=sum(mcSum>=Sum(id))/Reps;
%     P.Peak(i)=sum(mcPeak>=Peak(id))/Reps;      
%     
%     PercentDoneMonte=i/size(Run,1)
% 
% end

%% Run Monte From Cell

Run=find(Pass);
Reps=100;

[fdy fdx fdz]=ind2sub(siz,find(Dend));
fDend=[fdy fdx fdz];

mcSum=zeros(size(Run,1),Reps);
mcTop=mcSum; mcTop5=mcSum; mcPeak=mcSum; mcMean=mcSum;
for r = 1:Reps
New=zeros(siz,'uint8');   
for i = 1:size(Run,1)
    id = Run(i);
    ID(i)=id;
    
    %%Get relative surround
    rSur=cell2mat(Surround(id));
    for d = 1:3
        rSur(:,d)=rSur(:,d)-round(mean(rSur(:,d)));
    end
    
    %%Restrict Dendrite search area

    while 1 %Search for viable spot
            Pick=fix(rand*size(fdy,1))+1;
            NewSur=[rSur(:,1)+fDend(Pick,1) rSur(:,2)+fDend(Pick,2) rSur(:,3)+fDend(Pick,3)];
            if~(sum(NewSur(:)<1) | sum(NewSur(:,1)>ys) | sum(NewSur(:,2)>xs) | sum(NewSur(:,3)>zs))
                CheckI=sub2ind(siz,NewSur(:,1),NewSur(:,2),NewSur(:,3));
                if~sum(New(CheckI)), break, end %if space not already occupied
            end
    end
        
      
       New(CheckI)=1;
       Blue=I(CheckI);
       Blue=sort(Blue);
       %Blues(r)={Blue}; %%!!!!!!!!!!!!!!!!!!!! Ditch later or eat Mem
       mcSum(i,r)=sum(Blue);
       mcMean(i,r)=mean(Blue);
       mcPeak(i,r)=max(Blue(:));
       mcTop(i,r)=mean(Blue(fix(.75*size(Blue,1)):size(Blue,1)));
       mcTop5(i,r)=mean(Blue(max(1,size(Blue,1)-10):size(Blue,1)));
       %%Need a threshold measure
    
 

end
PercentDoneMonte=r/Reps*100
NewS=max(New,[],3);
image(sDend*3+NewS*30),pause(.01)

end


%%  Analyze Results
CNum=size(Run,1);
VNum=fix(CNum/20);

%%Scale all to puncta mean
Top5n=Top5(Pass)'./mean(mcTop5,2);
mcM=mcTop;
M=Top;


Thresh=median(mcM(:))*1;
Colo=M(Pass)>Thresh;
Res=sum(Colo);
mcColo=mcM>Thresh;
mcRes=sum(mcColo,1);
P=sum(mcRes>=Res)/Reps


%%Check Validity
%%Validity is the frequency that you would see any non-coloc in a
%%population where 5% of puncta are random
clear VM
for i = 1:size(mcM,2)
    Rpic=fix(rand(7,1)*CNum)+1;
    VM(:,i)=mcM(Rpic,i);
end
VColo=VM>Thresh;
VRes=sum(VColo,1)
Validity=sum(VRes>=VNum)/Reps


%%Plot possible Synapses
%Cs = probability of colocalization given synapse
Cr=mean(mcRes)/size(Run,1) %Probability of colocalization givin random
Co=Res/size(Run,1) %Observed number of colocalizations
Cs=[0:.01:1];


%%What is the threshold in which you would "always" see some non-loc if 5%
%%of the population was random
%Validity=size(Run,1)*.05 * (1-Cr)


S=(Co-Cr)./(Cs-Cr);
plot(S)
ylim([0 1])
minReal=(Co-Cr)./(1-Cr)




