...
clear I IT Imax MedThresh MeanThresh  Surround Dend InBounds
...
for i = 1: Dots.Num
    ...
    SurroundA(i)={Sur};
    if mod(i,100)==0; PercentDoneSurrounding=i/Dots.Num*100, end
end
...
InB=find(InBounds);
siz=[ys xs zs];
clear Top Top5 Mean Peak Sum Blues sumIT pThresh
ShowSurround=I*0;
for i =1: Dots.Num
   Plane=round(Dots.Pos(i,3)); 
...
New=zeros(siz,'uint8');  
for i = 1:size(Run,1)
fDend=AllDend(AllDend(:,3)>(Dots.Pos(i,3))-4 & (AllDend(:,3)<(Dots.Pos(i,3))+4),:);    
for r = 1:Reps
    id = Run(i);
    ID(i)=id;
...
BestR=find(CrC<.5 & minRealC == max(minRealC(CrC<.5)),1);
BestCo.P=P(BestR);
BestCo.Cr=CrC(BestR);
BestCo.Co=CoC(BestR);
...
for k = 1:size(kDir,1)
    TPN=[KPN kDir(k).name '\'] ;
    load([TPN 'CosumIT.mat'])
    COcell(k,:)=[struct2cell(Co.Best)' struct2cell(Co.Std)']
    CellName(k,1)={kDir(k).name};
    CrB(k)=Co.Best.Cr;
    CrS(k)=Co.Std.Cr;
    CoB(k)=Co.Best.Co;
    CoS(k)=Co.Std.Co;
    NumD(k)=Co.Std.NumDots;
end
FieldNames=[fieldnames(Co.Best)' fieldnames(Co.Std)'];

CRsy=size(CellName,1)+1;
CRsx=size(FieldNames,2)+1;
CoRes=cell(CRsy,CRsx);
CoRes(1,2:CRsx)=FieldNames;
CoRes(2:CRsy,1)=CellName;
CoRes(2:CRsy,2:CRsx)=COcell;
CoRes(1,1)={'Best then Std'}
