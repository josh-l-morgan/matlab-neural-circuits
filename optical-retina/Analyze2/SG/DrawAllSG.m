...
for k = 1:size(Kdir,1)
    ...
    if exist([TPN 'data\AllSegCut.mat']) & exist([TPN 'find\SG.mat'])%& ~exist([TPN 'images\IMf.tif'])
    ...
    [ys xs c] = size(Im); zs=size(Idir,1);
                ...
                for i=1:size(DrawSeg,1)
                    ...
                    for d=1:devs+1
                        ...
                        %%Draw Orthos
                        SegOrtho2(min(max(round(sy),1),ys),min(max(round(sz),1),zs))=SegOrtho2(min(max(round(sy),1),ys),min(max(round(sz),1),zs))+1;
                        SegOrtho1(min(max(round(sz),1),zs),min(max(round(sx),1),xs))=SegOrtho1(min(max(round(sz),1),zs),min(max(round(sx),1),xs))+1;
                    end
                end
                clear Dist
...
if exist([TPN 'Cell.mat'])
    load([TPN 'Cell.mat'])
    Name=[KPN 'CombosSG\' 'P' Cell.Age '_' Cell.Type '_' Kdir(k).name '.tif'];
else
    Name=[KPN 'CombosSG\' Kdir(k).name '.tif'];
end
