...
%{
%% Load mask for volume and area measurements
...
ystart=round(min(Mids(:,1)));ystop=round(max(Mids(:,1)));
xstart=round(min(Mids(:,2)));xstop=round(max(Mids(:,2)));
for y = ystart:1:ystop
    for x=xstart:1:xstop
         XYD=dist2(Dots,[y x]);
         XYMids=dist2(Mids,[y x]);
         %VolDist=dist(Dv,Mids(i,:));
  
    %% Run all Bins
    for b = 1:size(DBins,2)
        XYDots(y,x,b)=sum(XYD<=DBins(b));
        XYLength(y,x,b)=sum(Length(XYMids<=DBins(b)));
        %LocalVol(i,b)=sum(VolDist<=DBins(b))*(xyum *xyum*zum);
    end
    if mod(i,100)==0, XYPercentDone=i/size(Mid2,1)*100,end
    end
end
