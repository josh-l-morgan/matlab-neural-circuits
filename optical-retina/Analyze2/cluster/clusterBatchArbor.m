...
for M = 1:size(RD,2)
    ...
    MidsA=mean(AllSeg,3);  %find all  midpoints

    %%Load Dots
    load([TPN '\data\DotStats.mat'])
    DotsA=DotStats(:,:,3); %Get all Dots
    ...
    if isfield(Results,'Arbor')
        Zstart(2)=Results.Arbor(1).Top;
        Zstop(2)=Results.Arbor(1).Bottom;
        if size(Results.Arbor)>1
            Zstart(3)=Results.Arbor(2).Top;
            Zstop(3)=Results.Arbor(2).Bottom;
        end
    end

    for z = 1:size(Zstart,2)
        Mids=MidsA(MidsA(:,3)>=Zstart(z) & MidsA(:,3)<=Zstop(z),:);
        Dots=DotsA(DotsA(:,3)>=Zstart(z) & DotsA(:,3)<=Zstop(z),:);
        ...
        Local(z).Mid.MidMap=MidMap;
        Local(z).Dot.DotMap=DotMap;

        Local(z).DBins=DBins;
        Local(z).Mid.MidStep = MidStep;

        Local(z).Dot.DotToDots=DotToDots;
        Local(z).Dot.DotToLength=DotToLength;
        Local(z).Dot.DotToDD=DotToDD;

        Local(z).Mid.LocalDots=LocalDots;
        Local(z).Mid.LocalLength=LocalLength;
        Local(z).Mid.LocalDD=LocalDD;
        ...
        hold off 
        Local(z).Dot.Neighbors=Neighbors;
        ...
        Times=3;
        Local(z).Mid.meanBright=meanBright;
        ...
        for b = size(BinsUsed)
            ...
            name=[Cell.Name '_r' num2str(DBins(BinsUsed(b))) '_' num2str(z) '.tif']
            imwrite(field,cmap,[Target name])
            if b==1 & z ==1
                    imwrite(field,cmap,[Target 'index\' name])
            end
