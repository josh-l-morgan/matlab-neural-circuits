...
'Running Segments'
for i = 1: size(Dots,1)  
    DotDist=dist(Dots,Dots(i,:));
    MidDist=dist(Mids,Dots(i,:));
    %VolDist=dist(Dv,Mids(i,:));
...
for i = 1: size(Mids,1)
    for b = 1 : size(DBins,2)
        fieldC(round(Mids(i,1))+1,round(Mids(i,2))+1,b)= fieldC(round(Mids(i,1))+1,round(Mids(i,2))+1,b)+1;
    end        
end
fieldDD=fieldDD*0;
for i = 1: size(Mids,1)
    for b = 1 : size(DBins,2)
        fieldDD(round(Mids(i,1))+1,round(Mids(i,2))+1,b)=fieldDD(round(Mids(i,1))+1,round(Mids(i,2))+1,b)+LocalDD(i,b)/fieldC(round(Mids(i,1))+1,round(Mids(i,2))+1,b);
    end        
end
...
pause(.01)
clear field
scaleDD=round(255/(mean(LocalDD(:))*2));
while 1==1
for b = 1:size(DBins,2)
    field=fieldDD(:,:,b);
    field=field*scaleDD;
