...
%%and Skel for Bistratified cells
...
for k = 1:size(GetBIs,1)
    ...
    BC=Use.DPos; %Extract Dot Position
    ...
    if exist([TPN 'Rot.mat']), 
        eRot=1; load([TPN 'Rot.mat']),
        'Rotating'
        BC=CoRot(BC,Rot);
        SegMP=CoRot(SegMP,Rot);
    ...
    %% Enter Data to Structured Array
        ...
        Results.location=TPN;
        Back=find(TPN=='\');
        Results.name=TPN(Back(size(Back,2)-1)+1:Back(size(Back,2))-1)
        ...
        Results.CellStats.AverageDensity=TotalDots/TotalLength;
        ...
        %save Results
        save([TPN 'data\ResultsComp.mat'],'Results')
