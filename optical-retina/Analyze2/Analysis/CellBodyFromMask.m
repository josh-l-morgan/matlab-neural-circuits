...
RecordMasking={size(Kdir,1),2};
for k = 92:92%size(Kdir,1)
    TPN = [KPN '\' Kdir(k).name '\'];
    DPN = [TPN 'I\']   
     RecordMasking(k)=cellstr(TPN);
...
if exist([TPN 'mask']) & exist([TPN 'Dots.mat'])%% mask is image in folder 'mask' if exists
        ...
        RecordMasking(k,2)=cellstr(['mask found, CB = ' num2str(CBpos)]);
else
        RecordMasking(k,2)=cellstr('no mask found');
end
