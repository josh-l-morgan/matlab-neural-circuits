...
%% Get Threshold
if exist([TPN 'data/Threshold.mat'],'file')
    load([TPN 'data/Threshold.mat'])
else
Threshold=input('Enter Threshold for Dendrite (or run the damn Thresh finder) ==>')
end
...
image(max(Irm,[],3)*255/max(Irm(:))) %Image green
pause(.01)
...
%% Find statistics
stats=sort(Irm(1:min(10000000,ys*xs*zs))); %make one dimensional list of image values
dev=std(stats);
...
Thresh=Threshold;
save([TPN 'data/Thresh.mat'],'Thresh')
D=Irm>Thresh;
clear Irm
