    ...
    if exist([TPN 'BranchS.mat'])
        ...
        BranchMap=Branch.BranchMap;
           ...
           for ds=1:size(UseMid,1)
                DrawBranch(DrawSeg(UseMid(ds),1),DrawSeg(UseMid(ds),2))=1+Branch.DD(BranchMap(UseMid(ds)))*300;           
           end
       ...
       if exist([TPN 'Cell.mat'])
            ...
            Name=[KPN 'Images\SegDens\' cName];
        else
            Name=[KPN 'Images\SegDens\' Kdir(k).name '.tif'];
        end
        if isempty(find(Name=='?'));
            imwrite(DrawBranch,cmap,Name,'Compression','none')
        end
