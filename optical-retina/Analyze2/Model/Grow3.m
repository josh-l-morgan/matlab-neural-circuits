...
PreDens=.1;
PreSiz=siz*PreDens;
[Prey Prex]=find(ones(PreSiz));
Pre=[Prey Prex];
Pre(:,1)=Pre(:,1)*siz(1)/PreSiz(1);
Pre(:,2)=Pre(:,2)*siz(2)/PreSiz(2);
Pre(:,1)=Pre(:,1)-mean(Pre(:,1));
Pre(:,2)=Pre(:,2)-mean(Pre(:,2));
Pre=Pre+(randn(size(Pre))*siz(1)/PreSiz(1));
...
%     id = size(D,1);
    ...
    Tips=find(Tip & ~DCon & Live);
    for i = 1: size(Tips,1)
        Dists=sqrt((D(Tips(i),1,2)-Pre(:,1)).^2 + (D(Tips(i),2,2)-Pre(:,2)).^2);
        Dist=Dists<1;
        
        if sum(~PreCon(Dist))
            DCon(Tips(i))=1; %contact Post
        end
    ...
    %% Show Result
    Mids=mean(D(Live,:,:),3);
    IR=showvecS(Mids,siz);
    I(:,:,1)=IR;
    IG=showvecS(Mids(DCon(Live),:),siz);
    I(:,:,2)=IG*1000;
    IB=showvecS(Pre,siz);
    I(:,:,3)=IB;
