...
%%Figure out channels
nPlanes = size(d,1);
testPlane = round(nPlanes/2);
Ic(:,:,:)=imread([DPN d(testPlane).name]); %read
if size(Ic,3)==1, 'crash = only one channel read' %if only one channel
elseif size(Ic,3)==2, neurite=2; puncta=1; %if only two channels
elseif sum(sum(Ic(:,:,3)))==0, neurite=2; puncta=1; %if third channel is blank
else neurite=1; puncta=2; %if third channel is not blank
end
