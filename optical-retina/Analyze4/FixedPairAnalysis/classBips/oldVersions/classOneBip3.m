...
for i = 1: numWI
   ...
   if nodes == 1  % no joints
   elseif nodes == 2  % 1 joint (end point)
   else % muliple joints
...
for i = 1:size(jointMem,1)
    ...
    AllSeg(size(AllSeg,1)+1,:,:) = [cents(jointMem(i,1),:); jointPos(i,:)]';
    AllSeg(size(AllSeg,1)+1,:,:) = [cents(jointMem(i,2),:); jointPos(i,:)]';
