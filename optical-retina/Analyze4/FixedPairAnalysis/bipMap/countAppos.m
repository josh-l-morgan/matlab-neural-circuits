...
mkdir([rootD '\collectAppoImages'])
for i = 1: length(cellPos)
   if cellPos(i)
      bipID = cellPos(i);
      bipType = num2str(dat{cellPos(i),colBipClass});
      namCell = sprintf('type_%s_id_%.0f.tif',bipType,bipID);
      cellPath = [rootD '\collectAppoImages\' namCell];
      if exist([CellFolders{i} 'appo.tif'])
        appoPic = imread([CellFolders{i} 'appo.tif']);
        imwrite(appoPic,cellPath,'Compression','none');
      end
