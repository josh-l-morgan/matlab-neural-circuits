...
prompt = {'xy resolution : ', 'z resolution :'};
...
sDists=sqrt(((Sy-Smids(1))*xyum).^2 + ((Sx-Smids(2))*xyum).^2 + ((Sz-Smids(3))*zum).^2);
conR=conR(sDists<=Radz,:);

ShowS=zeros(Ssize);
ShowS(sub2ind(Ssize,conR(:,1),conR(:,2),conR(:,3)))=1; 
image(sum(ShowS,3)*100)
        ...
        while w<length(NewWave)
            ...
            while count>0  %while there are new voxels to do
                ...
                for t = 1:things
                    ...
                    if thingSize(t)>0%minFillSize

                        SDist=sqrt((Sold(:,1)-Snew(t,1)).^2+ (Sold(:,2)-Snew(t,2)).^2 + (Sold(:,3)-Snew(t,3)).^2); %find voxel distance from new to all old
                        source=find(SDist==min(SDist),1);

                        %Write Segments
                        SG=SG+1;
                        Seg(SG,1,1)=Sold(source,1);Seg(SG,2,1)=Sold(source,2); Seg(SG,3,1)=Sold(source,3);
                        Seg(SG,1,2)=Snew(t,1); Seg(SG,2,2)=Snew(t,2); Seg(SG,3,2)=Snew(t,3);
