...
for o=1:ob  %Run for All objects
    if sum(sum(sum(D==o)))>minObSize;
        ...
        while count>0  %while there are new voxels to do
            ...
            PrevF=OldWave{w};
            %PrevF=NewF;
            NewF=NewF*0;  %update list of new fill points
            ...
            for t = 1:things
                 ...
                 if thingSize(t)==max(thingSize)
                     NewWave{w}=Twave{t};   
                 end

                 if thingSize(t)>minFillSize
            ...
            if mod(SG,5)==0,  image((sumD>0)*255 - (sum(D,3)>0)*100),pause(.01),end
