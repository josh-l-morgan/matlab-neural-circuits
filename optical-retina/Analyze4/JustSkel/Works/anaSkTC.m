...
ImageInfo= {'50', '100','1',num2str(xyum),num2str(zum),'1.5','1','1','2'};


% 'getting image info', pause(.1)
% prompt = {'Minimum Branch volume (in voxels)', 'Minimum Object Size',...
%     'Skip Joints? (1=yes, 0 = no)',...
%     'xy resolution (um): ', 'z resolution :',...
%     'xy search (vox)','z search (vox) ',...
%     'minimum segment length: ', 'Number of Smoothings'};
% title = 'Define image resolution';
% nLines = 1;
% 
% ImageInfo= inputdlg(prompt,title,nLines,...
%     {'50', '100','1',num2str(xyum),num2str(zum),'1.5','1','1','2'});
% pause(.1)
% 
...
smoothNum= ImageInfo{9}; %number of times to smooth image
...
Iter=0;
EveryPic=100; % Show every X image when skeletonizing)
maxSegLength = 5;
...
sumD=sum(D,3);
colormap gray(256)
image(sumD*15/(mean(sumD(:)))),pause(.01)
...
Branches.Sizes=Sizes;

save([TPN 'Branches.mat'],'Branches')
