...
for k = 1:size(Kdir,1) %run all files in directory
    ...
    if strcmp('tif',DPN(size(DPN,2)-2:size(DPN,2))) %if a tif
        ...
        A=I*0+1; % create field of ones for area assignment
        [Ay Ax]=find(A);
        APos=[Ay Ax];
        clear Ay Ax
        maxDist=max(Dists(:));
        DUsed=((0:fix(maxDist))+.5)';
        Cnum=zeros(Dnum,size(DUsed,1),1);  %hist of cell bodies relative to each cellbody
        Anum=Cnum; %hist of area pixels relative to each cell body
        
        for c = 1:Dnum
            clear DistsA
            DistsA=dist(APos, DPos(c,:));  %Distance of each pixel to dot
