...
ImageInfo= inputdlg(prompt,Title,nLines,{'1','2','60','1','1024','2','1','50','0.115'});
...
for d = 1 : length(Bins)
    Cbin(d)=sum(Csum(max(Bins(d)-Rebin/2,2):min(Bins(d)+Rebin/2,size(Csum,1))));
    Abin(d)=sum(Asum(max(Bins(d)-Rebin/2,2):min(Bins(d)+Rebin/2,size(Asum,1))));
end
