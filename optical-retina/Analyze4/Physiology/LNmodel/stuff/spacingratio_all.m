...
for i=1:nCells
    [z beta0 xy] = gausspreparation (spikes(i).STA);
    ...
    %     hold on
    spikes(i).space.beta = beta;
end

SR = zeros(nCells);
for i=1:nCells
    xI = spikes(i).space.beta(2);
    yI = spikes(i).space.beta(3);
    for j=1:nCells
        if j > i
            xJ = spikes(j).space.beta(2);
            yJ = spikes(j).space.beta(3);
