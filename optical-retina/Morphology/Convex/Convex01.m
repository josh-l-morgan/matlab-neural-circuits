...
ImageInfo= inputdlg(prompt,title,nLines,{'1','1','2'});
...
prompt = {'input values? [1 = manual, 2 = gaussian, 3 = extremes]','Signal 1 [Ch1 Ch2] (R G)', 'Signal 2 [Ch1 Ch2] (R G)',...
...
if exist('.\PreviousSettings.mat')
    ...
    defAns = {'3',PreviousSettings.rS1, PreviousSettings.rS2, PreviousSettings.bg,PreviousSettings.kS};
else
    defAns = {'','', '', '','1 1'};
end
...
for o = 1:length(x)
    [y x]= find(NTpu);
    Dists=dist([y x],[y(o) x(o)]);
    ydif=y-y(o); xdif=x-x(o);
    for e = 1:length(x)
        for rep = 1:Dists(e)/res
            sy=y(o)+(ydif(e)/(Dists(e)/res)) * rep;
            sx=x(o)+(xdif(e)/(Dists(e)/res)) * rep;
    ...
    image(NTcon*100+NTp*100),pause(.01)
    NTpu(y(o),x(o))=0;
end
