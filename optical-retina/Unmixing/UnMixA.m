...
Rat=single(channel1)./single(channel2);
% [IDX,C] = kmeans(Rat(Cht12),2);
...
plot(channel1(Cht12).*(IDX==1)>0,channel2(Cht12).*(IDX==1),'.')
...
rawSignal1 = [mean(ChxSi1) mean(ChySi1)];
rawSignal2 = [mean(ChxSi2) mean(ChySi2)];
