...
BldStack = inputdlg(prompt,title,nLines,{'1'});
...
prompt = {'manually input values? [1 if yes]','Signal 1 [Ch1 Ch2] (R G)', 'Signal 2 [Ch1 Ch2] (R G)',...
...
if exist('.\PreviousSettings.mat')
    ...
    defAns = {'0',PreviousSettings.rS1, PreviousSettings.rS2, PreviousSettings.bg,PreviousSettings.kS};
else
    defAns = {'','', '', '','1 1'};
end
...
if BldStack == 2
    DPN = GetMyDir;
    DPNd=dir(DPN);DPNd=DPNd(3:length(DPNd));
    c=0;
    for i = 1:length(DPNd)
       Nam = DPNd(i).name;
       if sum(Nam(length(Nam)-3:length(Nam))=='.tif')==4
           ...
           if c==1
              Is=imread([DPN Nam]);
              [ys,xs,cs]=size(Is);
              I=zeros(ys,xs,cs,length(DPNd),'uint16');
              I(:,:,:,c)=Is;
           else
            I(:,:,:,c)=imread([DPN Nam]);
           end
...
Rat=single(channel1-BackC1)./(single(channel2-BackC2)+single(channel1-BackC2));


obj = gmdistribution.fit(Rat(Cht12),2);
RatMeans=obj.mu;
Ch2r=1./(RatMeans+1);
Ch1r=1-Ch2r;
