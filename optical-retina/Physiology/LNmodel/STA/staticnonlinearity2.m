...
channelsToBeDeleted = [1 2 3 9 14 17 19]; %user-defined parameter
mcStart = analog(28,1);
mcStop = analog(28,2);
...
%             randn('state',j - nTimeBins + k)
    ...
    rateFit0 = [10, -muSeed, max(outputRate), min(outputRate)];
    %rateFit0 =[sensitivity, maintainedDrive, maximum, minimum]
    [rateFit] = nlinfit(input, outputRate, @inputoutputfit,...
    ...
    spikes(i).outputProbability = outputProbability;
    spikes(i).outputRate = outputRate;
    spikes(i).logicTrain = logicTrain;
    spikes(i).probabilityTrain = probabilityTrain;
    spikes(i).probabilityFit = probabilityFit;
    spikes(i).rateFit = rateFit;
    save currentfile spikes
    clear spikes
