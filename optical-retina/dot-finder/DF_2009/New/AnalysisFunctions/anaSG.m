...
if isfield(Settings,'FirstThresholds')  % Check if FirstThresholds has been created
    if length(fieldnames(cP)) == length(fieldnames(Settings.FirstThresholds))
...
%% Find internal contrast (max - min brightness withing puncta)
for i = 1: Dots.Num
   brights = Dots.Vox(i).Igm; 
   brights = sort(brights);
   LB = length(brights);
   numBrights = fix(LB/10)+1;
   topB = brights(LB-numBrights:LB);
   bottomB = brights(1:numBrights);
   dP.internalContrast(i) = (mean(topB)-mean(bottomB));
end


Classify.dotProperties = dP;
...
%%Retrieve any absolute criteria for classifier
Only = ~(Dots.Cut == cP.cut) & Singles;
Classify.only = Only; 
...
if isfield(Settings,'guideThresholds')  % Check if FirstThresholds has been created
    if length(fieldnames(gP)) == length(fieldnames(Settings.guideThresholds)) 
...
if ~isempty(Miss)
    ...
    if gP.reset, gP = deFault; end % If reset was pressed, reset values to defalut
