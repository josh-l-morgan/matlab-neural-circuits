function[] = skeletonization(TPN) %#ok<FNDEF>
% SKELETONIZATION: skeletonizes neurites by propogating activated wave
% through a mask of the cell with the following values = materials:
% 0 = background, 1 = neurite, 2 = erroneous signal, 3 = soma. The output
%is an n x 3 x 2 matrix (allSegments) which contains x-y-z (3) coordinates
%of beginning and end (2) of all (n) segments of the skeleton. 

load([TPN 'Settings.mat']) % load user defined input settings

cellPath = TPN;
maskPath = [TPN 'mask\'];  %find the mask
maskFiles=dir(maskPath); maskFiles=maskFiles(3:end); % get names of files

% make sure the paths for saving results of skeletonization exist
% if not create them
...
disp('reading image mask ')
mask = zeros(yDim, xDim, zDim);
i = 1;
while i <= zDim %until all planes are read
    if istif(maskFiles(i).name) %see if current file is a tif
        mask(:,:,i) = imread([maskPath maskFiles(i).name]); %read
        i = i + 1;
    else
    end
end
mask = mask==1; %convert to logical discard all error(2) and soma(3) entries       
save ([cellPath mask])  
