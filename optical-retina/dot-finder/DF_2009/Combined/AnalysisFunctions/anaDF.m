...
%% Dot Finder 
% find punctate regions of image using iterative thresholding
...
%%Dot criteria
step=single(2); %Sensitivity=grey value step of iterative threshold (2 was standard)
...
if exist('Post') ==0, load([TPN 'Post.mat']); end %load green
Ig=single(Post(ystart:yend,xstart:xend,zstart:zend)); clear Post %subsample
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
'iterative threshold',
centroid=Ig*0; %set up matrix to map centroids
IgmTsum=Ig*0; %set up matrix to sum passed thresholds

%% Find Image Stats
IgStat = sort(Ig(:));
IgStat = IgStat(IgStat>0);
lowpoint=IgStat(1); %find top and bottom
highpoint=IgStat(length(IgStat)); 
%%Determine intensity value at which 'PercentBackground' is excluded from search
Gmode = IgStat(round(PercentBackground*length(IgStat)));
% Gmode = mode(single(IgStat(IgStat>0)));

 for i = fix(highpoint)+2:-step:Gmode %run thresholds through all relevant intensities

     ['Running ' num2str(i) ' from ' num2str(highpoint) ' to ' num2str(Gmode)]
       ...
       for p=1:labels  %run all lables
           if IglH(p)< MaxDot & IglH(p)>MinDot  %% Morphology Filter ! Puncta size criteria !
            %% Find Peaks to make centroid  (check against Josh 08)
...
Dotmap=uint16(zeros(ys,xs,zs));  %make 3D map where positions are labled with dot ID
...
for n=1:numdots%run dots
    ...
    peak=Ig(Py(n),Px(n),Pz(n)); %value at centroid
    ...
    if sum(sum(sum(Dotmap(ContourL==Ctarget)>0)))>1 % if contour includes more than 1 centroid
        ...
        for a=1:numa %run all pixels
            ...
            closest=find(AtoPD==min(AtoPD),1);
...
clear ContourT ContourL ContourD Ig ay ax az
clear  round sym Dotmap Ig 
%}
...
load([TPN 'Post.mat']);
for i = 1: Dots.Num
    Dots.Vox(i).RawBright=Post(Dots.Vox(i).Ind)
