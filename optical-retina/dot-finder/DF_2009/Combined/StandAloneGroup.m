...
G3d = zeros(Dots.ImSize,'uint8');
for i = 1 : length(Grouped.Vox)
    Posses=Grouped.Vox{i};
    gInds=sub2ind(Dots.ImSize,Posses(:,1),Posses(:,2),Posses(:,3));
    G3d(gInds)=rand*240+5;
end
imwriteNp([TPN 'pics\Grouped'],G3d,'G3d')
