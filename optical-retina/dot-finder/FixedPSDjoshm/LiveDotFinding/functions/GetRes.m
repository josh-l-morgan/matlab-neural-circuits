function [Res,stack] = GetRes(filename, img_first, img_last)
% Modification of tiffread2 to get resolultion from tag
...
if (nargin == 0)
    load('GetResLast.mat')
    [filename, pathname] = uigetfile(TPN,'*.tif;*.stk;*.lsm', 'select image file');
    TPN=pathname
    filename = [ pathname, filename ];
    if TPN~=0 
        save('C:\Notes\Notebook\matlab\Analyze2\functions\temp\GetResLast.mat','TPN')
    end
    Res.TPN=TPN;
end

if (nargin<=1);  img_first = 1; img_last = 1; end
...
%                if (entry.val ~= 1); error('Compression format not supported.'); end
            ...
            case 282         % X resolution
                ...
                Res.x_resolution=IMG.x_resolution;
                Res.res=1/IMG.x_resolution *10*1000;
            case 283         % Y resolution
                IMG.y_resolution   = entry.val;
                Res.y_resolution=IMG.y_resolution;
