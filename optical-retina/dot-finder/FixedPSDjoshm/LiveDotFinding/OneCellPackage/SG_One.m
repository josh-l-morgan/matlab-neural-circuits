...
for i = 1: size(P,2)
   ...
   maxPassed(mod(Dots.Vox(P(i)).Ind-1,YXsize)+1)=Dots.ITMax(P(i))*20+50; %ContrastV(P(i)); 
...
for i = 1: size(P,2)
   %maxID(mod(Dots.Vox(P(i)).Ind-1,YXsize)+1)=P(i);  
   maxPassed2(mod(Dots.Vox(P(i)).Ind-1,YXsize)+1)=Dots.ITMax(P(i))*20+50; %ContrastV(P(i)); 
end
