...
EdgeOfPeak=.75; %Determine Dot Edge = ratio of edge brightness to peak brightness (.5 was standard)
...
for i = 1:nLabels
    ...
    if nPeaks == 1
        cutOff = EdgeOfPeak * thresholdPeak;
        ...
        tMsamp=tMsamp>cutOff;
        LabSamp=bwlabeln(tMsamp,6);
        TargS=LabSamp(pMsamp>0);
        FinSamp=tMsamp==TargS;
        
        
        [yContour xContour zContour] = ind2sub(size(FinSamp),find(FinSamp));
        yContour=yContour+lystart-1; 
        xContour=xContour+lxstart-1; 
        zContour=zContour+lzstart-1; 
        if size(yContour,1)<size(yContour,2)
            yContour=yContour'; xContour= xContour'; zContour=zContour';
        end
        
        
        
        contourIndex=sub2ind([ys xs zs],yContour, xContour, zContour);
        ...
        Dots.Vox(next).Pos = [yContour,...
            xContour, zContour];
        Dots.Vox(next).Ind = contourIndex;
        ...
        pMsamp=peakMap(lystart:lystop,lxstart:lxstop,lzstart:lzstop);
        pMsamp(tLsamp~=i)=0;
%         image(max(tMsamp,[],3)*10),pause(.5) % !!!!!!!!!!!!! Delete for final
        ...
        for sy = 1:size(syPeak,1)
            passT=tMsamp>(pMsamp(syPeak(sy),sxPeak(sy),szPeak(sy))* EdgeOfPeak);
            ...
            if numPeaks == 1
                ...
                weight=(1./hightSc).*distance;  %assign wieghts

                [dummy, peakAllegiance] = min(weight,[],2);  %assign peaks
...
%         image(max(peaked,[],3)*20),pause(.01)

        clear peak
        for k=1:max(peaked(:))
            
            %%Clean Strays
            LabSamp=bwlabeln(peaked==k,6);
            TargS=LabSamp((pMsamp>0)&(peaked==k));
            FinSamp=LabSamp==TargS;
           
                      
            cI=find(FinSamp);
            [cSy cSx cSz]=ind2sub(size(peaked),cI);
            cSy=cSy+lystart-1; cSx=cSx+lxstart-1; cSz=cSz+lzstart-1;
            if size(cSy,1)<size(cSy,2)
                cSy=cSy'; cSx=cSx'; cSz=cSz';
            end
