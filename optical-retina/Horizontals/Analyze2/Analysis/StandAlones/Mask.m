...
%Get directory name
[DFN,DPN]=uigetfile('*.tif','DialogTitle','Choose first Image of Mask');
f=find(DPN=='\');
f2=f(size(f,2)-1);
f3=f(size(f,2)-2);
...
for i =1:size(DotStats,1)
  Dist=dist(mask,DotStats(i,:,3)); 
  mDotToMaskDist(i)=min(Dist);
  PercentDoneWithDots=double(i)/size(Dots,1)*100
  field=(max(1,
end
...
save([TPN 'data\DotStatsC.mat'],'DotStatsC')
