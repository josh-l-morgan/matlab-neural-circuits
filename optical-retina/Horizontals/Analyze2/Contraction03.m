...
Diverge=1;
while Diverge % loop untill all divergence is gone
Diverge = 0 
for p = 1 : Planes
    Availible=ones(CBs,1);
    for a = 1:CBs; Availible(Near(a,p))=0;end
    for i = 1:CBs  %search all source cells
       
        O=find(Near(:,p)==i);
        Occur(i,p)=size(O,1);
        if size(O,1)>1  %if multiple cells come from same source
            Diverge=1;
            for o = 1:size(O,1)
               DistO(:,o)=dist(CB(:,1:2,p-1), CB(O(o),1:2,p));
            end
            Closest=find(DistO(i,:)==min(DistO(i,:)),1);
            for o = 1:size(O,1)
                if o == Closest
                    Near(O(o),p)=Near(i,p-1);
                else
                    NextC=find(DistO(:,o)==min(DistO(Availible>0,o)),1); 
                    Near(O(o),p)=Near(NextC,p-1);
                end
            end
        end
        if Diverge,break,end
   end
   if Diverge,break,end
end
end % run until no divergence

if isempty(find(Occur==0))
    'No cells dropped out'
else
    'One or more cells dropped out'
end

if ~isempty(find(Occur==2))
    'Crap, need to manually track cells or r'
else    
    'No obviously ambiguous cells'
end
...
ExpChange=Changes(~Control); %% Collect change of experimentals
ControlChange=Changes(Control); %% Change of Control cells
...
ranksum(ExpChange,ControlChange)
%signtest(
