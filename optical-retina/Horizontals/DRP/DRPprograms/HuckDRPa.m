...
for k = 1:size(Kdir,1) %run all files in directory
    ...
    if strcmp('tif',DPN(size(DPN,2)-2:size(DPN,2))) %if a tif
        ...
        clear Ay Ax
        clear DistsA
        DistsA=zeros(Dnum,size(APos,1),'single');
        for d = 1:Dnum
            DistsA(d,:)=dist(APos, DPos(d,:));
            PercentDoneFindingDistances=d/Dnum *100
        end
        
        %% Map Density by distance Area
        maxDist=max(Dists(:));
        DUsed=(1:fix(maxDist)+1)';
        Cnum=zeros(size(DUsed,1),1);
        Anum=Cnum;
        for d=DUsed'
           Cnum(d)=sum(sum(Dists>d-1 & Dists <= d));
           Anum(d)=sum(sum(DistsA>d-1 & DistsA <= d));
           PercentDoneMapping=d/maxDist*100
        end
        CoA=Cnum./Anum;
        plot(CoA),pause(.01)
        bar(Cnum)
...
%% Bin
        Bin=10;
        for d = 1 : size(Anum,1)
            Cbin(d)=sum(Cnum(max(d-Bin/2,1):min(d+Bin/2,size(Cnum,1))));
            Abin(d)=sum(Anum(max(d-Bin/2,1):min(d+Bin/2,size(Anum,1))));
        end
        CAbin=Cbin./Abin;
        plot(CAbin)
        bar(Abin)
